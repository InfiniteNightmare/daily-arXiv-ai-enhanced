...
TL;DR: 本文研究QLoRA在事实知识注入中的容量-保留权衡，提出OSM匿名化事实获取基准，发现rank控制获取-保留边界，低rank保留OOD但获取少，高rank获取强但遗忘严重。


<details>
  <summary>Details</summary>
Motivation: 参数高效微调(PEFT)通常被认为因只更新少量参数而能保留预训练能力，但这一假设是否成立取决于适配器容量本身。本文旨在系统研究QLoRA在不同rank下注入新事实知识时，获取能力与保留能力的权衡关系，挑战PEFT"天然安全"的普遍认知。

Method: 基于OpenStreetMap构建匿名化事实获取基准(14个小城市的1,938条训练样本)，将实体名称替换为合成标识符以减少对预训练知识的依赖。对Qwen3-4B比较全微调(FFT)与QLoRA(rank 8/16/32/64)，从三个维度评估：(1)事实获取(EM准确率+释义泛化)；(2)OOD保留(HumanEval、IFEval、TruthfulQA、MMLU-Redux、BBH)；(3)模型漂移诊断(KL散度、RMS权重漂移、SVD入侵维度)。还用Qwen3-1.7B做标准LoRA对照实验，以及数学任务对照实验。

Result: 1) QLoRA rank诱导清晰的获取-保留边界：低rank(r=8)OOD保留好(70.7%)但事实获取弱；高rank(r=64)事实获取达83-90%但OOD降至28.1%；FFT作为保守基线，保留好但不达最高获取区。2) 模型漂移诊断与行为一致：rank越高，KL散度、权重更新范数、SVD入侵维度越大。3) 标准LoRA在Qwen3-1.7B上复现相同趋势(76%→79%→86%释义准确率，57.0%→52.0%→40.2% OOD)。4) 数学任务中该边界显著弱化：FFT与QLoRA(r=16,32)性能几乎相同(42.5/42.6/42.0)，OOD下降都较小(1.58-2.23点)，说明边界在新事实注入场景最明显，而非技能强化场景。

Conclusion: QLoRA不应被视为知识注入的固有安全方法；适配器rank控制可塑性权衡，决定新事实知识的安装程度与对预训练行为的扰动程度。该边界效应不依赖于量化(标准LoRA同样存在)，但在数学等技能强化任务中显著弱化，因此结论主要适用于需安装全新事实关联的场景而非通用微调。
...
TL;DR: 本文针对大语言模型在统计报告生成中出现的"统计声明失真"问题（包括数值漂移、效应方向反转、解读强度过强等），提出"声明锁定报告"（Claim-Locked Reporting）协议：先于文本生成之前固定每条可报告声明的证据来源、数值、方向和允许的语言强度，LLM仅负责撰写衔接性散文。在fMRI功能连接和RCT（Evidence Inference 2.0）两个场景下，跨种子复现率较混合模板基线分别提升37.4和20.5个百分点；盲法人工审计验证了方向保持和治理趋势。


<details>
  <summary>Details</summary>
Motivation: LLM生成的统计报告虽能流畅表达统计证据，但仍会出现数值漂移、效应方向反转、将阈值化对比表述为绝对效应等问题。现有控制方法（文本级提示/检索/结构化输出/后验校验，以及槽位级确定性渲染）均无法在生成前固定"哪些统计声明可被报告"，导致证据承载内容在不同运行间持续被采样，存在可靠性风险。本文将此问题重新定义为控制问题：报告的证据承载内容应由结构化统计结果决定，而非由散文生成过程采样。

Method: 提出声明锁定报告（Claim-Locked Reporting），即"先溯源后行文"（Provenance Before Prose）协议，包含四个组件：①声明构建器（Claim Builder）将结构化证据记录转换为声明账本（Claim Ledger），每条声明绑定证据指针、数值、效应方向、文献支持级别及允许语言强度；②风险审计器（Risk Auditor）按预设统计报告风险分类（数值支持、方向保持、实体/来源支持、推理范围、解读强度）输出只读审计记录；③策略控制器（Policy Controller）对被标记声明单调下调允许语言强度，并累积写入约束，禁止其上修；④确定性渲染器（Deterministic Renderer）从账本直接发射数值与方向标签，LLM仅撰写衔接散文。评估采用三条轴线：跨种子Jaccard复现率（Repro.）、声明治理（Strong/Hedge复合）、单次运行正确性（Num数值审计+盲法人工方向审计）。fMRI场景使用自建肥胖队列（428人）与HCP S1200队列（712人），RCT场景使用Evidence Inference 2.0方向性子集（200条记录）。比较七种方法（五种文本/检索/后验基线+混合模板+声明锁定），含两个LLM供应商（Moonshot kimi-k2.6与DeepSeek deepseek-v4-pro）和多种子。组件分析区分构建器单独、构建器+渲染器、完整流水线三种配置。FActScore与SelfCheckGPT作为标量诊断指标辅助比较。

Result: fMRI场景：声明锁定复现率达98.5%，较混合模板（61.1%）提升37.4个百分点（配对自助法95% CI [+15.1, +59.7]）；强断言句数从2.25降至0.75（CI [-2.05, -1.00]）；HCP子集同样达98.0% vs 62.0%。RCT场景：复现率达100.0%，较混合模板提升20.5点（CI [+17.8, +23.1]）；混合模板在RCT上展示出可靠性权衡——虽然原始Num标志最低（0.11 vs 0.23），但人工校准显示其2.5%输出含未支持的统计结果，声明锁定为0%；盲法方向审计中基线反转率5.6%–14.3%，声明锁定为0%（4例未判定）。fMRI治理盲审中四位标注者均将声明锁定评为违规最少（强语言2.55/1.85/0.60，未支持内容1.65/2.33/0.53）。成本层面（DeepSeek），声明锁定输入/输出token与中位延迟均最低（12482/4209 token，77.3秒）。组件分析显示：渲染器将fMRI复现率从85.0%提至98.0%；风险审计器+策略控制器主要抑制强断言。FActScore倾向字面支持，而SelfCheckGPT倾向一致性，二者与方法排名不一致（声明锁定在FActScore最低0.552却在SelfCheckGPT最高0.974），表明二者衡量的可靠性性质不同。

Conclusion: 声明锁定报告通过将控制单元从"文本"和"槽位"上移至"声明级"，在生成前固定可报告声明的证据来源、数值、方向与允许语言强度，显著提升统计报告的跨运行稳定性、方向保持与治理合规性，且降低写作侧计算开销。其揭示的核心教训是：单纯确定性渲染不足以保障可靠性——当LLM仍可选择槽位内容时，未支持的统计结果仍会出现；必须在生成前固定"哪些证据绑定声明可被渲染"。该协议不替代统计验证、机制发现或上游真理性保证，无法纠正上游预处理、模型设定或证据提取中的错误；自动指标为协议测试而非专家判断替代品，文献支持标签仍需专家校验。
...
### [73] [When Does Frequency Decomposition Benefit Physics-Informed Neural Networks? A Preliminary Ablation Study](https://arxiv.org/abs/2608.24940)
*Shubham Rai*
...
### [74] [FAMPWQ: Fisher Information-based Adaptive Mixed Precision Weight Quantization for Effective LLM Inference](https://arxiv.org/abs/2608.24945)
...
### [75] [CAT-GS: Balanced Multimodal Learning via Calibrated Gating and Fusion Surgery](https://arxiv.org/abs/2608.24947)
...
### [76] [Demystifying Reinforcement Learning Post-Training of Language Models](https://arxiv.org/abs/2608.24949)
...
### [77] [Resource-Efficient Pruning for Transformer via Low-Rank Importance Estimation](https://arxiv.org/abs/2608.24973)
...
### [78] [Clearing the Underbrush: AI-Enhanced RF Interference Suppression](https://arxiv.org/abs/2608.24974)
...
### [79] [D$^3$-MOPD: Adaptive Dynamic Domain ScheDuling for Efficient Multi-Teacher Distillation](https://arxiv.org/abs/2608.24987)
...
### [80] [DeMMO: Longitudinal and Cross-Disease Modelling of Digital Mobility Outcomes via Multi-Task Learning](https://arxiv.org/abs/2608.25073)
...
### [81] [NVExplain: Explaining Time Series Forecasting with Latent Trajectory Analysis and Structure-Preserving Surrogates](https://arxiv.org/abs/2608.25080)
*Muyan Anna Li,Manikandan Ravikiran,Aditi Gautam*

Main category: cs.LG

TL;DR: NVExplain 是一个面向时间序列预测的模型无关事后可解释性框架，通过分析模型潜在表征中的"语义流"(semantic flow) 来量化信息随时间的演化，并构建滞后期-预测步的归因矩阵以提供 horizon 级别的解释。该方法还引入了保持结构的扰动策略来拟合稀疏局部代理模型，生成可读的局部解释。实验表明其语义流变体在多个基准数据集上的忠实度( faithfulness) 优于或媲美 SHAP、Integrated Gradients、TimeSHAP 等基线，并显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测的事后可解释性方法（如 LIME、SHAP、TimeSHAP、Integrated Gradients）主要在输入空间内进行归因，存在三大局限：①扰动可能破坏时序连续性与季节性；②无法提供 horizon 特异性（即不同预测步对应不同历史影响）的解释；③忽略了模型内部表征随时间的演变。论文旨在解决这些局限，提出一种面向时序预测场景的、可解释且高效的归因框架。

Method: 框架分四个阶段：①**潜在轨迹构建**：对滚动上下文窗口调用模型的 embedding 接口，得到 latent 状态序列 Z_τ，必要时用预测值延拓轨迹；②**语义流计算**：用相邻 latent 状态的 L2 距离 m_τ = ‖Z_{τ+1} − Z_τ‖₂ 度量表征变化强度，可用 EMA 平滑；③**滞后期-预测步归因矩阵**：用 horizon 相关的指数核 w_h(a) = exp(−a/s_h) 对语义流进行加权聚合，并经 softmax 得到 A(j, h)，每列给出该 horizon 上各滞后期的重要性分布；④**结构保持扰动 + 稀疏局部代理**：通过分块 bootstrap 与傅里叶域幅度扰动生成时序一致的扰动样本，对每个 (channel, horizon) 拟合 locality 加权 L1 正则的稀疏线性代理，得到可读的局部解释。评估方面：忠实度采用基于滞后期替换的 FPR、AOPC、Gap 指标；稳定性采用嵌入敏感性 R_t 与轨迹稳定性（过零率、方向翻转、抖动）诊断。

Result: 在 MOMENT 模型、四个 Nixtla 长期预测基准（ETTh1、Exchange、ILI、Weather）上的实验显示：Ours (flow) 在 ETTh1 上 FPR 达 99.1%（与 TimeSHAP 持平），在 Exchange（88.9%）、Weather（96.3%）上超越所有基线；AOPC 与 Gap 在多数数据集上均最高。运行时显著优于 IntGrad（数十到数百秒）与 TimeSHAP，通常仅需数秒。稳定性方面 Weather/ILI/Exchange 的嵌入敏感性 < 0.02，ETTh1 为异常值（0.37）。代理变体（full）在强非线性数据集（Exchange、ILI、Weather）上出现忠实度下降。研究指出 Weather 同时具备高忠实度与稳定 latent，是最可靠的解释场景。

Conclusion: NVExplain 通过潜在空间的语义流将预测归因到 horizon 特异的历史滞后期，提供了一种模型无关的、计算高效的事后可解释性方案；其语义流变体在大多数基准上获得最优或并行的忠实度，并显著降低计算成本。但代理模型阶段在强非线性场景下会牺牲保真度，应仅在局部线性可接受时启用。局限在于语义流组件需要访问 embedding 接口（灰盒假设）。未来工作将扩展到多变量预测、结合滞后期与特征级解释，并通过用户中心研究评估可读性。
...
### [82] [The Von-Neumann State-Space Transformer for neural decoding](https://arxiv.org/abs/2608.25088)
*Morteza Sarafyazd*
...
### [83] [Understanding the Energy Scaling of Large Language Model Inference Across Context Lengths and Attention Architectures](https://arxiv.org/abs/2608.25096)
...
### [84] [Flower Hub: A Reproducible Benchmarking Platform for Federated Learning in Simulation and Deployment](https://arxiv.org/abs/2608.25114)
...
### [85] [GRAPE: Gradient Refinement and Progress-Aware Exploitation for Query-Efficient High-Dimensional Bayesian Optimization](https://arxiv.org/abs/2608.25116)
...
### [86] [Toward Machine Learning with the Unit as a Primitive: Learning from Unit-Linked Events](https://arxiv.org/abs/2608.25118)
*Heyang Gong*
...
### [87] [When Does Context Routing Help? A Systematic Study of Multi-Modal Fusion in Time Series Forecasting](https://arxiv.org/abs/2608.25128)
...
### [88] [Drift Variation Autoencoder: Unifying Generation and Representation Learning through Conditional Posterior Flow Matching](https://arxiv.org/abs/2608.25138)
*Jiarui Cao*
...
### [89] [Bayesian Flow Networks for Offline Trajectory Planning](https://arxiv.org/abs/2608.25163)
...
### [92] [Output Dilution: Redundant but Fragile Representations in MoE Models](https://arxiv.org/abs/2608.25231)
...
### [93] [Neural-Bayesian Structure Learning for Discrete Choice Modeling](https://arxiv.org/abs/2608.25258)
...
### [96] [Neither Precision Nor Architecture Alone: Controlled Tests of Failure Remedies for Physics-Informed Neural Networks](https://arxiv.org/abs/2608.25327)
...
### [97] [Escaping Low-Dimensional Overlap: Multi-Task Model Merging via High-Dimensional Sparse Disentanglement](https://arxiv.org/abs/2608.25354)
...
### [98] [Refusal geometry reflects refusal training: diverse refusal prefixes can raise stable rank and weaken refusal vector ablation attacks](https://arxiv.org/abs/2608.25390)
*Andrey Labunets*
...
### [99] [Physics-Informed Foresight Pruning for Sparse PINN Solvers of Nonlinear PDEs](https://arxiv.org/abs/2608.25564)
...
### [100] [LDAC-Net: A Learnable Multi-Lag Differencing Attention-Convolution Network for Drift-Robust Recognition with Low-Cost MOX Gas Sensors](https://arxiv.org/abs/2608.25646)
...
### [101] [It's a matter of timescale: non-linear utility in successor features and multi-objective planning and learning](https://arxiv.org/abs/2608.25723)
...
### [102] [TailSFT: Filtered Fine-Tuning Improves Post-Training Performance](https://arxiv.org/abs/2608.25756)
...
### [103] [VINCENT: Validated Interaction Network for Cross-drug Explanation of Therapeutics](https://arxiv.org/abs/2608.25841)
...
### [104] [Towards A Unified Information Bottleneck Framework for Time Series Explanations](https://arxiv.org/abs/2608.25897)
...
### [105] [One Symptom, Three Levers: A Critical Review of On-Policy Self-Distillation](https://arxiv.org/abs/2608.25936)
...
### [106] [DualOPSD: Adaptive Privileged Teachers for On-Policy Self-Distillation](https://arxiv.org/abs/2608.26019)
*Yutong Chen,Guangfu Guo,Zhichao Xu,Kunpeng Liu*

Main category: cs.LG

TL;DR: 本文提出DualOPSD，一种用于在线策略自蒸馏（OPSD）的非对称交替框架。该方法让特权教师在学生更新后，沿同一轨迹向更新后的学生分布靠拢，从而使后续监督信号能响应学习者的变化，无需额外rollout。在Qwen3系列的1.7B、4B、8B模型上的数学竞赛基准测试中，DualOPSD在4B和8B上显著优于固定教师的OPSD，但在1.7B上反而下降，表明教师适应效果与模型规模强相关。


<details>
  <summary>Details</summary>
Motivation: 现有OPSD方法将特权教师保持固定不变，但学生分布在训练过程中持续变化，输出风格也随之改变。固定教师无法响应被学生拒绝的风格信号，导致监督与学习者脱节。研究核心问题是：特权教学策略本身是否也应作为在线策略自蒸馏的一部分进行适应？

Method: DualOPSD采用非对称交替更新框架：(1) 学生使用逐点裁剪的KL散度（pointwise clipped forward KL）从特权教师学习，保留OPSD的裁剪机制以避免风格标记主导梯度；(2) 更新后，教师在相同学生轨迹上使用完整的反向KL（reverse KL）向更新后的学生分布靠拢；(3) 两者通过两个独立的LoRA适配器在同一冻结基础模型上实现，共享同一条学生采样轨迹，无需额外rollout或验证器。学生采用保守监督，教师接收完整的学习者信号，形成非对称闭环。

Result: 在Qwen3非思考模式下：AIME 2024/2025和HMMT 2025上，8B模型avg@12较OPSD分别提升23.61、13.89、10.00点；4B提升13.06、5.00、6.67点；但1.7B下降3.61/1.94/1.94点。所有规模下截断率均下降，4B的双向KL诊断显示教师-学生耦合度降低。训练成本仅增加约40%（4B: 1.0→1.4 A100小时）。消融实验验证了完整反向KL信号优于低裁剪变体（DualOPSD-C）。

Conclusion: DualOPSD将特权上下文自蒸馏转化为非对称闭环，有效降低截断率并提升中大模型（4B/8B）精度。然而精度增益高度依赖模型规模，1.7B上出现性能反转，说明该方法不具有规模无关性。限制包括仅覆盖单一模型家族、单一训练种子和有限的竞赛问题集。KL降低仅表明策略耦合变化，并不直接证明推理质量提升。模型规模是未来自适应特权教学工作中的核心变量。
...
### [107] [How Much Rank Does LoRA Need? Rank-Error Bounds for Transformer Attention](https://arxiv.org/abs/2608.26052)
...
### [110] [Resilient Decentralized Wireless Federated Learning via Gradient Tracking with AdamW](https://arxiv.org/abs/2608.25535)
...
### [111] [Cooperative Multi-Agent Reinforcement Learning for Adaptive Aggregation in Semi-Supervised Federated Learning with non-IID Data](https://arxiv.org/abs/2608.25794)
...
### [112] [Dynamic Influence-Weighted Distillation for Single-IMU Activity Recognition](https://arxiv.org/abs/2608.24904)
*Bingxuan Xie*
...
### [113] [MacroAgent: Regularity-Aware Macro Legalization with LLM-Agent-Designed Contour Algorithms](https://arxiv.org/abs/2608.24946)
...
### [114] [AFDBench: A Reasoning-First AI Scientist for NationalWeather Service Forecast Discussions](https://arxiv.org/abs/2608.24954)
...
### [115] [Why and When Neural Networks Improve Local Approximation in Optimization](https://arxiv.org/abs/2608.24963)
...
### [116] [Physics-Informed Error Field Learning: A Post-Training Optimization Framework for Physics-Informed Neural Networks](https://arxiv.org/abs/2608.24970)
...
### [117] [MSR-IVA: Masked Structural Residual Independent Vector Analysis for State-Aware Fusion of Structural MRI and Dynamic Functional Network Connectivity](https://arxiv.org/abs/2608.24978)
...
### [118] [Rollout-Decoded Reconstruction for Long-Horizon Prediction in Latent World Models](https://arxiv.org/abs/2608.25017)
...
### [119] [On the Representational Geometry of Dynamic Programs](https://arxiv.org/abs/2608.25034)
...
### [120] [The Frame Kernel Method for Multiscale Operator Learning](https://arxiv.org/abs/2608.25084)
...
### [121] [Multimodal Injury Risk Prediction in Tennis](https://arxiv.org/abs/2608.25126)
...
### [127] [LibriBrain100: One Hundred Hours of Broad and Deep MEG Data for Neural Speech Decoding at Scale](https://arxiv.org/abs/2608.25204)
...
### [128] [Representing MAX functions using two-hidden-layer ReLU networks](https://arxiv.org/abs/2608.25221)
...
### [129] [Trust the Mass: Forced Weights in KV-Cache Eviction](https://arxiv.org/abs/2608.25230)
*Jack Shi,Jerry Gu*
...
### [130] [Long-Term Behavioral Evaluation for Trusted Collaborator Selection via Bidirectional Mamba](https://arxiv.org/abs/2608.25232)
...
### [131] [ShuttleArena: Interpretable Self-Play in Physics-Based Badminton](https://arxiv.org/abs/2608.25246)
*Peize Ding*
...
### [136] [Beyond Pairwise Feedback: Listwise Vision-Language Supervision for Preference-Based Reward Learning](https://arxiv.org/abs/2608.25350)
...
### [137] [PaSta: Noisy Node Classification with Partial Label Learning](https://arxiv.org/abs/2608.25365)
...
### [138] [Joint Initialization of Flux Networks and Effective Multiplication Factor for Physics-Informed Neural Networks Solving Neutron Diffusion Problems](https://arxiv.org/abs/2608.25443)
...
### [139] [Resolving Multi-Modal Regression by Difference-Quotient-Based Clustering:Fast Coarse Conditional-Label Assignment](https://arxiv.org/abs/2608.25467)
*Huang Weiquan*
...
### [140] [A Storage-Retrieval Gap in Parametric Knowledge Graph Memory](https://arxiv.org/abs/2608.25489)
...
### [141] [FedQoS: Federated QoS-Risk Learning for Heterogeneous Indoor-Outdoor Access Selection](https://arxiv.org/abs/2608.25496)
...
### [142] [Reflection Steering: Disentangling Reflection from Reasoning in Activation Space for Token-Efficient Inference](https://arxiv.org/abs/2608.25542)
...
### [143] [Interpreting Protein Language Model Embeddings via Orthogonal Projection for Protein Fitness Prediction](https://arxiv.org/abs/2608.25548)
...
### [144] [Beyond Optimal Rates in Stochastic Optimization: Trajectory-Adaptive Stopping Rules](https://arxiv.org/abs/2608.25551)
...
### [145] [Beyond Scaling: Self-Evolving LLM Agents for Hardware Kernel Optimization via an Experience-Driven Workflow and Experience Graph Memory](https://arxiv.org/abs/2608.25570)
...
### [146] [Individual Fairness in Hierarchical Clustering](https://arxiv.org/abs/2608.25586)
...
### [147] [M-Fibration Theory with Applications to Neural Network Compression](https://arxiv.org/abs/2608.25598)
*Paolo Boldi*
...
### [148] [Frequency-aware forecasting for short-term typhoon gust prediction](https://arxiv.org/abs/2608.25604)
...
### [149] [DCEO: Direct Causal Effect Optimization for Long-Term User Value Modeling in E-commerce Search](https://arxiv.org/abs/2608.25635)
...
### [150] [A Token-Level Analysis of Sampled-Token Reverse-KL On-Policy Distillation](https://arxiv.org/abs/2608.25643)
...
### [151] [Adversarial Training of Linear Models under Stealthy Attacks](https://arxiv.org/abs/2608.25681)
...
### [152] [Modeling spatio-temporal locality in multi-step forecasting of geo-referenced time series](https://arxiv.org/abs/2608.25698)
...
### [153] [Tropospheric temperature and humidity profile retrieval from Meteosat Flexible Combined Imager based on deep learning](https://arxiv.org/abs/2608.25700)
...
### [154] [Fairness-Aware Test-Time Prompt Tuning](https://arxiv.org/abs/2608.25707)
...
### [155] [Are LLM-Enhanced GNNs Privacy-Safe?](https://arxiv.org/abs/2608.25727)
*Longzhu He,Zelang Wen,Chaozhuo Li,Sen Su*

Main category: cs.LG

TL;DR: 本文系统评估了LLM增强型图神经网络（GNN）的隐私风险，提出统一评估框架，并发现语义增强虽提升模型效用，却显著加剧了针对链接、标签和成员推理三类隐私攻击的脆弱性，同时差分隐私防御会引入显著的效用下降。


<details>
  <summary>Details</summary>
Motivation: LLM增强型GNN通过融合语义信息显著提升了节点表示质量，但其隐私风险尚未得到充分研究。语义丰富化可能放大嵌入空间中的隐私敏感信号，带来比传统GNN更严重的隐私泄露隐患。

Method: 提出五阶段统一评估框架：①数据集准备（6个真实文本属性图数据集）；②受害者模型训练（6种LLM特征增强器×7种GNN骨干，共42种配置）；③隐私攻击（链接、标签、成员推理共6种方法）；④风险评估；⑤防御分析。实验在黑盒访问设定下进行，并以浅层文本嵌入（BoW/Word2Vec）为基线。

Result: 实验表明：①LLM增强型GNN在节点分类准确率上均优于浅层基线，证实了效用提升；②在链接推理（MLA/ELA）、标签推理（HGA/NIA）和成员推理（MIA/NMA）三类攻击下，LLM增强型GNN的泄露程度普遍高于浅层基线；③嵌入空间分析显示语义增强放大了类间/类内、连接/非连接、成员/非成员信号之间的可分性；④差分隐私防御（嵌入级/链接级/标签级）虽能部分缓解风险，但导致显著效用下降。

Conclusion: LLM增强型GNN存在固有的隐私-效用权衡：语义增强带来效用提升的同时也放大了隐私泄露风险，现有差分隐私机制难以兼顾两者。研究呼吁重新审视并设计面向LLM增强型图学习的隐私保护机制。
...
### [156] [Why Does Graph Learning Fail to Fully Benefit from a Text Teacher?](https://arxiv.org/abs/2608.25741)
...
### [157] [A Constitutive Markov Physics-Informed Neural Operator (MPNO) for Autoregressive Stability in Transient Dynamics](https://arxiv.org/abs/2608.25744)
...
### [158] [Comparing Corrupted Constrained Learning Problems](https://arxiv.org/abs/2608.25745)
...
### [159] [Learning from waste: Machine Learning for health risk prediction and computer vision-based sorting in Ghana](https://arxiv.org/abs/2608.25759)
...
### [160] [Drift-Aware Multimodal User Representation Learning via Multi-Scale Temporal Modeling and Sparse Mixture-of-Experts](https://arxiv.org/abs/2608.25773)
...
### [161] [EXAONE Tabular 1.0 : Technical Report](https://arxiv.org/abs/2608.25774)
...
### [162] [Geometry-Constrained Kolmogorov-Arnold Networks: Learning Edge Geometry via Banach Duality](https://arxiv.org/abs/2608.25807)
*K S Sesh Kumar*
...
### [163] [Canalization Before Generalization: Grokking as a Dynamical Probe](https://arxiv.org/abs/2608.25813)
*Yiming Lin*
...
### [164] [Learning Continuous Regional Temperature Fields with Lead-Time and Resolution Queries](https://arxiv.org/abs/2608.25823)
...
### [165] [CEDAR: Controlled and Event-Driven Demand Forecasting via Residual Decomposition](https://arxiv.org/abs/2608.25871)
...
### [166] [How Edge of Stability Hinders SCAFFOLD in Federated Optimization](https://arxiv.org/abs/2608.25873)
...
### [167] [A General-Purpose Molecular Foundation Model Transfers Across Diverse Olfactory Tasks](https://arxiv.org/abs/2608.25893)
...
### [168] [Forecasting Multiple Observables with SCROLL: Score-Trained Uncertainty for Stochastic Dynamics](https://arxiv.org/abs/2608.25898)
*Pavel Prochazka*
...
### [169] [Quantum-Inspired Modeling of Driving Behavior](https://arxiv.org/abs/2608.25907)
...
### [170] [When Pruning Meets Interpretability: Preserving Sparse Autoencoder Robustness in LLMs](https://arxiv.org/abs/2608.25941)
...
### [171] [Spectral Allocation: Why Muon Outperforms Adam, and How to Improve Muon](https://arxiv.org/abs/2608.25990)
...
### [172] [Robust CurveMoE: Multi-Norm Adversarial Defense for Mixture-of-Experts Models via Mode Connectivity](https://arxiv.org/abs/2608.26043)
*Xu Zhang,Ren Wang*
...
### [173] [Group-Shared Low-Rank Approximation for Mobile-Efficient Pointwise Convolutions in Large-Kernel CNNs](https://arxiv.org/abs/2608.26069)
...
### [175] [Probabilistic Performance Analysis of Parallel Signature Search Strategies in Multi-Level Tree Networks](https://arxiv.org/abs/2608.25087)
...
### [176] [Hierarchical Shared Memory-Aware Optimization for TRSM on GPU Platforms](https://arxiv.org/abs/2608.25469)
...
### [177] [psRL: Efficient Training for Agentic AI via Training-Time Prefix Sharing](https://arxiv.org/abs/2608.25683)
...
### [178] [An Oversubscription and Service Pricing Exploitation-Based Profit Maximization Framework for Industry Cloud Resource Management](https://arxiv.org/abs/2608.25712)
...
### [179] [A Spatially-Aware Publish-Subscribe Middleware for IoT Applications](https://arxiv.org/abs/2608.25728)
...
### [180] [REE-TM: Reliable and Energy-Efficient Traffic Management Model for Diverse Cloud Workloads](https://arxiv.org/abs/2608.25747)
...
### [181] [Quantum Blackhole Learning-Optimized Hadamard Neural Network Model for Dynamic Resource Reservation in Industry Clouds](https://arxiv.org/abs/2608.25754)
...
### [182] [Scalable Multi-GPU Simulation of 3D Multicellular Growth with RNN-Based Workload Balancing](https://arxiv.org/abs/2608.25890)
...
### [183] [Slasher: Power Flexibility for Cloud Datacenters](https://arxiv.org/abs/2608.26021)
...
### [184] [Evaluating and Preventing Security Smells in AI-Generated Ansible Code](https://arxiv.org/abs/2608.24962)
...
### [185] [ARISMA: Guidelines for AI- and LLM-Assisted Systematic Reviews, Scoping Reviews, and Mapping Studies](https://arxiv.org/abs/2608.25050)
...
### [186] [SPECMINE: A Large-Scale Corpus of Spec-Driven Development Artifacts](https://arxiv.org/abs/2608.25202)
...
### [187] [A Few Pages of Markdown: Committed AI Configuration and Lower Quality Cost after Coding-Agent Adoption](https://arxiv.org/abs/2608.25241)
...
### [188] [RotDroid: Cross-Orientation State Equivalence Testing for Detecting GUI Rotation Bugs in Android Apps](https://arxiv.org/abs/2608.25425)
*Mengdi Qin,Bo Jiang*
...
### [189] [A Hybrid Usability Approach for Rating Evaluation of M-Commerce Applications](https://arxiv.org/abs/2608.25550)
...
### [190] [From General Agents to RCA Experts: A Self-Evolving Harness for Root Cause Analysis](https://arxiv.org/abs/2608.25661)
...
### [191] [Secret MCP: Evidence-Bounded and Context-Isolated Design Specification Generation from Web Screenshots](https://arxiv.org/abs/2608.24944)
*Yeongjin Jo*
...
### [192] [The Evolution of Binary Decompilation in the Modern Era: A Taxonomy, Literature Review, and Future Perspectives](https://arxiv.org/abs/2608.24955)
...
### [193] [Model-Based Agentic Software Engineering](https://arxiv.org/abs/2608.25174)
*James C. Davis,Kelechi Kalu,Huiyun Peng,Parth V. Patil*

Main category: cs.SE

TL;DR: 本文提出 MAGE（Model-Based Agentic Software Engineering），一个针对编码智能体时代的可信自主软件工程理论与框架。其核心主张是：随着编码智能体使实现能力变得充裕，工程进展的瓶颈转向"治理工程环境"——通过建模（Modeling）将关键工程知识外化为最小化、目的明确的表示，通过对齐（Alignment）为已确定的义务赋予适当权威（约束、传感器、验证器、闸门），将重复性重构与判断转化为可继承的持久工程结构。


<details>
  <summary>Details</summary>
Motivation: 编码智能体大幅提升实现产出率，但项目意图、系统结构与验收证据并不会自动显性化。已有方法（更大的提示词、仓库检索、逐变更审查）仍要求智能体与工程师反复重构关键工程属性，在实现产能相对于工程判断变得充裕时，重构与裁决成为自动化与人工监督的双重瓶颈。核心问题由此转为：在实现丰裕条件下，如何让关键工程属性可被推理、评估与治理，而无需反复从代码中重建。

Method: 采用探索性、解释性的理论建构设计，分两阶段：(1) 纵向案例研究——以作者主导的 DocAble（文档可访问性系统，20 周全职开发，约 54 万行生产代码、160 万行支持性治理基础设施）作为经验种子，重构工程事件，识别机制与时间顺序；(2) 比较重构——对 Cloudflare、Spotify、Shopify、Docker、Siemens、Zenseact 六份独立报告的工业实践进行同框架解读，识别机制重现、替代实现与范围条件。两源证据通过反复迭代与从业者交流逐步精炼出 MAGE 概念，并提出 P1–P4 方向性命题及 H1–H3 可证伪假设与配套实验方案。

Result: 实证层面：DocAble 仓库支持装置与生产代码之比由原型期的 0.85× 上升至成熟期的约 3×，硬化期峰值 3.68×；项目专属 lint 从 0 增至 747，闸门脚本从 0 增至 102；208 次提交将修复与防复发 lint 配对；派生检查捕获 6 次模型–代码漂移实例；建模序列使未建模实现元素占比由 56% 降至 7.89%。初步 in situ 实验显示，显式建模使用指引可减少 token、轮次与耗时，对较弱模型（Sonnet）效应更明显。理论层面：MAGE 由建模原则、对齐原则、治理工程环境与工程资本构成双重反馈环路（压力–适应平衡环、能力–放大增强环），语义差距（semantic gap）被定义为义务产生点与环境具备足够语义、证据与权威以判定满足之间的差距。

Conclusion: MAGE 是一个针对智能体时代软件工程的可信自主理论框架，强调将意图与知识外化、为义务赋予权威、把反复判断沉淀为可继承结构。其将规模与保证强度解耦，可适用于从轻量表示到形式化推导的全谱系，将形式化方法视为其一种实例化。论文同时提出可证伪研究议程（H1 表示杠杆、H2 表示完整性、H3 表示诱导判定化），并主张商品化智能体使实现能力与工程环境比以往更可分离，从而为长期悬而未决的"工程结构如何贡献软件结果"问题提供更具实验可分性的研究载体。
...
### [194] [Metis: Typed Runtime Mediation for Tool-Using Software Agents](https://arxiv.org/abs/2608.25322)
*Jun Yu*
...
### [195] [Point-in-Time Audit Before Alpha: Public-Archive Availability and a Negative Matched-Budget Study on BTC Perpetual Futures](https://arxiv.org/abs/2608.25348)
...
### [196] [Retry Amplification in Distributed Systems: A Systematic Analysis of Retry Policies and Their Role in Cascading Failures](https://arxiv.org/abs/2608.25403)
...
### [197] [Predicting Struggling Students in CS1 Programming Using Keystroke-Level Editing Features](https://arxiv.org/abs/2608.25769)
...
### [198] [Beyond the Editing Canvas: Evidence Divergence in OOXML-to-LLM Ingestion](https://arxiv.org/abs/2608.25880)
...
### [199] [Answer Is Cheap, Show Me the Evidence! Augmenting Automated Vulnerability Assessment with Evidence](https://arxiv.org/abs/2608.25905)
...
### [200] [XREPOTEST: Benchmarking Multilingual Repository-Level Unit Test Generation for Large Language Models](https://arxiv.org/abs/2608.25939)
...
### [201] [Vulnerable Code Search: Transferable Attack for Code Language Models](https://arxiv.org/abs/2608.26031)
...
### [202] [Praxist: From Experimental Artifacts to Solution Lineages](https://arxiv.org/abs/2608.25955)
...
### [203] [BVR Sim: An Open and High-Throughput Environment for Heterogeneous Air-Combat Reinforcement Learning](https://arxiv.org/abs/2608.25419)
...
### [204] [Trust-Aware Sequential Decision Making and Rollout Planning for Resilient Multi-Robot Systems](https://arxiv.org/abs/2608.25690)
...
### [205] [HypoForge: A Self-Improving Multi-Agent Framework for Automated Hypothesis Generation and Testing via Scientific Skill Learning](https://arxiv.org/abs/2608.25770)
...
### [206] [AI Agentic Selective Laser Sintering Process Optimization](https://arxiv.org/abs/2608.25928)
...
### [212] [Self-Explanation Tutor for Active Study of CS1 Worked Examples](https://arxiv.org/abs/2608.25180)
...
### [213] [Rare Diseases, Common Dilemmas: LLMs Prioritize Equal Resource Distribution over Patient Benefit in Decision-Making](https://arxiv.org/abs/2608.25236)
...
### [214] [Spatial-Knowledge-Graph-Grounded LLM Agents for Neighborhood Livability Evaluation](https://arxiv.org/abs/2608.25952)
*Haiyan Hao*
...
### [215] [GGSS: Geodesic-Gated Spherical Steering for Inference-Time Debiasing of Generative Vision-Language Models](https://arxiv.org/abs/2608.25375)
...
### [216] [The AI Adaptation Gap in Higher Education: Students, Faculty, and Administrative Staff](https://arxiv.org/abs/2608.25063)
...
### [217] [Framing War Across Languages: Power, Agency, and Sentiment in Wikipedia's Multilingual War Narratives](https://arxiv.org/abs/2608.25337)
...
### [218] [Toward a Threat Actor Profiling Taxonomy for Pre-Release Risk Management of Open-Weight Frontier Models](https://arxiv.org/abs/2608.25361)
*James Zhang*
...
### [219] [CompanionHarm: A Multi-Turn Benchmark for Detecting Harms in Real-World AI Companion Conversations](https://arxiv.org/abs/2608.25377)
...
### [220] [GenAIT: Development and Validation of an Objective Generative AI Literacy Test for High School Students](https://arxiv.org/abs/2608.25815)
...
### [221] [Non-Great-Power Conflict and AI Risk](https://arxiv.org/abs/2608.25839)
...
### [222] [Giving Mechanical Engineers Intelligent Tools: A Project-Based AI Education Curriculum in Thermal Engineering](https://arxiv.org/abs/2608.26056)
...
### [223] [Data Citation for Large Language Models: A Challenge](https://arxiv.org/abs/2608.25663)
*Gianmaria Silvello*
...
### [224] [VisDocAgentBench: Benchmarking Agents for Visually Rich Document Retrieval](https://arxiv.org/abs/2608.17889)
...
### [225] [CRAMER: Control via Request-Aware Masking for Editing Recommenders](https://arxiv.org/abs/2608.25370)
...
### [226] [When Stale Constraints Go Unchecked: Budgeted Verification Failures in Inherited Agent Memory](https://arxiv.org/abs/2608.25553)
*Kazuki Nakayashiki*
...
### [227] [The "Curse of Knowledge" in LLM Query Simulation: Concept Provenance for Tracing Answer-Side Intrusion](https://arxiv.org/abs/2608.25245)
...
### [228] [PlanSightRAG: A Visual-First Multimodal RAG for Automating Question Answering and Compliance Checking for Civil Standard Plans](https://arxiv.org/abs/2608.26091)
...
### [229] [SWIM: Step-Wise Integrated Measure for Session-supervised List Evaluation in Generative Re-ranking](https://arxiv.org/abs/2608.25104)
...
### [230] [Rank-Deviation Quality: A Distance-Aware Metric for Multi-Answer Retrieval and Ranking Evaluation](https://arxiv.org/abs/2608.25318)
...
### [231] [MOTIF: Motivation-guided Topology Inference for Cold-start Multimodal Recommendation](https://arxiv.org/abs/2608.25381)
...
### [232] [DocPC: Document-Level Visual Retrieval via Representative Page Composition](https://arxiv.org/abs/2608.25434)
...
### [233] [Query Expansion Is More Than Generation: Improving Dense Retrieval through Better Integration](https://arxiv.org/abs/2608.25521)
...
### [234] [TransRetrieval: Scaling Up Transformer-Based Retrieval for Industrial Recommendation](https://arxiv.org/abs/2608.25528)
...
### [235] [An Event is Worth One Token: Event Tokenization for Industrial-scale LLM Recommendation](https://arxiv.org/abs/2608.25546)
...
### [236] [RetrievalRouter: Joint Modality and Architecture Selection for Document Retrieval](https://arxiv.org/abs/2608.25625)
...
### [237] [D3ER: Supporting Multi-Modal Recommendation via Disentangle and Distillation-based Dynamic Ensemble](https://arxiv.org/abs/2608.25737)
...
### [238] [Hamiltonian Spectral-Temporal Dissipative Dynamics for Sequential Recommendation](https://arxiv.org/abs/2608.25755)
...
### [239] [PUMA: Post-Hoc Sparsification of Universal Multimodal Embeddings for Efficient Retrieval](https://arxiv.org/abs/2608.25780)
...
### [240] [HCC+: Hyperbolic Guarding for Certified Attention Retrieval](https://arxiv.org/abs/2608.24971)
*Liangchen Ge*
...
### [241] [Fuzzy Pattern Matching in Ordered Structures](https://arxiv.org/abs/2608.25032)
...
### [242] [Improved Low-Overhead Communication-Efficient String Reconciliation and Edit Distance](https://arxiv.org/abs/2608.25179)
...
### [243] [Exact algorithms for optimal discretization](https://arxiv.org/abs/2608.25197)
...
### [244] [Time-Optimal APSP and Matrix Multiplication in Classes of Linear Neighborhood Complexity](https://arxiv.org/abs/2608.25212)
...
### [245] [A General Framework for Metropolis-Adjusted Dikin Walks: Dimension-Square Mixing on Polytopes and Log-Det Walks on Spectrahedra](https://arxiv.org/abs/2608.25273)
...
### [246] [Paging with Per-Replacement Maximum Delay](https://arxiv.org/abs/2608.25290)
...
### [247] [A Spectral Local-to-Global Principle for Spin Systems on Graphs with Girth At Least Five](https://arxiv.org/abs/2608.25491)
...
### [248] [Not All Degree Constraints Are Created Equal when Computing Spanning Trees](https://arxiv.org/abs/2608.25530)
...
### [249] [Partially-Dynamic All-Pairs Maxflow and Effective Resistance via Stable Sparsifiers](https://arxiv.org/abs/2608.26037)
...
### [250] [Nearly Optimal Strong Coresets for $\ell_p$ Subspace Approximation](https://arxiv.org/abs/2608.26047)
...
### [252] [VLM-based automatic multi-granularity graph representation of building layouts for design informatics](https://arxiv.org/abs/2608.24886)
...
### [253] [SIMGUIDE: Procedurally Grounded Multi-Context Representations for Personalized Agent Planning](https://arxiv.org/abs/2608.24888)
*Chirag Shah*

Main category: cs.AI

TL;DR: 本文提出SimGuide方法与SimBench评测基准，核心论点是"表征格式"而非"表征内容"是个性化AI代理规划的一阶设计变量。SimGuide将用户表示为按生活领域划分的类型化"Sim"块，并通过源自过去决策的过程性示例（procedural grounding）将每条约束落地。


<details>
  <summary>Details</summary>
Motivation: 现有个性化方法将用户视为单一实体（扁平profile拼接到prompt中），在跨生活领域、多上下文场景下产生"上下文干扰"与"冲突盲视"两类失效。RAG仅基于表面相似性检索，无法区分不同生活上下文下的决策。现有基准（WebArena、τ-bench测试任务完成，LaMP、PersonalLLM测试单上下文偏好推断）均未评估"正确规划依赖于激活哪个用户上下文"这一关键性质。

Method: 1. **Sim表征**：将用户u表示为S_k=⟨p_k, c_k, σ_k, e_k⟩，包含优先级集、带hard/soft标记的约束集、通信风格以及过程性示例集。2. **过程性接地（Procedural Grounding）**：为每条约束附加1-2条过往决策的具体示例。3. **仲裁形式化**：规划目标转化为约束优化问题——在满足所有hard约束的前提下，按Sim优先级rank的倒数与约束权重的乘积最大化软偏好满足。4. **SimBench基准**：47个偏好条件规划任务，覆盖4个领域（旅行、日历、通讯、研究）与4类冲突类型（优先级、约束、时间、通信），9个典型用户每人含2-5个Sim。5. **评估指标**：Plan Correctness (PC)、Preference Adherence (PA)、Conflict Resolution Accuracy (CRA)。6. **参数化实验**：在Mistral-7B上以4-bit量化的LoRA微调（r=8, α=32, lr=1e-4, 5 epochs）对比四种适配器：领域不匹配（Amazon评论）、任务匹配（合成数据）、每用户、Sim类型路由。

Result: **SimBench结果（47任务）**：GPT-4o上Sim+G（0.625）显著优于RAG（0.546，+7.9 PA, p=0.013）与Sim（0.547, p=0.017）；Sim-A（无仲裁）得分0.496，低于RAG与Sim。**τ-bench外部验证（100任务）**：Sim+G在GPT-4o（+5.4 vs RAG, p=0.008）与Claude Sonnet 4.5（+14.8 vs RAG, p<0.001）上均显著优于RAG。**消融实验**：每约束1个示例即带来+10.5 PA（p=0.001），第2个示例仅再增+1.7（p=0.280）；跨用户示例（0.653）虽高于Sim（0.587），但显著性与正确用户示例（0.692）差距不明确。**LoRA结果**：Amazon LoRA仅+0.4 ROUGE-L；合成LoRA +12.8；每用户LoRA平均+10.4（10个用户中6个为负）；Sim类型路由（72%分类器精度）达+20.1 ROUGE-L，与oracle路由差距仅+0.0025。**GNI（Grounding Necessity Index）**：调度参数型约束均值+0.146高于分类过程型+0.066；GPT-4o正向的约束在Claude上可能为负（行为过拟合现象）。**模型能力依赖**：Llama 8B所有条件下均≤0.243，能力不足；Sim+G在Claude的SimBench上未超RAG，源于天花板效应而非机制失效。

Conclusion: 本文确立"表征格式"是个性化AI系统的一阶设计变量——其重要性超过表征内容或适配器架构。核心结论包括：(1) 声明式Sim不优于RAG，过程性接地使其反超；(2) 在prompt层级，过程性格式承载主要增益，用户特定内容次要；(3) 在参数层级，训练分布决定适配是否可行——领域不匹配时架构无关紧要；(4) 数据规模受限时，Sim类型路由（而非用户身份）是个性化的实用均衡点，对72%路由噪声鲁棒；(5) 表示复杂度需与模型容量匹配，前沿模型适用完整过程性接地，小模型需参数化方法且数据粒度合适。
...
### [254] [Reliable LLM-Powered Decision Engines for Large-Scale Supply Chain Operations: Architecture, Safety, and Performance Guarantees](https://arxiv.org/abs/2608.24889)
*Nirmal Kumar Jingar*
...
### [255] [AI-Powered Mental Health Chatbots in Africa: A Systematic Review and Culturally Adaptive Framework](https://arxiv.org/abs/2608.24890)
...
### [256] [Measurement-Budget Allocation in Quantum Learning with Finite-Shot Generalization Guarantees](https://arxiv.org/abs/2608.24891)
*Ferhat Ozgur Catak*
...
### [257] [Natural Language Input, Semantic Track Representation, and LLM Inference: Making the Maritime Information Exchange Model Tractable](https://arxiv.org/abs/2608.24892)
*Frederick Roth*
...
### [258] [Same-Player Verification for Account Consistency in Counter-Strike 2](https://arxiv.org/abs/2608.24893)
*Xuchen Zhang*
...
### [259] [Semantic Graph Unification for Industrial Digital Threads: Bridging 11 Heterogeneous Manufacturing Systems Through Ontology-Driven Knowledge Graphs](https://arxiv.org/abs/2608.24918)
*Grama Chethan*
...
### [260] [post-graph-rag: A PostgreSQL-Native Graph RAG Engine](https://arxiv.org/abs/2608.24921)
*Chandan Rajah*
...
### [261] [FrontierChallenge: Evaluating Scientific Workflow Completion](https://arxiv.org/abs/2608.24979)
...
### [262] [Solving Robust POMDPs with Omega-regular Objectives via Partially Observable Stochastic Games](https://arxiv.org/abs/2608.24986)
...
### [263] [CVE-SAI: Counterfactual Visual Evidence-Guided Selective Attribute Indexing for Risk-Controlled E-commerce Search](https://arxiv.org/abs/2608.25023)
...
### [264] [LifePlanner: Evaluating LLM Agents for Geo-spatial Planning with Social Media Data](https://arxiv.org/abs/2608.25039)
...
### [265] [SimVerity: When Does Simulated Agent Success Survive Physical Deployment?](https://arxiv.org/abs/2608.25067)
...
### [266] [Auto-Policy, not Auto-Skill: Compiled Agent Skills for the Physical World](https://arxiv.org/abs/2608.25091)
...
### [267] [PhysElite: How Far Are LLMs from Solving Olympiad-Level Physics Problems?](https://arxiv.org/abs/2608.25097)
...
### [268] [Towards Reliable, Generalizable, and Specific In-Context Knowledge Editing via Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2608.25100)
...
### [269] [FuzzingBrain-Bench V1: Evaluating Open-Ended Bug Discovery by LLMs](https://arxiv.org/abs/2608.25158)
...
### [270] [Tunable Tool-Call Rates in LLM Agents via Representation Steering](https://arxiv.org/abs/2608.25198)
...
### [271] [Federation Is Nearly Free, Reasoning Is Not: Tradeoffs for AI Co-Scientists in Protein Characterization Workflows](https://arxiv.org/abs/2608.25215)
...
### [272] [LLM-Driven, Datasheet-Aware Automated Hardware Compatibility Verification for Early-Stage, Pre-Schematic Embedded System Design](https://arxiv.org/abs/2608.25217)
...
### [273] [FLARE: Verifying MILP Reformulations with LLM-Based Theorem Proving](https://arxiv.org/abs/2608.25220)
...
### [274] [Hierarchical MoE for Multi-Modal ILD Diagnosis](https://arxiv.org/abs/2608.25261)
...
### [275] [PhaseShift: Topology-Aware Data Harmonization and Model Consolidation Across Signalized Intersections](https://arxiv.org/abs/2608.25275)
...
### [276] [BixBench3: Benchmarking AI agents on research-study-scale computational biology tasks](https://arxiv.org/abs/2608.25286)
...
### [277] [FinRiskAtlas: Decision-Aligned Evaluation of Large Language Models for Financial Risk Review](https://arxiv.org/abs/2608.25325)
...
### [278] [Learning What to Share and What to Personalize: Hierarchical Strategy Co-Evolution for Agent Memory](https://arxiv.org/abs/2608.25329)
...
### [279] [Where vs What: Decomposing Structural and Content Failures in LLM-Generated Structured Outputs](https://arxiv.org/abs/2608.25358)
...
### [280] [Can your AI agent be cheaper? Investigating the effects of task specifications on token spend in agentic coding tasks](https://arxiv.org/abs/2608.25399)
*Jakub Smékal*
...
### [281] [Paint What You See: Benchmarking Dexterous Visual Tool Use in Multimodal Agents](https://arxiv.org/abs/2608.25417)
...
### [282] [Distance Is Not Enough: Forget-Retain Alignment Gap Predicts LLM Relearning Robustness](https://arxiv.org/abs/2608.25429)
...
### [283] [Training Alignment Auditors via Reinforcement Learning](https://arxiv.org/abs/2608.25460)
...
### [284] [PonsRAG: A Pons-Inspired RAG Bridging Cognitive Islands for Coordinated Long Narrative Reasoning](https://arxiv.org/abs/2608.25486)
...
### [285] [CaSKG: Counterfactual-Causal Skill Graphs for Scalable Agent Skill Retrieval](https://arxiv.org/abs/2608.25500)
...
### [286] [Agentic Game Development as a Verifiable Trajectory Data Engine for Scaling World Models](https://arxiv.org/abs/2608.25518)
...
### [287] [Using profiles of cognitive capability to assess AI suitability for workplace tasks](https://arxiv.org/abs/2608.25623)
...
### [288] [Narcissus: Program Synthesis Using Context-Aware LLM Approximations](https://arxiv.org/abs/2608.25657)
...
### [289] [ToST: A Tree-of-Thought Socratic Teaching Framework for Multi-Path Guidance and Parallel Thinking](https://arxiv.org/abs/2608.25775)
*Feng Ling,Heng Yu*
...
### [290] [LocalLSTC: A Long Short-Term Control Architecture for Locally Deployed GUI Agents](https://arxiv.org/abs/2608.25777)
...
### [291] [Choose Your Game Wisely: Measuring Game-Theoretic Structures in Real-World Vehicle Interactions](https://arxiv.org/abs/2608.25917)
...
### [292] [Repair or Resample? Rethinking Failure Debugging in LLM Multi-Agent Systems](https://arxiv.org/abs/2608.25920)
...
### [293] [Formal, Executable and Explainable Runtime Monitoring of Spoken Air Traffic Control Operational Procedures](https://arxiv.org/abs/2608.25926)
...
### [294] [How Robust Are Automated Fact-Checking Systems? A Cross-Benchmark Evaluation](https://arxiv.org/abs/2608.25934)
...
### [295] [Candidate supply and answer selection shape the value of LLM judging in multi-agent systems](https://arxiv.org/abs/2608.25937)
...
### [296] [LivingRAG: Augmenting Graph RAG with Experience](https://arxiv.org/abs/2608.25960)
...
### [297] [Quantitative Analysis of $ω$-Regular Robust MDPs](https://arxiv.org/abs/2608.25968)
...
### [298] [SciMIF: Understanding Multimodal Instruction Following in Scientific Domains](https://arxiv.org/abs/2608.25973)
...
### [299] [Multi-Granularity Context-Enhanced RAG over Multimodal Knowledge Graphs](https://arxiv.org/abs/2608.25986)
...
### [300] [ProgRouter: Online Progress-Guided Orchestration for Multi-Agent LLM Workflows under Quality-Cost Tradeoffs](https://arxiv.org/abs/2608.25992)
...
### [301] [AsymSpec: Context-Asymmetric Speculative Decoding for Agentic LLMs](https://arxiv.org/abs/2608.26004)
...
### [302] [Imitation Learning for Connection-Tableau Construction](https://arxiv.org/abs/2608.26009)
...
### [303] [Trace Integrity for LLM Data Agents: A Vision for Auditable Structured Reasoning in Real-World Systems](https://arxiv.org/abs/2608.26036)
...
### [304] [SwarmWorld: Stigmergic technological evolution in societies of language-model agents](https://arxiv.org/abs/2608.26081)
...
### [305] [Planetary Prediction Engine: Autonomous Geospatial Prediction via Intelligent Data Selection and Foundation Model Embeddings](https://arxiv.org/abs/2608.26088)

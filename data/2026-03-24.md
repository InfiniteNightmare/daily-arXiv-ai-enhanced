...
### [1] [ProMAS: Proactive Error Forecasting for Multi-Agent Systems Using Markov Transition Dynamics](https://arxiv.org/abs/2603.20260)
*Xinkui Zhao,Sai Liu,Yifan Zhang,Qingyu Ma,Guanjie Cheng,Naibo Wang,Chang Liu*

Main category: cs.AI

TL;DR: 本文提出PROMAS框架，通过马尔可夫转移和因果增量特征实现多智能体系统中逻辑错误的主动预测和实时干预，解决现有方法依赖事后分析导致干预延迟高的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型集成的多智能体系统虽然能协作解决复杂长程任务，但集体智能本质脆弱——单个逻辑谬误会快速传播导致系统级失败。现有研究多依赖事后故障分析，无法实现实时干预。

Method: PROMAS框架包含三个核心组件：1) 因果增量特征(Causal Delta Features)提取语义位移；2) 量化向量马尔可夫空间(Vector Markov Space)将推理建模为概率转移；3) 主动预测头(Proactive Prediction Head)结合跳跃检测(Jump Detection)，通过风险加速度而非静态阈值定位错误。

Result: 在Who&When基准上，PROMAS实现22.97%的步骤级准确率，仅处理27%的推理日志即可达到与反应式监控器MASC相当的性能，数据开销降低73%。

Conclusion: 虽然该方法相比事后分析方法存在精度权衡，但显著改善了干预延迟，在自主推理的实时需求与诊断精度之间取得平衡。
...
### [2] [Domain-Specialized Tree of Thought through Plug-and-Play Predictors](https://arxiv.org/abs/2603.20267)
*Xuanqi Gao,Haoyu Wang,Jun Sun,Shiqing Ma,Chao Shen*

Main category: cs.AI

TL;DR: DST是一个轻量级监督预测器，通过动态上下文感知剪枝解决LLM中ToT框架的探索深度与计算效率之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)在复杂推理方面取得进展，但Tree of Thoughts(ToT)框架面临探索深度与计算效率的关键权衡。现有的ToT实现依赖重量级的LLM自评估或刚性启发式剪枝，使其成本高昂且缺乏灵活性。

Method: 提出DST，一个可适应的即插即用预测器，作为轻量级监督启发式来指导ToT搜索过程。该预测器实现动态上下文感知剪枝，允许搜索在简单推理步骤中以近乎贪心效率进行，仅在遇到不确定性或任务复杂性时自适应扩展搜索宽度。

Result: 在数学推理、一般推理和复杂逻辑推理的多样化基准测试集上评估，结果显示该方法达到与强基线（包括标准ToT）相当或更优的准确率，同时减少26-75%的计算开销。

Conclusion: 有效解决了树状推理中准确率与效率的权衡问题，将ToT从资源密集型技术转变为LLM复杂问题求解的可扩展实用范式。
...
### [3] [FactorSmith: Agentic Simulation Generation via Markov Decision Process Decomposition with Planner-Designer-Critic Refinement](https://arxiv.org/abs/2603.20270)
*Ali Shamsaddinlou,Morteza NourelahiAlamdari*

Main category: cs.AI

TL;DR: FactorSmith是一个框架，通过结合因子化POMDP分解和分层 planner-designer-critic 智能体工作流，从自然语言描述生成可执行的游戏模拟代码。该方法将模拟规范分解为模块化步骤，每个步骤仅处理最小相关的状态变量子集，并通过三智能体交互实现迭代质量 refinement。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理大型、相互关联的代码库时推理能力有限，难以直接从自然语言规范生成可执行的模拟代码。

Method: 1) 基于FactorSim的因子化POMDP表示，将模拟规范分解为模块化步骤，每个步骤仅处理最小相关状态变量子集，限制上下文窗口；2) 嵌入三层智能体交互：planner（编排工作流）、designer（提出代码工件）、critic（通过结构化评分评估质量），实现迭代refinement和检查点回滚。

Result: 在PyGame Learning Environment基准测试上，FactorSmith生成的模拟在提示对齐、运行时错误和代码质量方面均优于非智能体因子化基线。

Conclusion: 结合因子化POMDP分解与层次化智能体工作流可以有效解决LLM生成大型代码库的挑战，为从自然语言生成可执行游戏模拟提供了一种有前景的方法。
...
### [4] [Me, Myself, and $π$ : Evaluating and Explaining LLM Introspection](https://arxiv.org/abs/2603.20276)
*Atharv Naphade,Samarth Bhargav,Sean Lim,Mcnair Shah*

Main category: cs.AI

TL;DR: 本文研究大型语言模型(LLM)的内省能力,提出将内省形式化为模型策略和参数上特定算子的潜在计算,并通过Introspect-Bench评估套件进行严格测试。结果显示前沿模型对自身策略具有特权访问能力,能够预测自身行为,且内省机制可通过注意力扩散自然涌现。


<details>
  <summary>Details</summary>
Motivation: 内省是人类智能的标志,当前对LLM内省能力的评估往往无法区分真正的元认知与一般世界知识或基于文本的自我模拟,需要更严谨的评估框架。

Method: 提出内省的形式化_taxonomy,将内省定义为策略和参数上的潜在计算操作;构建Introspect-Bench多面评估套件进行严格能力测试;提供因果机制证据解释LLM如何学会内省以及内省机制如何通过注意力扩散涌现。

Result: 前沿模型表现出对自身策略的特权访问能力,在预测自身行为方面优于同类模型。

Conclusion: LLM可以在没有显式训练的情况下学会内省,内省机制通过注意力扩散涌现,这为理解模型的元认知能力提供了新的理论框架和实证证据。
...
### [5] [LLM-Enhanced Energy Contrastive Learning for Out-of-Distribution Detection in Text-Attributed Graphs](https://arxiv.org/abs/2603.20293)
*Xiaoxu Ma,Dong Li,Minglai Shao,Xintao Wu,Chen Zhao*

Main category: cs.AI

TL;DR: 针对文本属性图中的分布外（OOD）检测问题，提出了一种结合大语言模型和能量对比学习的新方法LECT，通过生成依赖感知的伪OOD样本和能量基对比学习来区分分布内和分布外节点，在六个数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设训练和测试数据分布一致，当面对分布外（OOD）数据时会出现显著的性能下降。需要解决文本 attributed 图中节点级别的OOD检测问题，同时保持准确的节点分类和识别OOD节点。

Method: 提出了一种名为LECT的新型方法，结合大语言模型（LLMs）和基于能量的对比学习。该方法利用LLM的语义理解和上下文知识生成依赖感知的伪OOD节点作为高质量OOD样本，并应用基于能量函数的对比学习来区分分布内（IND）和分布外（OOD）节点。

Result: LECT方法在六个基准数据集上持续优于最先进的基线方法，实现了高分类准确率和强大的OOD检测能力。

Conclusion: 在六个基准数据集上的广泛实验表明，LECT方法持续优于现有方法，实现了高分类准确率和鲁棒的OOD检测能力的平衡。
...
### [6] [Compression is all you need: Modeling Mathematics](https://arxiv.org/abs/2603.20396)
*Vitaly Aksenov,Eve Bodnia,Michael H. Freedman,Michael Mulligan*

Main category: cs.AI

TL;DR: 论文提出人类数学(HM)是形式数学(FM)中可通过层级嵌套定义、引理和定理进行压缩的子集。通过Monoid模型和MathLib实证分析，发现HM占据FM中多项式增长的空间。


<details>
  <summary>Details</summary>
Motivation: 人类数学为何是形式数学中极小的子集？需要找出区分人类数学的特征——即人类数学具有可压缩性。

Method: 1) 使用Monoid建模：Abelian monoid A_n中稀疏宏集可实现指数级表达力扩展；非Abelian monoid F_n中需近最大密度才能实现超线性扩展。2) 对MathLib进行实证分析：测量深度(定义嵌套层数)、wrapped length(定义中的token数)、unwraped length(完全展开后的原始符号数)。

Result: 1) 理论模型：A_n中稀疏宏达指数扩展；F_n中需密集宏才能超线性扩展。2) 实证结果：unwraped length随深度和wrapped length指数增长；wrapped length在各深度近似常数。3) 一致性：A_n模型与实证结果一致；F_n模型与实证结果不一致。

Conclusion: 人类数学占据指数增长形式数学空间中的多项式增长子集。压缩率和PageRank分析可量化数学兴趣并引导自动推理聚焦于可压缩区域。
...
### [7] [gUFO: A Gentle Foundational Ontology for Semantic Web Knowledge Graphs](https://arxiv.org/abs/2603.20948)
*João Paulo A. Almeida,Giancarlo Guizzardi,Tiago Prince Sales,Claudenir M. Fonseca*

Main category: cs.AI

TL;DR: gUFO是一个轻量级的统一基础本体(UFO)实现，专为Semantic Web OWL 2 DL应用设计。它支持类型分类(OntoClean指南)、内在和关系方面的重新ification模式、情境和高阶类型，提供解决知识图中常见问题的模式。


<details>
  <summary>Details</summary>
Motivation: UFO是一个成熟的基础本体，具有丰富的公理化，已在多个研究和工业项目中应用，目前正在被ISO标准化为ISO/IEC CD 21838-5。现有实现缺乏对类型分类、reification模式、情境和高阶类型的支持。

Method: 提出gUFO实现，包含其构成类别、关系和约束，区别于原始UFO参考本体，并与现有OWL基础本体实现(BFO、DOLCE等)进行系统比较。

Result: gUFO在支持类型分类(OntoClean指南)、内在和关系方面reification模式、情境和高阶类型方面独特，与BFO和DOLCE实现不同。

Conclusion: gUFO为Semantic Web知识图谱提供了良好的模式，解决常见问题，已获得社区采用。
...
TL;DR: ZeroHungerAI是一个结合NLP和ML的集成框架，用于在极端数据稀缺条件下进行基于证据的粮食安全政策建模。该系统使用基于迁移学习的DistilBERT架构，将结构化社会经济指标与上下文政策文本嵌入相结合，在25个区的1200个样本混合数据集上实现了91%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 粮食安全政策制定在数据稀缺地区面临关键挑战，主要由于结构化数据集有限、文本报告碎片化以及决策系统中的 demographics 偏差。该研究旨在解决低资源治理环境下的政策智能问题。

Method: 该系统采用DistilBERT架构进行迁移学习和上下文政策文本嵌入，结合结构化社会经济指标。实验在1200个样本的混合数据集上进行，包含25个区，并采用公平性感知优化来减少 demographic parity 差异。

Result: 模型达到91%分类准确率、0.89精确率、0.85召回率、0.86 F1分数。在不平衡条件下，比SVM提升13%，比逻辑回归提升17%。少数类检测的平均精确率约为0.88，demographic parity差异降至3%。

Conclusion: 基于transformer的上下文学习显著增强了低资源治理环境下的政策智能，能够实现可扩展且具有偏差意识的饥饿预测系统，为数据稀缺地区的粮食安全政策制定提供了有效工具。
...
TL;DR: 本文主张在医疗多智能体系统中， Contestable AI（可质询AI）是比单纯可解释性更必要的可信设计要求，通过人机协作框架集成结构化论证和角色式质询机制，以维护人类自主性、临床责任和高风险护理环境中的信任。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统在医疗决策中面临信任、问责和人类监督的挑战，现有可解释AI方法不足以支持医疗伙伴对系统输出进行挑战或修正，因此需要引入可质询AI来增强人类对决策过程的干预能力。

Method: 提出一个人在环框架，集成结构化论证和角色式质询机制，为高风险护理场景提供透明度、结构化干预机会以及审查、修正或推翻系统输出的机制。

Result: 识别了当前MAS和XAI研究的关键局限性，论证了可质询性作为可信多智能体算法护理系统的必要设计要求。

Conclusion: 可质询性应作为可信多智能体算法护理系统的必要设计要求，需要通过人机协作框架 Preserve 人类自主性、临床责任和信任。
...
TL;DR: DiffGraph是一个新型的基于智能体的图式模型融合框架，用于解决文本到图像(T2I)领域中在线专家模型的高效利用问题。该框架通过构建可扩展图结构对在线专家进行组织和管理，并根据用户需求动态激活特定子图，实现灵活组合不同专家模型以满足多样化生成需求。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像生成社区的快速发展，大量在线专家模型（预训练扩散模型的变体）不断涌现。然而现有模型融合方法无法充分利用这些丰富的在线专家资源，难以满足现实场景中多样化的用户需求。

Method: DiffGraph框架包含两个核心机制：1) 节点注册与校准 - 将不断增长的在线专家模型组织到可扩展的图结构中；2) 动态子图激活 - 根据用户需求动态选择特定子图，实现灵活组合不同专家模型以达到用户期望的生成效果。

Result: 广泛实验验证了该方法的有效性。

Conclusion: DiffGraph能够有效利用在线专家资源，通过图式组织与动态激活机制灵活满足多样化的用户生成需求，为模型融合领域提供了新的解决思路。
...
### [15] [ARYA: A Physics-Constrained Composable & Deterministic World Model Architecture](https://arxiv.org/abs/2603.21340)
...
### [16] [Context Cartography: Toward Structured Governance of Contextual Space in Large Language Model Systems](https://arxiv.org/abs/2603.20578)
...
### [17] [Reasoning Provenance for Autonomous AI Agents: Structured Behavioral Analytics Beyond State Checkpoints and Execution Traces](https://arxiv.org/abs/2603.21692)
*Neelmani Vispute*
...
### [30] [AutoMOOSE: An Agentic AI for Autonomous Phase-Field Simulation](https://arxiv.org/abs/2603.20986)
...
### [31] [Can we automatize scientific discovery in the cognitive sciences?](https://arxiv.org/abs/2603.20988)
...
### [32] [The Intelligent Disobedience Game: Formulating Disobedience in Stackelberg Games and Markov Decision Processes](https://arxiv.org/abs/2603.20994)
...
### [33] [A Framework for Low-Latency, LLM-driven Multimodal Interaction on the Pepper Robot](https://arxiv.org/abs/2603.21013)
...
### [34] [Knowledge Boundary Discovery for Large Language Models](https://arxiv.org/abs/2603.21022)
...
### [35] [KLDrive: Fine-Grained 3D Scene Reasoning for Autonomous Driving based on Knowledge Graph](https://arxiv.org/abs/2603.21029)
...
### [36] [DomAgent: Leveraging Knowledge Graphs and Case-Based Reasoning for Domain-Specific Code Generation](https://arxiv.org/abs/2603.21430)
...
### [45] [RoboAlign: Learning Test-Time Reasoning for Language-Action Alignment in Vision-Language-Action Models](https://arxiv.org/abs/2603.21341)
...
### [46] [The AI Scientific Community: Agentic Virtual Lab Swarms](https://arxiv.org/abs/2603.21344)
*Ulisses Braga-Neto*
...
### [47] [AgentHER: Hindsight Experience Replay for LLM Agent Trajectory Relabeling](https://arxiv.org/abs/2603.21357)
*Liang Ding*
...
### [48] [AdaRubric: Task-Adaptive Rubrics for LLM Agent Evaluation](https://arxiv.org/abs/2603.21362)
*Liang Ding*
...
### [49] [A transformer architecture alteration to incentivise externalised reasoning](https://arxiv.org/abs/2603.21376)
...
### [50] [PivotRL: High Accuracy Agentic Post-Training at Low Compute Cost](https://arxiv.org/abs/2603.21383)
...
### [51] [Persona Vectors in Games: Measuring and Steering Strategies via Activation Vectors](https://arxiv.org/abs/2603.21398)
...
### [52] [The Myhill-Nerode Theorem for Bounded Interaction: Canonical Abstractions via Agent-Bounded Indistinguishability](https://arxiv.org/abs/2603.21399)
*Anthony T. Nixon*
...
### [53] [Silent Commitment Failure in Instruction-Tuned Language Models: Evidence of Governability Divergence Across Architectures](https://arxiv.org/abs/2603.21415)
*Gregory M. Ruddell*
...
### [54] [Is the future of AI green? What can innovation diffusion models say about generative AI's environmental impact?](https://arxiv.org/abs/2603.21419)
...
### [55] [Behavioural feasible set: Value alignment constraints on AI decision support](https://arxiv.org/abs/2603.21435)
*Taejin Park*
...
### [56] [Safety as Computation: Certified Answer Reuse via Capability Closure in Task-Oriented Dialogue](https://arxiv.org/abs/2603.21448)
*Cosimo Spera*
...
### [57] [Beyond Correlation: Refutation-Validated Aspect-Based Sentiment Analysis for Explainable Energy Market Returns](https://arxiv.org/abs/2603.21473)
...
### [58] [Unified-MAS: Universally Generating Domain-Specific Nodes for Empowering Automatic Multi-Agent Systems](https://arxiv.org/abs/2603.21475)
...
### [59] [Counterfactual Credit Policy Optimization for Multi-Agent Collaboration](https://arxiv.org/abs/2603.21563)
...
### [60] [Adaptive Robust Estimator for Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2603.21574)
...
### [61] [Mind over Space: Can Multimodal Large Language Models Mentally Navigate?](https://arxiv.org/abs/2603.21577)
...
### [62] [A Multidisciplinary AI Board for Multimodal Dementia Characterization and Risk Assessment](https://arxiv.org/abs/2603.21597)
...
### [63] [INTRYGUE: Induction-Aware Entropy Gating for Reliable RAG Uncertainty Estimation](https://arxiv.org/abs/2603.21607)
...
### [64] [EnterpriseLab: A Full-Stack Platform for developing and deploying agents in Enterprises](https://arxiv.org/abs/2603.21630)
...
### [65] [Silicon Bureaucracy and AI Test-Oriented Education: Contamination Sensitivity and Score Confidence in LLM Benchmarks](https://arxiv.org/abs/2603.21636)
...
### [66] [Mirage The Illusion of Visual Understanding](https://arxiv.org/abs/2603.21687)
...
### [67] [AI Token Futures Market: Commoditization of Compute and Derivatives Contract Design](https://arxiv.org/abs/2603.21690)
*Yicai Xing*
...
### [68] [Deterministic Hallucination Detection in Medical VQA via Confidence-Evidence Bayesian Gain](https://arxiv.org/abs/2603.21693)
...
### [69] [MIND: Multi-agent inference for negotiation dialogue in travel planning](https://arxiv.org/abs/2603.21696)
...
### [70] [A Blueprint for Self-Evolving Coding Agents in Vehicle Aerodynamic Drag Prediction](https://arxiv.org/abs/2603.21698)
...
### [71] [Compensating Visual Insufficiency with Stratified Language Guidance for Long-Tail Class Incremental Learning](https://arxiv.org/abs/2603.21708)
...
### [72] [CurvZO: Adaptive Curvature-Guided Sparse Zeroth-Order Optimization for Efficient LLM Fine-Tuning](https://arxiv.org/abs/2603.21725)
...
### [73] [EvoIdeator: Evolving Scientific Ideas through Checklist-Grounded Reinforcement Learning](https://arxiv.org/abs/2603.21728)
...
### [74] [The Reasoning Error About Reasoning: Why Different Types of Reasoning Require Different Representational Structures](https://arxiv.org/abs/2603.21736)
*Yiling Wu*
...
### [75] [The Presupposition Problem in Representation Genesis](https://arxiv.org/abs/2603.21745)
*Yiling Wu*
...
### [78] [Tacit Knowledge Management with Generative AI: Proposal of the GenAI SECI Model](https://arxiv.org/abs/2603.21866)
*Naoshi Uchihira*
...
### [79] [Guideline-grounded retrieval-augmented generation for ophthalmic clinical decision support](https://arxiv.org/abs/2603.21925)
...
### [80] [Future-Interactions-Aware Trajectory Prediction via Braid Theory](https://arxiv.org/abs/2603.22035)
...
### [81] [A Context Engineering Framework for Improving Enterprise AI Agents based on Digital-Twin MDP](https://arxiv.org/abs/2603.22083)
...
### [82] [GSEM: Graph-based Self-Evolving Memory for Experience Augmented Clinical Reasoning](https://arxiv.org/abs/2603.22096)
...
### [83] [SpecTM: Spectral Targeted Masking for Trustworthy Foundation Models](https://arxiv.org/abs/2603.22097)
...
### [84] [MARCUS: An agentic, multimodal vision-language model for cardiac diagnosis and management](https://arxiv.org/abs/2603.22179)
...
### [85] [GateANN: I/O-Efficient Filtered Vector Search on SSDs](https://arxiv.org/abs/2603.21466)
*Nakyung Lee,Soobin Cho,Jiwoong Park,Gyuyeong Kim*

Main category: cs.OS

TL;DR: GateANN是一个I/O高效的基于SSD的图ANNS系统，通过引入图隧道(graph tunneling)技术，在未修改的图索引上支持过滤向量搜索，避免了后过滤的I/O浪费和过滤感知索引重建的问题。


<details>
  <summary>Details</summary>
Motivation: 现有SSD-based图ANNS系统存在两个问题：(1)后过滤(post-filtering)方式会浪费大量I/O读取不需要的向量；(2)过滤感知索引需要昂贵的索引重建。两种方法都无法高效支持过滤向量搜索。

Method: GateANN的核心创新是解耦图遍历与向量检索。其关键洞察是：遍历节点只需要邻居列表和近似距离，不需要SSD上的完整精度向量。在此基础上引入图隧道技术：在内存中检查每个节点的过滤谓词，对不匹配的节点直接在内存中路由通过，保留图连通性而无需为不匹配节点读取SSD。

Result: 实验结果表明，GateANN可减少最多10倍的SSD读取，并提高最多7.6倍的吞吐量。

Conclusion: 通过图隧道技术，GateANN成功实现了在未修改的图索引上进行高效的过滤向量搜索，既避免了后过滤的I/O浪费，也无需进行昂贵的过滤感知索引重建，为SSD-based图ANNS系统提供了一种高效的过滤搜索解决方案。
...
### [86] [SkyHOST: A Unified Architecture for Cross-Cloud Hybrid Object and Stream Transfer](https://arxiv.org/abs/2603.20512)
*Muhammad Arslan Tariq,Grégoire Danoy,Pascal Bouvry*

Main category: cs.DC

TL;DR: SkyHOST是一个统一的数据移动架构，构建在Skyplane框架之上，通过单一控制平面和CLI桥接批量对象传输与流式工作负载，支持URI-based路由自动选择传输机制，兼容结构化数据和大型二进制对象。


<details>
  <summary>Details</summary>
Motivation: 传统传输工具仅针对单一范式设计（流复制或批量传输），导致用户需部署和管理多个独立系统，增加了运维复杂性。随着云和大型数据工作负载跨多云提供商和区域分布，需要一种统一的数据移动方案来简化操作。

Method: 提出SkyHOST架构，基于Skyplane框架构建单一控制平面和CLI；通过URI-based路由自动选择适当的传输机制；支持结构化数据的记录级摄取和大型二进制对象的分块传输。

Result: 通过环境监控用例和实证评估表明，SkyHOST能在单一控制平面下整合异构数据移动模式，同时在跨区域传输中实现具有竞争力的吞吐量。

Conclusion: SkyHOST通过统一架构实现了操作简化与性能兼顾，为跨云跨区域的数据移动提供了高效的解决方案。
...
### [87] [Epistemic Observability in Language Models](https://arxiv.org/abs/2603.20531)
*Tony Mason*

Main category: cs.DC

TL;DR: 论文发现大语言模型在编造内容时自我报告的置信度反而最高（与传统认知相反），通过形式化证明纯文本观察下无法区分诚实输出与虚构内容，但通过导出token级熵和概率分布可有效检测fabrication，在四个模型家族上达到AUC 0.757，显著优于纯文本基线。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型的诚实性检测问题：模型在生成错误或虚构内容时反而表现出更高的置信度，这是一个反直觉但严重的问题，现有方法无法可靠区分诚实输出与可信的fabrication。

Method: 1) 形式化建模证明在纯文本观察下任何策略都无法满足认知诚实性；2) 构建tensor interface导出per-token entropy和log-probability分布作为检测信号；3) 在OLMo-3、Llama-3.1、Qwen3、Mistral四个模型家族上验证。

Result: 自报告置信度与准确率呈负相关（AUC 0.28-0.36，越低越反直觉）；per-token entropy达到pooled AUC 0.757，比纯文本基线提升2.5-3.9个百分点；跨架构泛化Spearman ρ=0.762；提供了验证预算到检测精度的成本曲面映射。

Conclusion: 在text-only observation下无法实现可靠的认知诚实检测，需通过tensor interface导出计算副产品（entropy）来突破这一 impossibility，且该方法与模型规模或训练过程无关。
...
### [88] [Incremental GNN Embedding Computation on Streaming Graphs](https://arxiv.org/abs/2603.20622)
*Qiange Wang,Haoran Lv,Yanfeng Zhang,Weng-Fai Wong,Bingsheng He*

Main category: cs.DC

TL;DR: 针对流图上GNN推理中RTEC的高计算开销问题，提出增量计算框架，通过解耦GNN为细粒度算子并重排序，将全邻居计算转化为受影响子图上的高效形式，同时设计GPU-CPU协同处理系统扩展到大规模场景。实验表明可减少64%-99%计算量，实现1.7x-145.8x加速。


<details>
  <summary>Details</summary>
Motivation: GNN在流图上的应用日益普及，但实际部署面临挑战。推理过程依赖Runtime Embedding Computation (RTEC)来捕获最近的图变化，该过程产生繁重的多跳图遍历开销，严重影响计算效率。观察到图演化过程中大部分图的中间结果保持不变，因此可以通过精心设计的增量方法有效消除冗余计算。

Method: 核心方法包括：(1)将GNN计算解耦为 generalized fine-grained operators 并安全重排序，将全邻居GNN计算转化为受影响子图上的高效形式；(2)设计GPU-CPU协同处理系统，将嵌入卸载到CPU内存并通过通信优化调度扩展到大规模历史结果场景。该框架保留原始全邻居计算的语义和精度，同时支持具有复杂消息传递模式的各种GNN模型。

Result: 实验在多种图规模和GNN模型上验证了方法有效性。结果显示：该方法减少计算量64%-99%，相比现有解决方案实现1.7x-145.8x的加速比。

Conclusion: 提出了一种高效增量框架来加速流图上的GNN推理，通过解耦GNN计算为细粒度算子并重排序，将昂贵的全邻居计算转化为受影响子图上的高效形式。同时设计了GPU-CPU协同处理系统来处理大规模历史嵌入。实验表明该方法可减少64%-99%的计算量，实现1.7x-145.8x的加速比，有效解决了流图上GNN推理的效率瓶颈问题。
...
### [89] [Modernizing Amdahl's Law: How AI Scaling Laws Shape Computer Architecture](https://arxiv.org/abs/2603.20654)
*Chien-Ping Lu*

Main category: cs.DC

TL;DR: 本文重新审视了经典Amdahl定律，将其扩展到现代异构系统和可扩展工作负载。核心发现是存在一个"有限崩溃阈值"：当可扩展部分超过临界值时，专用加速变得次优，专用投资降为零，形成一种相变而非传统认为的渐进尾部效应。该框架解释了GPU可编程性增强的原因，以及领域特定AI加速器未能取代GPU的现象。


<details>
  <summary>Details</summary>
Motivation: 经典Amdahl定律假设串行与并行工作之间有固定分解和同构复制，但在现代系统中已不适用。现代系统结合了专用加速器、可编程计算、张量数据通路和演化流水线，工作负载结构也在演变。作者试图解决核心张力：问题不仅在于串行与并行的分割，更在于考虑到效率差异和 workload 结构的异构硬件资源分配问题。

Method: 1. 建立考虑异构硬件（专用加速器vs可编程计算）的资源分配数学模型
2. 引入可扩展工作负载概念
3. 推导最优专用化投资与效率优势的关系
4. 识别"有限崩溃阈值"这一关键相变点

Result: 1. 发现存在有限崩溃阈值：当可扩展部分超过临界值时，专用化变得次优
2. 无论专用硬件相对于可编程计算有何效率优势，最优专用投资都会降为零
3. 这是一种"相变"现象，而非经典Amdahl定律预测的渐进尾部
4. 该框架可解释GPU可编程性增强和DSA未取代GPU的产业现象

Conclusion: Amdahl定律需要针对现代异构系统重新表述。核心洞见是：可扩展工作负载的存在导致专用硬件投资存在上限，超过临界点后专用化反而成为次优选择。这一"相变"视角解释了为什么即使专用加速器在特定任务上更高效，整个系统仍倾向于保持通用可编程组件（如GPU）的核心地位。
...
### [90] [Compass: Optimizing Compound AI Workflows for Dynamic Adaptation](https://arxiv.org/abs/2603.20821)
*Milos Gravara,Juan Luis Herrera,Stefan Nastic*

Main category: cs.DC

TL;DR: Compass是一个用于复合AI系统动态配置切换的框架，通过离线配置发现（COMPASS-V算法）和在线自适应（Planner和Elastico Controller）实现精度-延迟权衡优化，在固定基础设施上实现90-98%的SLO合规性。


<details>
  <summary>Details</summary>
Motivation: 复合AI系统是将专门的AI/ML模型与工程软件组件编排成AI工作流的统一系统。现有部署在固定基础设施上运行，无法水平扩展，且现有优化方法仅考虑精度而忽视工作负载变化。需要在满足精度、延迟和成本目标的同时，根据当前负载动态调整配置。

Method: Compass框架包含三个核心组件：1) COMPASS-V算法：使用有限差分引导搜索结合爬山法和横向扩展来发现精度可行的配置；2) Planner：使用排队论模型对配置进行性能分析并推导出切换策略；3) Elastico Controller：监控队列深度并根据阈值进行配置切换。

Result: 1) COMPASS-V在两个工作流上实现100%召回率，配置评估相比穷举搜索平均减少57.5%，在严格精度阈值下效率提升达95.3%；2) 运行时自适应在动态负载模式下达到90-98%的SLO合规性，比静态高精度基线提升71.6%，同时比静态快速基线提升准确率3-5%。

Conclusion: Compass框架通过离线优化和在线自适应相结合的方式，成功实现了复合AI系统的动态配置切换。该方案能够在固定基础设施条件下，通过在准确率和延迟之间进行权衡来满足不同工作负载的需求。COMPASS-V算法在配置发现阶段实现了高效的帕累托最优解搜索，Planner基于排队论模型推导出切换策略，Elastico控制器负责运行时自适应。实验结果表明，该方法在两个复合AI工作流上均取得了显著效果：配置评估减少57.5%（tight accuracy thresholds时达95.3%），SLO合规性提升71.6%，同时准确率提升3-5%。
...
### [91] [Communication Lower Bounds and Algorithms for Sketching with Random Dense Matrices](https://arxiv.org/abs/2603.20966)
*Hussam Al Daas,Grey Ballard,Laura Grigori,Md Taufique Hussain,Suraj Kumar,Mohammad Marufur Rahman,Kathryn Rouse*

Main category: cs.DC

TL;DR: 本文研究分布式内存并行计算中矩阵草图（sketching）算法的通信下界及其最优算法设计。研究针对密集矩阵建立了通信复杂度下界，证明了少量处理器无需通信，并给出了达到该下界的最优算法及在CPU-GPU异构超算平台上的高效实现。


<details>
  <summary>Details</summary>
Motivation: 在大规模矩阵的随机化线性代数计算中，草图技术被广泛用于低秩矩阵近似、列子集选择等问题。然而，分布式内存环境下的通信开销成为现代超级计算机的瓶颈，但该领域尚未有系统性的并行策略研究。

Method: 1. 建立密集矩阵草图的通信下界理论<br>2. 设计通信最优的并行算法<br>3. 将方法推广到Nyström近似（双重草图）<br>4. 在CPU-GPU异构超算平台实现并验证可扩展性

Result: 1. 建立了通信下界，揭示少量处理器无需通信<br>2. 证明了算法下界是最优的<br>3. Nytrom近似通信下界及接近下界的并行算法<br>4. 在现代超算平台展示了良好的并行可扩展性

Conclusion: 该工作为分布式内存环境下的草图计算提供了理论基础和实践指导，通信最优算法填补了该领域的空白，为大规模随机线性代数提供了高效的并行策略。
...
### [92] [NeSy-Edge: Neuro-Symbolic Trustworthy Self-Healing in the Computing Continuum](https://arxiv.org/abs/2603.21145)
*Peihan Ye,Alfreds Lapkovskis,Alaa Saleh,Qiyang Zhang,Praveen Kumar Donta*

Main category: cs.DC

TL;DR: 本文提出NeSy-Edge，一个面向计算连续体的神经符号化可信自愈框架，采用边缘优先设计，在资源受限的边缘节点进行本地感知和推理，仅在最终诊断阶段调用云端模型，实现根因分析和恢复推荐。


<details>
  <summary>Details</summary>
Motivation: 现代AI服务的计算需求正从集中式云端向边缘和端侧设备迁移，形成计算连续体。然而，这些环境的规模、异构性和跨层依赖性使得维护韧性面临挑战。现有故障管理方法过于静态、碎片化或笨重，无法支持及时的自愈，尤其在噪声日志和边缘资源约束条件下。

Method: NeSy-Edge框架包括三个核心步骤：1)将原始运行时日志转换为结构化事件表示；2)构建先验约束的稀疏符号因果图；3)将因果证据与历史故障排除知识整合，用于根因分析和恢复推荐。框架采用边缘优先架构，边缘节点负责本地感知和推理，云端模型仅在最终诊断阶段被调用。

Result: 在Loghub数据集上针对多个语义噪声水平进行评估，结果显示：1)解析质量、因果推理、端到端诊断和边缘资源使用均得到评估；2)在最高噪声水平下仍保持鲁棒性；3)达到75%的根因分析准确率和65%的端到端准确率；4)本地内存消耗约为1500MB。

Conclusion: NeSy-Edge成功证明了神经符号化方法在边缘计算自愈场景中的有效性，在资源受限和噪声环境下仍能实现可信的根因诊断，为计算连续体环境下的韧性维护提供了新的技术路径。
...
### [93] [Decidability of Livelock Detection for Parameterized Self-Disabling Unidirectional Rings](https://arxiv.org/abs/2603.21443)
*Aly Farahat*

Main category: cs.DC

TL;DR: 论文证明了在参数化对称单向环中，自禁用进程的livelock检测在多项式时间内是可判定的。算法通过计算 deflationary monotone 算子的最大不动点来决定是否存在livelock，时间复杂度为O(|T|³)，与环大小K无关。


<details>
  <summary>Details</summary>
Motivation: 解决参数化并发系统中的livelock检测问题。之前的代数 characterization (Farahat 2012) 建立了livelock存在的必要充分条件，但没有解决可判定性问题。

Method: 算法计算定义在有限集合T上的 deflationary monotone 算子的最大不动点。如果不动点非空则返回livelock，否则证明对所有K都是livelock-free。

Result: livelock检测在O(|T|³)时间内可判定，与环大小K无关。算法能检测某些环大小K≥2是否存在livelock，同时也能处理(1,1)-非对称情况。

Conclusion: 通过不动点方法实现了livelock检测的多项式时间可判定性，无需对环大小进行搜索即可证明所有K的livelock自由性。
...
### [94] [Communication-Avoiding SpGEMM via Trident Partitioning on Hierarchical GPU Interconnects](https://arxiv.org/abs/2603.21444)
*Julian Bellavita,Lorenzo Pichetti,Thomas Pasquali,Flavio Vella,Giulia Guidi*

Main category: cs.DC

TL;DR: Trident是一种层次感知的2D分布式稀疏矩阵乘法(SpGEMM)算法，通过创新的trident划分方案利用节点内GPU间高带宽特性，减少节点间通信，在非结构化矩阵上实现最高2.38倍加速(几何平均1.54倍)，节点间通信量减少达2倍，并在Markov聚类中实现2倍加速。


<details>
  <summary>Details</summary>
Motivation: 当前SpGEMM实现忽视节点内和节点间带宽的差异，导致不必要的数据移动和同步，未能充分利用快速的节点内互连。此外，非结构化稀疏性限制了内存效率和可扩展性，分布式环境中跨节点交换和合并部分乘积的成本进一步制约性能。

Method: 提出trident划分方案，实现层次感知的分解，通过通信避免技术和异步通信利用现代超级计算架构的层次异构性。核心创新在于利用GPU在节点内相比跨节点更高的带宽来减少节点间通信。

Result: 在非结构化矩阵上实现最高2.38倍加速，几何平均加速1.54倍；在NERSC Perlmutter超级计算机上节点间通信量减少最高2倍；应用于Markov聚类加速达2倍。

Conclusion: Trident有效利用了现代超级计算互连的层次异构架构，通过层次感知的方法显著提升了分布式SpGEMM性能。
...
### [95] [Benchmarking Message Brokers for IoT Edge Computing: A Comprehensive Performance Study](https://arxiv.org/abs/2603.21600)
...
### [96] [exaCB: Reproducible Continuous Benchmark Collections at Scale Leveraging an Incremental Approach](https://arxiv.org/abs/2603.22251)
...
### [97] [HCAG: Hierarchical Abstraction and Retrieval-Augmented Generation on Theoretical Repositories with LLMs](https://arxiv.org/abs/2603.20299)
*Yusen Wu,Xiaotie Deng*

Main category: cs.SE

TL;DR: HCAG是一个用于复杂理论驱动代码库（如算法博弈论）的检索增强生成框架，通过分层知识结构实现架构引导的代码生成。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法无法捕捉复杂理论驱动代码库中的高级架构模式和跨文件依赖，导致抽象概念与可执行实现之间存在语义和结构鸿沟。

Method: 采用两阶段设计：(1)离线分层抽象阶段：递归解析代码仓库和对齐的理论文本，构建多分辨率语义知识库；(2)在线分层检索和脚手架生成阶段：自顶向下逐层检索，引导LLM采用"先架构后模块"的生成范式；并集成多智能体讨论机制。

Result: HCAG在代码质量、架构一致性和需求通过率方面显著优于仓库级基线方法；产生的理论与实现对齐数据集可有效增强领域特定LLM。

Conclusion: HCAG为复杂系统的挖掘、重用和生成提供了通用蓝图，不仅适用于算法博弈论，还可推广到其他领域。
...
### [98] [From Human Interfaces to Agent Interfaces: Rethinking Software Design in the Age of AI-Native Systems](https://arxiv.org/abs/2603.20300)
*Shaolin Wang,Yi Mei,Haoyang Che,He Jiang,Shui Yu,Ying Gu*

Main category: cs.SE

TL;DR: 本文提出软件工程正在经历从面向人类界面到面向AI智能体调用系统的范式转变。传统软件设计强调人机交互和图形用户界面，而大语言模型驱动的智能体正成为软件的主要使用者。文章形式化了"智能体接口"概念，引入"可调用能力"作为AI导向软件的基本构建块，并提出机器可解释性、可组合性和调用可靠性等设计原则。该研究为AI原生软件设计提供了概念基础。


<details>
  <summary>Details</summary>
Motivation: 传统软件系统传统上为人类交互设计，强调图形用户界面和可用性。然而，大语言模型(LLM)驱动的智能体正在改变软件的主要消费者群体——软件不再仅供人类使用，还被AI智能体通过结构化接口自主调用。这一趋势催生了重新思考软件设计范式的迫切需求。

Method: 本文采用概念分析和方法论研究方式：(1)形式化定义"智能体接口"概念；(2)引入"可调用能力"作为AI导向软件的基本构建块；(3)提出面向智能体系统的设计原则；(4)讨论该转变带来的架构和组织影响。

Result: 研究结果表明：面向AI智能体的软件设计需要遵循三大设计原则——机器可解释性(computational interpretability)、可组合性(composability)和调用可靠性(invocation reliability)。架构层面，软件正从单体应用向可被AI智能体动态组合的能力型系统转变。

Conclusion: 软件工程正在经历从人类导向界面到智能体导向调用系统的范式转变。AI原生软件设计需要建立新的概念基础，包括智能体接口、可调用能力等核心概念，以及相应的架构和组织原则。这一转变为软件工程理论和方法论带来了新的挑战和机遇。
...
### [99] [Semantic Tool Discovery for Large Language Models: A Vector-Based Approach to MCP Tool Selection](https://arxiv.org/abs/2603.20313)
*Sarat Mudunuri,Jian Wan,Ally Qin,Srinivasan Manoharan*

Main category: cs.SE

TL;DR: 针对MCP工具过多导致的token开销大、精度下降和上下文窗口限制问题，提出了一种基于向量检索的语义工具发现架构，通过密集嵌入索引MCP工具，动态选择3-5个最相关工具，替代直接暴露50-100+个工具的方案。


<details>
  <summary>Details</summary>
Motivation: 当前LLM调用工具时面临可扩展性挑战：提供所有可用工具会产生大量token开销、增加成本、降低精度，并受制于上下文窗口限制。MCP作为标准化框架，单个服务器可能暴露数十至数百个工具，传统方案难以应对。

Method: 提出语义工具发现架构，使用密集嵌入（dense embeddings）对MCP工具进行语义索引，通过查询-工具相似度进行动态工具选择，从50-100+工具中动态选取3-5个最相关工具。

Result: 在5个MCP服务器共121个工具的140条查询基准测试中，实现了：工具相关token消耗减少99.6%，K=3命中率达97.1%，MRR为0.91，检索延迟低于100ms。

Conclusion: 该架构通过语义索引和动态选择显著提升了工具调用效率，可扩展至多智能体和跨组织工具发现场景，为LLM与大规模工具集的集成提供了高效可行的解决方案。
...
### [100] [Procedural Refinement by LLM-driven Algorithmic Debugging for ARC-AGI-2](https://arxiv.org/abs/2603.20334)
*Yu-Ning Qiu,Lin-Feng Zou,Jiong-Da Wang,Xue-Rong Yuan,Wang-Zhou Dai*

Main category: cs.SE

TL;DR: 本文提出了一种神经符号程序调试方法ABPR，将大语言模型与元解释器结合，通过算法程序调试(APD)理论实现程序修复。该方法在ARC-AGI-2基准测试上使用Prolog语言，Gemin-3-Flash达到56.67%的Pass@2得分。


<details>
  <summary>Details</summary>
Motivation: 当前对话式LLM代码修复主要依赖"似是而非的推理"，缺乏形式化的调试程序。Udi Shapiro的算法程序调试(APD)理论提供了形式化基础，但尚未与LLM结合。

Method: 提出ABPR方法，将LLM与元解释器耦合，将程序执行物化为紧凑的声明式树结构trace，遵循APD原则。使用Prolog作为目标语言。

Result: 在ARC-AGI-2基准测试上，ABPR配合Gemini-3-Flash达到56.67%的Pass@2得分，即使在LLM通常表现不佳的语言中也能取得此成绩。

Conclusion: 通过将LLM与经典形式方法相结合，可以实现更可审计的程序修复范式，为程序调试提供了新的神经符号结合思路。
...
### [101] [Beyond LLM-based test automation: A Zero-Cost Self-Healing Approach Using DOM Accessibility Tree Extraction](https://arxiv.org/abs/2603.20358)
*Renjith Nelson Joseph*

Main category: cs.SE

TL;DR: 本文提出一种零成本的Web测试自动化自愈框架，通过结构化可访问性树提取算法替代LLM，使用十级优先级定位器层次结构实现元素发现，并在失败时仅重新提取损坏的选择器而非重新运行完整发现流程。


<details>
  <summary>Details</summary>
Motivation: 现代Web测试自动化框架严重依赖CSS选择器、XPath表达式和可见文本标签来定位UI元素，这些定位器本质上脆弱——当Web应用更新DOM结构或类名时，测试套件会大规模失败。现有自愈方法越来越依赖LLM进行元素发现，但企业级规模下每次运行的API成本变得令人望而却步。

Method: 该框架采用十级优先级排序的定位器层次结构——get_by_role(W3C标准)、data-testid、ARIA标签、CSS类片段、可见文本——通过一次遍历从实时DOM中发现健壮的选择器。自愈机制仅在失败时重新提取损坏的选择器，而非重新运行完整发现流程。框架在automationexercise.com上针对三个设备配置文件（桌面Chrome、桌面Safari、iPhone 15）和十个业务流程测试工作流进行验证，采用三层层次结构（L0领域、L1流程、L2功能）。

Result: 验证结果显示：在31个测试组合中达到31/31（100%）的通过率，并行执行总耗时22秒。自愈机制经验证： stale选择器在不到1秒内被检测到并重新发现，无需人工干预。该框架可扩展至300多个测试用例，且无持续API成本。

Conclusion: 该零成本自愈框架成功消除了LLM依赖和相关的API开销，同时保持了高可靠性。通过优先级定位器层次结构和有针对性的自愈机制，实现了100%的测试通过率和快速失败恢复，为企业级测试自动化提供了一种经济高效的解决方案。
...
### [102] [The Nature of Technical Debt in Research Software](https://arxiv.org/abs/2603.20415)
*Neil A. Ernst,Ahmed Musa Awon,Swapnil Hingmire,Ze Shi Li*

Main category: cs.SE

TL;DR: 研究软件（也称为科学软件）对于推动科学发展至关重要，但面临技术债务的普遍挑战。本研究通过多方法研究检查研究软件中的技术债务，包括检查28个代码注释和9个研究软件项目中的自报告技术债务实例，并对研究软件工程师和科学家进行访谈。识别出9种研究软件特有的自认技术债务类型和4个影响该技术债务的主题。


<details>
  <summary>Details</summary>
Motivation: 研究软件的技术债务程度和性质缺乏研究，特别是其具体形式以及开发团队对技术债务的看法。技术债务会影响可靠性、可维护性和科学有效性，而研究软件通常依赖科学界的主动性进行维护，需要科学和软件工程领域的多元专业知识。

Method: 多方法研究：1）检查28k代码注释，识别9个研究软件项目中的自报告技术债务实例；2）基于发现结果，对研究软件工程师和科学家进行访谈，了解技术债务如何在其经验中体现，以及对研究软件和研究产出的影响。

Result: 识别出9种研究软件特有的自认技术债务类型，以及4个影响技术债务的主题。

Conclusion: 研究软件存在独特的技术债务形式，需要关注并采取相应的管理和缓解措施。
...
### [103] [yProv4DV: Reproducible Data Visualization Scripts Out of the Box](https://arxiv.org/abs/2603.20437)
*Gabriele Padovani,Sandro Fiore*

Main category: cs.SE

TL;DR: yProv4DV是一个轻量级Python库，通过 provenance 信息追踪实现数据可视化脚本的可重复性，只需单次调用即可追踪输入、输出和源代码文件，最小化代码修改需求。


<details>
  <summary>Details</summary>
Motivation: 当前学术论文中的图表常被分享但缺乏完整的代码、输入数据、执行环境和输出结果，导致无法独立复现。现有的可重复性解决方案侧重于计算管道或工作流管理系统，未涵盖研究者和实践者常用的基于脚本的可视化实践。此外，Python数据可视化库的极简主义特性加速了图像创建，降低了用户集成额外工具的动力。

Method: 提出yProv4DV库，通过provenance信息追踪实现可重复的数据可视化脚本设计。用户通过单次函数调用即可追踪输入、输出和源代码文件，实现数据可视化软件的保存和完整可重复性。

Result: 该库实现了最小化代码修改需求下的可视化脚本可重复性，支持保存输入、输出和源代码文件。

Conclusion: yProv4DV填补了可重复研究工作流中的空白，通过解决科学出版物中图表的可重复性问题，为可视化脚本提供了轻量级的可重复性解决方案。
...
### [104] [Software Entropy: A Statistical Mechanics Framework for Software Testing](https://arxiv.org/abs/2603.20528)
...
### [105] [An Industrial-Scale Retrieval-Augmented Generation Framework for Requirements Engineering: Empirical Evaluation with Automotive Manufacturing Data](https://arxiv.org/abs/2603.20534)
...
### [106] [LogFold: Compressing Logs with Structured Tokens and Hybrid Encoding](https://arxiv.org/abs/2603.20618)
...
### [107] [Evaluating LLM-generated code for domain-specific languages: molecular dynamics with LAMMPS](https://arxiv.org/abs/2603.20630)
...
### [108] [AEGIS: From Clues to Verdicts -- Graph-Guided Deep Vulnerability Reasoning via Dialectics and Meta-Auditing](https://arxiv.org/abs/2603.20637)
...
### [109] [REVERE: Reflective Evolving Research Engineer for Scientific Workflows](https://arxiv.org/abs/2603.20667)
...
### [110] [SWE-Next: Scalable Real-World Software Engineering Tasks for Agents](https://arxiv.org/abs/2603.20691)
...
### [111] [Engineering Pitfalls in AI Coding Tools: An Empirical Study of Bugs in Claude Code, Codex, and Gemini CLI](https://arxiv.org/abs/2603.20847)
...
### [112] [The Role of Road Features and Vehicle Dynamics in Cost-Effective Autonomous Vehicles Safety Testing: Insights from Instance Space Analysis](https://arxiv.org/abs/2603.21066)
...
### [113] [LLM-based Automated Architecture View Generation: Where Are We Now?](https://arxiv.org/abs/2603.21178)
...
### [114] [Which Alert Removals are Beneficial?](https://arxiv.org/abs/2603.21322)
*Idan Amit*
...
### [115] [Awakening: Modern Challenges and Opportunities of Software Engineering Research](https://arxiv.org/abs/2603.21403)
...
### [116] [Dynasto: Validity-Aware Dynamic-Static Parameter Optimization for Autonomous Driving Testing](https://arxiv.org/abs/2603.21427)
...
### [117] [LLM-Powered Workflow Optimization for Multidisciplinary Software Development: An Automotive Industry Case Study](https://arxiv.org/abs/2603.21439)
...
### [118] [RuntimeSlicer: Towards Generalizable Unified Runtime State Representation for Failure Management](https://arxiv.org/abs/2603.21495)
...
### [119] [Efficient Failure Management for Multi-Agent Systems with Reasoning Trace Representation](https://arxiv.org/abs/2603.21522)
...
### [120] [LLM-Based Test Case Generation in DBMS through Monte Carlo Tree Search](https://arxiv.org/abs/2603.21530)
...
### [121] [One-Year Internship Program on Software Engineering: Students' Perceptions and Educators' Lessons Learned](https://arxiv.org/abs/2603.21548)
...
### [122] [AI In Cybersecurity Education -- Scalable Agentic CTF Design Principles and Educational Outcomes](https://arxiv.org/abs/2603.21551)
...
### [123] [A Curated List of Open-source Software-only Energy Efficiency Measurement Tools: A GitHub Mining Study](https://arxiv.org/abs/2603.21772)
...
### [124] [StreamSampling.jl: Efficient Sampling from Data Streams in Julia](https://arxiv.org/abs/2603.21996)
*Adriano Meligrana*
...
### [125] [Dynamic analysis enhances issue resolution](https://arxiv.org/abs/2603.22048)
...
### [126] [From Technical Debt to Cognitive and Intent Debt: Rethinking Software Health in the Age of AI](https://arxiv.org/abs/2603.22106)
...
### [127] [Lemma Discovery in Agentic Program Verification](https://arxiv.org/abs/2603.22114)
...
TL;DR: JointFM是首个用于联合时间序列分布预测的基础模型，通过从无限合成SDE中学习，在零样本设置下预测联合概率分布，相比最强基线降低能量损失14.2%。


<details>
  <summary>Details</summary>
Motivation: 解决SDE在实际应用中的三大挑战：建模风险高、校准过程脆弱、高保真模拟计算成本昂贵。传统方法需要将SDE拟合到数据，文章试图逆转这一范式。

Method: JointFM采用反向范式：通过采样无限量的合成SDE来训练通用模型，使其能够直接预测未来联合概率分布，而非传统地将SDE拟合到数据。该模型在完全零样本设置下运行，无需任何任务特定的校准或微调。

Result: 在恢复由 unseen synthetic SDE 生成的 oracle 联合分布时，JointFM 相比最强基线将能量损失降低了14.2%，成为首个可进行耦合时间序列分布预测的基础模型。

Conclusion: JointFM成功建立了首个无需任务特定校准或微调的联合时间序列分布预测基础模型，在零样本设置下取得了显著的性能提升，为基于SDE建模的传统范式提供了替代方案。
...
TL;DR: MARLIN是一种基于多智能体强化学习的增量有向无环图(DAG)学习方法，用于从观测数据中高效发现因果关系。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的因果结构学习方法缺乏效率，难以应用于在线场景。现有方法通常不是为增量学习设计的，无法有效处理动态数据流。

Method: 1. DAG生成策略：将连续实值空间映射到DAG空间
2. 双RL智能体架构：状态特定智能体和状态不变智能体
3. 增量学习框架：支持在线/动态数据处理
4. 分解动作空间：增强并行化效率
5. 合成数据集和真实数据集验证

Result: 在合成数据集和真实数据集上的广泛实验表明，MARLIN在效率和有效性方面均优于现有最先进方法。

Conclusion: MARLIN成功地将多智能体强化学习与增量学习相结合，为因果结构发现提供了一种高效且有效的解决方案，特别适合在线应用场景。
...
### [130] [Transformer-Based Predictive Maintenance for Risk-Aware Instrument Calibration](https://arxiv.org/abs/2603.20297)
...
TL;DR: 本文将校准调度问题重新表述为预测性维护问题，利用传感器历史数据预测漂移时间(TTD)，并比较了经典回归器、循环/卷积序列模型和Transformer在TTD预测中的表现。研究表明，基于条件的校准可以建模为联合预测和决策问题，结合序列模型和风险感知策略可实现更智能的校准规划。


<details>
  <summary>Details</summary>
Motivation: 固定间隔校准方案管理简单，但忽视了仪器在不同条件下以不同速率漂移的事实，导致校准不足或过度校准。需要一种基于仪器实际状态的条件性校准方法。

Method: 1) 将NASA C-MAPSS基准适配到校准场景：选择漂移敏感传感器、定义虚拟校准阈值、插入模拟重校准的合成重置事件；2) 比较经典回归器、循环卷积序列模型和紧凑Transformer在TTD预测中的性能；3) 构建基于分位数的不确定性模型支持保守调度；4) 设计违反感知的成本模型评估调度策略。

Result: Transformer在FD001数据集上提供最强点预测，在较难的FD002-FD004数据集上保持竞争力；分位数不确定性模型支持噪声较大时的保守调度；预测性调度相比反应式和固定策略降低成本；不确定性感知触发器在点预测不可靠时显著减少违规事件。

Conclusion: 基于条件的校准可表述为联合预测和决策问题；将序列模型与风险感知策略相结合是实现更智能校准规划的实用路径。
...
### [131] [Rolling-Origin Validation Reverses Model Rankings in Multi-Step PM10 Forecasting: XGBoost, SARIMA, and Persistence](https://arxiv.org/abs/2603.20315)
*Federico Garcia Crespi,Eduardo Yubero Funes,Marina Alfosea Simon*

Main category: cs.LG

TL;DR: 研究比较了静态划分与滚动原点协议在PM10空气质量预测中的评估差异。静态评估显示XGBoost表现良好，但滚动原点评估显示XGBoost在短期和中期预测范围内并不持续优于持续性模型，而SARIMA在所有预测范围内保持正向技能。研究揭示了静态评估可能高估机器学习方法的操作实用性。


<details>
  <summary>Details</summary>
Motivation: 许多空气质量预测研究报告机器学习的收益，但评估通常使用静态时间划分且缺乏持续性基准，导致常规更新下的实际运营价值不明确。

Method: 使用2017-2024年南欧城市背景监测站的2350个每日PM10观测值，比较XGBoost和SARIMA与持续性基准在静态划分和滚动原点协议（每月更新）下的表现，报告特定预测 horizon 的技能和可预测性 horizon（定义为持续性相对技能为正的最大预测 horizon）。

Result: 静态评估表明XGBoost在一到七天前表现良好，但滚动原点评估反转了排名：XGBoost在短期和中期预测范围不持续优于持续性模型，而SARIMA在整个预测范围内保持正向技能。

Conclusion: 对于研究者，静态划分可能高估操作实用性并改变排名；对于实践者，滚动原点、持续性参考的技能谱显示每种方法在各个预测提前期的可靠性。
...
### [132] [Bounded Coupled AI Learning Dynamics in Tri-Hierarchical Drone Swarms](https://arxiv.org/abs/2603.20333)
*Oleksii Bychkov*

Main category: cs.LG

TL;DR: 本文研究一种三层层级结构的蜂群学习系统，同时包含三种异构学习机制：局部Hebbian在线学习（快时间尺度，10-100ms）、多智能体强化学习MARL（中等时间尺度，1-10s）和元学习MAML（慢时间尺度，10-100s）。论文建立了四个定理来证明系统总误差、表示漂移、层级兼容性和误差非累积的有界性，为异构学习机制的耦合动力学提供了形式化保证。


<details>
  <summary>Details</summary>
Motivation: 现代自主多智能体系统结合了在不同时间尺度上运行的异构学习机制，一个开放问题是如何形式化地保证这些机制的耦合动态保持在可接受的运行范围内。

Method: 通过在学习率上施加契约约束、层级间映射的Lipschitz连续性假设，以及权重稳定化条件，建立数学分析框架，证明了四个核心定理。

Result: 1) 有界总误差定理：在给定条件下，总次优性具有时间一致的分量上界；2) 有界表示漂移定理：给出了Hebbian更新对协调层级嵌入影响的最坏情况估计；3) 元层级兼容定理：提供了战略适应保持下层不变性的充分条件；4) 非累积定理：证明误差不会随时间无限增长。

Conclusion: 该研究为异构学习机制的多智能体系统提供了形式化稳定性保证，证明了在适当约束下三层层级学习系统的误差有界且不会累积，回应了自主系统耦合动力学的可保证性问题。
...
### [133] [Hybrid Autoencoder-Isolation Forest approach for time series anomaly detection in C70XP cyclotron operation data at ARRONAX](https://arxiv.org/abs/2603.20335)
*F Basbous,F Poirier,F Haddad,D Mateus*

Main category: cs.LG

TL;DR: 本研究提出了一种基于自编码器(AE)和隔离森林(IF)混合方法的早期异常检测系统，用于C70XP回旋加速器的故障预警。通过利用AE重建数据的均方误差(MCE)作为IF的输入特征，有效解决了传统IF方法在检测靠近正常数据均值的细微异常方面的局限性，并在质子束强度时间序列数据上验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: C70XP回旋加速器系统复杂且成本高昂,其故障会导致运行中断。传统的基于轴平行分割的隔离森林(IF)方法在检测靠近正常数据均值的细微异常时存在局限,需要开发更有效的早期异常检测方法以提升系统性能。

Method: 提出了一种混合异常检测方法,将全连接自编码器(AE)与隔离森林(IF)相结合。利用AE重建传感器数据的均方误差(MCE)作为IF模型的输入特征,增强对细微异常的检测能力。

Result: 在质子束强度时间序列数据上的实验结果表明,与传统IF方法相比,该混合方法在异常检测性能上有明显提升。

Conclusion: AE-IF混合方法能够有效提升回旋加速器系统的早期异常检测能力,为放射性同位素生产过程的稳定运行提供了技术保障。该方法可为其他工业系统的异常检测提供参考。
...
### [134] [Interpretable Multiple Myeloma Prognosis with Observational Medical Outcomes Partnership Data](https://arxiv.org/abs/2603.20341)
*Salma Rachidi,Aso Bozorgpanah,Eric Fey,Alexander Jung*

Main category: cs.LG

TL;DR: 本文提出两种新的正则化技术来确保ML模型的可解释性，应用于多发性骨髓瘤患者的五年生存率预测。使用812例患者数据验证，测试集准确率达0.721，SHAP值表明模型依赖于选择的重要特征。


<details>
  <summary>Details</summary>
Motivation: ML在医疗领域应用前景广阔，但模型的不透明性限制了其在临床决策中的采纳。需要确保ML模型的可解释性以促进临床应用。

Method: 提出两种替代正则化惩罚项构造方法：1）惩罚与基于两个手动选择特征的可解释逻辑回归预测的偏差；2）要求模型预测与修订版国际分期系统(R-ISS)保持一致。使用SHAP值评估特征重要性。

Result: 在812例患者数据上验证，测试集准确率达到0.721，SHAP值显示模型确实依赖所选的重要特征进行预测。

Conclusion: 所提出的正则化技术能够有效确保ML模型的可解释性，在多发性骨髓瘤患者五年生存率预测任务中表现良好，具有临床应用潜力。
...
### [135] [SymCircuit: Bayesian Structure Inference for Tractable Probabilistic Circuits via Entropy-Regularized Reinforcement Learning](https://arxiv.org/abs/2603.20392)
*Y. Sungtaek Ju*
...
### [136] [KV Cache Optimization Strategies for Scalable and Efficient LLM Inference](https://arxiv.org/abs/2603.20397)
...
### [137] [Putnam 2025 Problems in Rocq using Opus 4.6 and Rocq-MCP](https://arxiv.org/abs/2603.20405)
...
### [138] [The Workload-Router-Pool Architecture for LLM Inference Optimization: A Vision Paper from the vLLM Semantic Router Project](https://arxiv.org/abs/2603.21354)
...
### [139] [SLE-FNO: Single-Layer Extensions for Task-Agnostic Continual Learning in Fourier Neural Operators](https://arxiv.org/abs/2603.20410)
...
### [140] [Data-driven discovery of roughness descriptors for surface characterization and intimate contact modeling of unidirectional composite tapes](https://arxiv.org/abs/2603.20418)
...
### [141] [Detecting Neurovascular Instability from Multimodal Physiological Signals Using Wearable-Compatible Edge AI: A Responsible Computational Framework](https://arxiv.org/abs/2603.20442)
...
### [142] [SDE-Driven Spatio-Temporal Hypergraph Neural Networks for Irregular Longitudinal fMRI Connectome Modeling in Alzheimer's Disease](https://arxiv.org/abs/2603.20452)
...
### [143] [Reinforcement Learning from Multi-Source Imperfect Preferences: Best-of-Both-Regimes Regret](https://arxiv.org/abs/2603.20453)
...
### [144] [From Data to Laws: Neural Discovery of Conservation Laws Without False Positives](https://arxiv.org/abs/2603.20474)
*Rahul D Ray*
...
### [145] [Spatio-Temporal Grid Intelligence: A Hybrid Graph Neural Network and LSTM Framework for Robust Electricity Theft Detection](https://arxiv.org/abs/2603.20488)
...
### [146] [AE-LLM: Adaptive Efficiency Optimization for Large Language Models](https://arxiv.org/abs/2603.20492)
...
### [147] [Distributed Gradient Clustering: Convergence and the Effect of Initialization](https://arxiv.org/abs/2603.20507)
...
### [148] [Delightful Distributed Policy Gradient](https://arxiv.org/abs/2603.20521)
*Ian Osband*
...
### [149] [Does This Gradient Spark Joy?](https://arxiv.org/abs/2603.20526)
*Ian Osband*
...
### [150] [RMNP: Row-Momentum Normalized Preconditioning for Scalable Matrix-Based Optimization](https://arxiv.org/abs/2603.20527)
...
### [151] [Towards Practical Multimodal Hospital Outbreak Detection](https://arxiv.org/abs/2603.20536)
...
### [152] [Understanding Behavior Cloning with Action Quantization](https://arxiv.org/abs/2603.20538)
...
### [153] [LJ-Bench: Ontology-Based Benchmark for U.S. Crime](https://arxiv.org/abs/2603.20572)
...
### [154] [RECLAIM: Cyclic Causal Discovery Amid Measurement Noise](https://arxiv.org/abs/2603.20585)
...
### [155] [MKA: Memory-Keyed Attention for Efficient Long-Context Reasoning](https://arxiv.org/abs/2603.20586)
...
### [156] [Neural collapse in the orthoplex regime](https://arxiv.org/abs/2603.20587)
...
### [157] [Generating from Discrete Distributions Using Diffusions: Insights from Random Constraint Satisfaction Problems](https://arxiv.org/abs/2603.20589)
...
### [158] [Bayesian Learning in Episodic Zero-Sum Games](https://arxiv.org/abs/2603.20604)
...
### [159] [Beyond Token Eviction: Mixed-Dimension Budget Allocation for Efficient KV Cache Compression](https://arxiv.org/abs/2603.20616)
...
### [160] [Optimal low-rank stochastic gradient estimation for LLM training](https://arxiv.org/abs/2603.20632)
...
### [161] [CFNN: Continued Fraction Neural Network](https://arxiv.org/abs/2603.20634)
...
### [162] [Diffusion Model for Manifold Data: Score Decomposition, Curvature, and Statistical Complexity](https://arxiv.org/abs/2603.20645)
...
### [163] [Do Papers Match Code? A Benchmark and Framework for Paper-Code Consistency Detection in Bioinformatics Software](https://arxiv.org/abs/2603.22018)
...
### [166] [Centrality-Based Pruning for Efficient Echo State Networks](https://arxiv.org/abs/2603.20684)
*Sudip Laudari*
...
### [167] [Neuronal Self-Adaptation Enhances Capacity and Robustness of Representation in Spiking Neural Networks](https://arxiv.org/abs/2603.20687)
...
### [168] [Adversarial Attacks on Locally Private Graph Neural Networks](https://arxiv.org/abs/2603.20746)
...
### [169] [Evaluating Uplift Modeling under Structural Biases: Insights into Metric Stability and Model Robustness](https://arxiv.org/abs/2603.20775)
...
### [170] [OmniPatch: A Universal Adversarial Patch for ViT-CNN Cross-Architecture Transfer in Semantic Segmentation](https://arxiv.org/abs/2603.20777)
...
### [171] [Neural Autoregressive Flows for Markov Boundary Learning](https://arxiv.org/abs/2603.20791)
...
### [172] [Large Neighborhood Search meets Iterative Neural Constraint Heuristics](https://arxiv.org/abs/2603.20801)
...
### [173] [Achieving $\widetilde{O}(1/ε)$ Sample Complexity for Bilinear Systems Identification under Bounded Noises](https://arxiv.org/abs/2603.20819)
...
### [174] [Simple Projection-Free Algorithm for Contextual Recommendation with Logarithmic Regret and Robustness](https://arxiv.org/abs/2603.20826)
*Shinsaku Sakaue*
...
### [175] [Beyond the Academic Monoculture: A Unified Framework and Industrial Perspective for Attributed Graph Clustering](https://arxiv.org/abs/2603.20829)
...
### [176] [A Knowledge-Informed Pretrained Model for Causal Discovery](https://arxiv.org/abs/2603.20842)
...
### [177] [Semantic Sections: An Atlas-Native Feature Ontology for Obstructed Representation Spaces](https://arxiv.org/abs/2603.20867)
*Hossein Javidnia*
...
### [178] [Incentive-Aware Federated Averaging with Performance Guarantees under Strategic Participation](https://arxiv.org/abs/2603.20873)
...
### [179] [Beyond the Birkhoff Polytope: Spectral-Sphere-Constrained Hyper-Connections](https://arxiv.org/abs/2603.20896)
...
### [180] [Natural Gradient Descent for Online Continual Learning](https://arxiv.org/abs/2603.20898)
...
### [181] [Bayesian Scattering: A Principled Baseline for Uncertainty on Image Data](https://arxiv.org/abs/2603.20908)
...
### [182] [LLM-ODE: Data-driven Discovery of Dynamical Systems with Large Language Models](https://arxiv.org/abs/2603.20910)
...
### [183] [Enhancing LIME using Neural Decision Trees](https://arxiv.org/abs/2603.20919)
...
### [184] [Discriminative Representation Learning for Clinical Prediction](https://arxiv.org/abs/2603.20921)
...
### [185] [Causally-Guided Diffusion for Stable Feature Selection](https://arxiv.org/abs/2603.20930)
...
### [214] [Pretrained Video Models as Differentiable Physics Simulators for Urban Wind Flows](https://arxiv.org/abs/2603.21210)
...
### [215] [Does Mechanistic Interpretability Transfer Across Data Modalities? A Cross-Domain Causal Circuit Analysis of Variational Autoencoders](https://arxiv.org/abs/2603.21236)
...
### [216] [Amortized Variational Inference for Logistic Regression with Missing Covariates](https://arxiv.org/abs/2603.21244)
...
### [217] [Aggregation Alignment for Federated Learning with Mixture-of-Experts under Data Heterogeneity](https://arxiv.org/abs/2603.21276)
...
### [218] [Sonny: Breaking the Compute Wall in Medium-Range Weather Forecasting](https://arxiv.org/abs/2603.21284)
*Minjong Cheon*
...
### [219] [Direct Interval Propagation Methods using Neural-Network Surrogates for Uncertainty Quantification in Physical Systems Surrogate Model](https://arxiv.org/abs/2603.21308)
...
### [220] [FluidWorld: Reaction-Diffusion Dynamics as a Predictive Substrate for World Models](https://arxiv.org/abs/2603.21315)
*Fabien Polly*
...
### [221] [Stream separation improves Bregman conditioning in transformers](https://arxiv.org/abs/2603.21317)
*James Clayton Kerce*
...
### [222] [Active Inference Agency Formalization, Metrics, and Convergence Assessments](https://arxiv.org/abs/2603.21319)
*Eduard Kapelko*
...
### [223] [AutoKernel: Autonomous GPU Kernel Optimization via Iterative Agent-Driven Search](https://arxiv.org/abs/2603.21331)
...
### [224] [TIDE: Token-Informed Depth Execution for Per-Token Early Exit in LLM Inference](https://arxiv.org/abs/2603.21365)
*Jaber Jaber,Osama Jaber*
...
### [225] [PLR: Plackett-Luce for Reordering In-Context Learning Examples](https://arxiv.org/abs/2603.21373)
...
### [226] [Constrained Online Convex Optimization with Memory and Predictions](https://arxiv.org/abs/2603.21375)
...
### [227] [A Generalised Exponentiated Gradient Approach to Enhance Fairness in Binary and Multi-class Classification Tasks](https://arxiv.org/abs/2603.21393)
...
### [228] [Mechanisms of Introspective Awareness](https://arxiv.org/abs/2603.21396)
...
### [229] [DSPA: Dynamic SAE Steering for Data-Efficient Preference Alignment](https://arxiv.org/abs/2603.21461)
...
### [230] [Off-Policy Evaluation for Ranking Policies under Deterministic Logging Policies](https://arxiv.org/abs/2603.21485)
...
### [231] [Learning Can Converge Stably to the Wrong Belief under Latent Reliability](https://arxiv.org/abs/2603.21491)
...
### [232] [Multinoulli Extension: A Lossless Continuous Relaxation for Partition-Constrained Subset Selection](https://arxiv.org/abs/2603.21492)
...
### [233] [Quotient Geometry, Effective Curvature, and Implicit Bias in Simple Shallow Neural Networks](https://arxiv.org/abs/2603.21502)
...
### [234] [Optimizing Feature Extraction for On-device Model Inference with User Behavior Sequences](https://arxiv.org/abs/2603.21508)
...
### [235] [BOxCrete: A Bayesian Optimization Open-Source AI Model for Concrete Strength Forecasting and Mix Optimization](https://arxiv.org/abs/2603.21525)
...
### [236] [Generalization Limits of In-Context Operator Networks for Higher-Order Partial Differential Equations](https://arxiv.org/abs/2603.21534)
...
### [237] [Sharper Generalization Bounds for Transformer](https://arxiv.org/abs/2603.21541)
...
### [238] [What Do World Models Learn in RL? Probing Latent Representations in Learned Environment Simulators](https://arxiv.org/abs/2603.21546)
*Xinyu Zhang*
...
### [239] [Kolmogorov Complexity Bounds for LLM Steganography and a Perplexity-Based Detection Proxy](https://arxiv.org/abs/2603.21567)
*Andrii Shportko*
...
### [240] [SSAM: Singular Subspace Alignment for Merging Multimodal Large Language Models](https://arxiv.org/abs/2603.21584)
...
### [241] [In-network Attack Detection with Federated Deep Learning in IoT Networks: Real Implementation and Analysis](https://arxiv.org/abs/2603.21596)
...
### [242] [Riemannian Geometry Speaks Louder Than Words: From Graph Foundation Model to Next-Generation Graph Intelligence](https://arxiv.org/abs/2603.21601)
*Philip S. Yu,Li Sun*
...
### [243] [mSFT: Addressing Dataset Mixtures Overfiting Heterogeneously in Multi-task SFT](https://arxiv.org/abs/2603.21606)
...
### [244] [Rule-State Inference (RSI): A Bayesian Framework for Compliance Monitoring in Rule-Governed Domains](https://arxiv.org/abs/2603.21610)
*Abdou-Raouf Atarmla*
...
### [245] [Towards Multimodal Time Series Anomaly Detection with Semantic Alignment and Condensed Interaction](https://arxiv.org/abs/2603.21612)
...
### [246] [Proximal Policy Optimization in Path Space: A Schrödinger Bridge Perspective](https://arxiv.org/abs/2603.21621)
...
### [247] [MISApp: Multi-Hop Intent-Aware Session Graph Learning for Next App Prediction](https://arxiv.org/abs/2603.21653)
...
### [248] [TrustFed: Enabling Trustworthy Medical AI under Data Privacy Constraints](https://arxiv.org/abs/2603.21656)
...
### [249] [Thinking Deeper, Not Longer: Depth-Recurrent Transformers for Compositional Generalization](https://arxiv.org/abs/2603.21676)
*Hung-Hsuan Chen*
...
### [250] [Data-Free Layer-Adaptive Merging via Fisher Information for Long-to-Short Reasoning LLMs](https://arxiv.org/abs/2603.21705)
*Tian Xia*
...
### [251] [When Exploration Comes for Free with Mixture-Greedy: Do we need UCB in Diversity-Aware Multi-Armed Bandits?](https://arxiv.org/abs/2603.21716)
...
### [252] [Uncertainty Quantification for Distribution-to-Distribution Flow Matching in Scientific Imaging](https://arxiv.org/abs/2603.21717)
...
### [253] [FISformer: Replacing Self-Attention with a Fuzzy Inference System in Transformer Models for Time Series Forecasting](https://arxiv.org/abs/2603.21724)
...
### [254] [CellFluxRL: Biologically-Constrained Virtual Cell Modeling via Reinforcement Learning](https://arxiv.org/abs/2603.21743)
...
### [255] [Extending Precipitation Nowcasting Horizons via Spectral Fusion of Radar Observations and Foundation Model Priors](https://arxiv.org/abs/2603.21768)
...
### [256] [Show Me What You Don't Know: Efficient Sampling from Invariant Sets for Model Validation](https://arxiv.org/abs/2603.21782)
...
### [257] [CoRA: Boosting Time Series Foundation Models for Multivariate Forecasting through Correlation-aware Adapter](https://arxiv.org/abs/2603.21828)
...
### [258] [Deriving Health Metrics from the Photoplethysmogram: Benchmarks and Insights from MIMIC-III-Ext-PPG](https://arxiv.org/abs/2603.21832)
...
### [259] [On the Number of Conditional Independence Tests in Constraint-based Causal Discovery](https://arxiv.org/abs/2603.21844)
...
### [260] [Holistic Scaling Laws for Optimal Mixture-of-Experts Architecture Optimization](https://arxiv.org/abs/2603.21862)
...
### [261] [P^2O: Joint Policy and Prompt Optimization](https://arxiv.org/abs/2603.21877)
...
### [262] [SmaAT-QMix-UNet: A Parameter-Efficient Vector-Quantized UNet for Precipitation Nowcasting](https://arxiv.org/abs/2603.21879)
...
### [263] [SparseDVFS: Sparse-Aware DVFS for Energy-Efficient Edge Inference](https://arxiv.org/abs/2603.21908)
...
### [264] [Deep Reinforcement Learning and The Tale of Two Temporal Difference Errors](https://arxiv.org/abs/2603.21921)
...
### [265] [Demystifying Reinforcement Learning for Long-Horizon Tool-Using Agents: A Comprehensive Recipe](https://arxiv.org/abs/2603.21972)
...
### [266] [BOOST-RPF: Boosted Sequential Trees for Radial Power Flow](https://arxiv.org/abs/2603.21977)
...
### [267] [TREX: Trajectory Explanations for Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2603.21988)
...
### [268] [λ-GELU: Learning Gating Hardness for Controlled ReLU-ization in Deep Networks](https://arxiv.org/abs/2603.21991)
...
### [269] [CRPS-Optimal Binning for Conformal Regression](https://arxiv.org/abs/2603.22000)
*Paolo Toccaceli*
...
### [270] [ROM: Real-time Overthinking Mitigation via Streaming Detection and Intervention](https://arxiv.org/abs/2603.22016)
...
### [271] [AdditiveLLM2: A Multi-modal Large Language Model for Additive Manufacturing](https://arxiv.org/abs/2603.22017)
...
### [272] [On the Interplay of Priors and Overparametrization in Bayesian Neural Network Posteriors](https://arxiv.org/abs/2603.22030)
...
### [273] [On the Failure of Topic-Matched Contrast Baselines in Multi-Directional Refusal Abliteration](https://arxiv.org/abs/2603.22061)
*Valentin Petrov*
...
### [274] [MIHT: A Hoeffding Tree for Time Series Classification using Multiple Instance Learning](https://arxiv.org/abs/2603.22074)
...
### [275] [On the Direction of RLVR Updates for LLM Reasoning: Identification and Exploitation](https://arxiv.org/abs/2603.22117)
...
### [276] [Computationally lightweight classifiers with frequentist bounds on predictions](https://arxiv.org/abs/2603.22128)
...
### [277] [RAMPAGE: RAndomized Mid-Point for debiAsed Gradient Extrapolation](https://arxiv.org/abs/2603.22155)
*Abolfazl Hashemi*
...
### [278] [Multimodal Survival Analysis with Locally Deployable Large Language Models](https://arxiv.org/abs/2603.22158)
...
### [279] [Causal Evidence that Language Models use Confidence to Drive Behavior](https://arxiv.org/abs/2603.22161)
...
### [280] [Calibeating Made Simple](https://arxiv.org/abs/2603.22167)
...
### [281] [Revisiting Quantum Code Generation: Where Should Domain Knowledge Live?](https://arxiv.org/abs/2603.22184)
...
### [282] [Chimera: Latency- and Performance-Aware Multi-agent Serving for Heterogeneous LLMs](https://arxiv.org/abs/2603.22206)
...
### [283] [SPA: A Simple but Tough-to-Beat Baseline for Knowledge Injection](https://arxiv.org/abs/2603.22213)
...
### [284] [Noise Titration: Exact Distributional Benchmarking for Probabilistic Time Series Forecasting](https://arxiv.org/abs/2603.22219)
*Qilin Wang*
...
### [285] [Confidence-Based Decoding is Provably Efficient for Diffusion Language Models](https://arxiv.org/abs/2603.22248)
...
### [286] [Decoupling Exploration and Policy Optimization: Uncertainty Guided Tree Search for Hard Exploration](https://arxiv.org/abs/2603.22273)
...
### [287] [Scaling DoRA: High-Rank Adaptation via Factored Norms and Fused Kernels](https://arxiv.org/abs/2603.22276)
...
### [288] [Global Dataset of Solar Power Plants: Multidimensional Integration and Analysis](https://arxiv.org/abs/2603.20601)
...
TL;DR: 提出了一种可靠、标准化、多维度的全球太阳能光伏数据集，解决了现有数据集缺乏整合、存在显著差距和地理覆盖有限的问题。该数据集包含27个属性，涵盖地理、地形、气候和电力等关键信息，共58,978条记录，可供学术和专业社区使用。


<details>
  <summary>Details</summary>
Motivation: 现有太阳能数据集存在缺乏整合、存在显著差距和地理覆盖有限的问题，无法支持光伏发电厂的快速发展和能源优化利用，因此需要建立一个更全面、更详细的太阳能数据资源。

Method: 通过可重现的方法论和自动化流程，收集、生成并整合了27个属性（包括地理、地形、物流、气候和电力性质），并对58,978条记录进行描述性统计分析。

Result: 成功编译了包含58,978条记录的全球光伏发电厂多维度数据集，涵盖27个关键属性，将原始数据转化为有价值的信息。

Conclusion: 该数据集可作为知识发现的来源，公开供学术和专业社区使用，为全球光伏发电厂研究提供重要的数据支持。
...
### [289] [From (Elementary) Mathematical Data Model Schemas to Safe Blazor Web Applications with Claude AI](https://arxiv.org/abs/2603.21388)
*Christian Mancas,Diana Christina Mancas*

Main category: cs.DB

TL;DR: 本文研究如何利用Claude AI (Sonnet 4.5)从基础数学数据模型模式开发安全的MS Blazor Web应用程序，并总结软件工程最佳实践规则及MS Blazor Server平台的相关问题。


<details>
  <summary>Details</summary>
Motivation: 当前Web应用开发面临安全挑战，MS Blazor作为新兴的Web框架需要结合AI辅助工具来提升开发质量和安全性，同时需要系统性地整理软件工程最佳实践。

Method: 基于( Elementary) Mathematical Data Model数学数据模型模式，利用Claude AI (Sonnet 4.5)版本进行辅助开发，遵循软件工程最佳实践规则，构建安全的MS Blazor Web应用。

Result: 形成了一套从数学数据模型到安全MS Blazor Web应用的开发方法论，整理了软件工程最佳实践规则清单，并识别了MS Blazor Server平台的相关问题。

Conclusion: 通过AI辅助结合数学模型驱动的方法可以有效提升MS Blazor应用的安全性，但需注意MS Blazor Server平台的特定限制和最佳实践的应用。
...
### [290] [FGIM: a Fast Graph-based Indexes Merging Framework for Approximate Nearest Neighbor Search](https://arxiv.org/abs/2603.21710)
*Zekai Wu,Jiabao Jin,Peng Cheng,Xiaoyao Zhong,Lei Chen,Yongxin Tong,Zhitao Shen,Jingkuan Song,Heng Tao Shen,Xuemin Lin*

Main category: cs.DB

TL;DR: 本文提出了一种快速图索引合并框架(FGIM),用于将多个现有的基于图的近似最近邻搜索(ANNS)索引合并为一个统一的索引,以解决分布式系统中的集群整合和实时向量数据库中的读写争用问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中在设计高效的基于图的ANNS方法,而对合并多个现有图索引的关注较少。这一问题在分布式系统集群整合和实时向量数据库读写争用等实际场景中至关重要,需要将多个图索引合并为单一索引以提升效率。

Method: 提出FGIM框架包含三个核心技术:(1)PGs到k-NNG转换:通过跨查询从输入图索引中提取潜在候选邻居;(2)k-NNG refinement:识别被忽略的高质量邻居并维护图连通性;(3)k-NNG到PG转换:提高图导航性并增强搜索性能。将FGIM与HNSW及其他主流图方法集成进行验证。

Result: 在六个真实数据集上的广泛实验表明,FGIM框架适用于各种主流图方法,相比HNSW增量构建实现最高3.5倍加速,相比不支持增量构建的方法平均实现7.9倍加速,同时保持相当或更优的搜索性能。

Conclusion: FGIM框架成功实现了多个图索引的高效合并,在保持搜索性能的同时显著提升了构建效率,具有较强的通用性和实用价值,为分布式ANNS系统中的索引管理提供了有效解决方案。
...
### [291] [Approximate Butterfly Counting in Sublinear Time](https://arxiv.org/abs/2603.21816)
*Chi Luo,Jiaxin Song,Yuhao Zhang,Kai Wang,Zhixing He,Kuan Yang*

Main category: cs.DB

TL;DR: 研究在查询模型下（允许度查询、邻居查询和顶点对查询）估计二分图中蝴蝶（即2x2二分子图）的数量。提出TLS算法，通过两级采样、小度优先采样、小子集楔形采样、轻重划分和猜测验证等关键技术，仅访问有限的图结构即可实现(1+ε)近似精度，在稠密图上达到亚线性时间。实验表明TLS在15个数据集上比现有方法低达三个数量级的查询成本和运行时间。


<details>
  <summary>Details</summary>
Motivation: 蝴蝶计数是二分图分析的基础问题。现有算法需要访问整个图的大部分结构，在实际场景中当图极其庞大且I/O成本昂贵时面临挑战。

Method: 提出TLS算法——一种实用的两级采样算法，在标准查询模型下（允许度查询、邻居查询、顶点对查询）估计蝴蝶数量。关键 techniques包括：小度优先采样、小子集楔形采样、重轻划分和猜测验证。

Result: 算法可对任意近似参数0<ε<1实现(1+ε)精度保证，承诺在稠密图上达到亚线性时间复杂度。15个数据集实验表明，TLS比现有解决方案低达三个数量级的查询成本和运行时间，且估计结果稳健。

Conclusion: TLS是一种实用的两级采样算法，通过新颖的方差控制技术（重轻划分、猜测验证）集成，能够在仅访问有限图结构的情况下准确估计蝴蝶数量，在查询成本和运行时间上显著优于现有方法。
...
### [292] [Accelerating Fresh Data Exploration with Fluid ETL Pipelines](https://arxiv.org/abs/2603.22220)
*Maxwell Norfolk,Dong Xie*

Main category: cs.DB

TL;DR: 本文提出了一种名为"流式ETL流水线"(fluid ETL pipelines)的新型数据摄取系统，旨在解决实时数据探索场景中传统ETL管道资源消耗过大的问题。该系统允许用户在不影响数据摄取的前提下，按需启动/停止任意数据预处理例程(DPR)，并能够利用空闲或低成本资源（如Amazon Spot实例）来加速未来探索查询。


<details>
  <summary>Details</summary>
Motivation: 实时数据探索需求日益增长，数据分析师需要探索新收集数据的主要特征或检测异常。传统ETL流水线虽然能实现低查询延迟，但必须执行大量数据预处理例程（如解析和索引）以应对不可预测的数据特征和分析目标，导致资源消耗极大。此外，传统方法难以跟上快速数据积累的吞吐量需求。

Method: 提出流式ETL流水线框架，允许用户：1)在不阻塞数据摄取的情况下按需启动/停止任意DPR；2)在空闲/廉价资源可用时启动可能有用的DPR以加速未来探索查询；3)根据用户不断变化的需求动态调整运行哪些DPR。该方法利用偶尔的空闲系统容量或廉价抢占式资源（如Amazon Spot实例）来处理数据预处理任务。

Result: 在真实数据集上进行实验，验证了愿景的可行性。实验表明流式ETL流水线能够有效支持实时数据探索，同时降低资源成本。

Conclusion: 流式ETL流水线为实时数据探索提供了一种新的解决思路，但也带来了新的挑战，包括：临时查询处理、DPR生成和DPR管理等任务。论文详细讨论了这些开放性研究挑战，并提出了潜在的解决方向。
...
### [293] [Reason-to-Transmit: Deliberative Adaptive Communication for Cooperative Perception](https://arxiv.org/abs/2603.20308)
*Aayam Bansal,Ishaan Gangwani*

Main category: cs.MA

TL;DR: R2T是一个基于轻量级Transformer的推理框架，用于V2X协同感知中的通信决策。该框架通过推理局部场景上下文、估计邻居信息差距和带宽预算来做出每区域传输决策，在高遮挡场景下表现接近神性能，并能优雅地处理高达50%的丢包率。


<details>
  <summary>Details</summary>
Motivation: Cooperative perception among autonomous agents overcomes single-agent sensing limitations, but V2X networks face bandwidth constraints. Existing approaches use reactive mechanisms (confidence maps, learned gating, sparse masks) to decide what to transmit, without reasoning about why a message benefits the receiver.

Method: We introduce Reason-to-Transmit (R2T), a framework that equips each agent with a lightweight transformer-based module that reasons over local scene context, estimated neighbor information gaps, and bandwidth budget to make per-region transmission decisions. Trained end-to-end with a bandwidth-aware objective.

Result: Evaluated against nine baselines in a multi-agent bird's-eye-view perception environment: (1) Any communication improves performance by about 58% AP over no communication; (2) At low bandwidth, all selective methods perform similarly; (3) R2T shows clear gains under high occlusion, where information asymmetry is greatest, approaching oracle performance; (4) All methods degrade gracefully under packet drops up to 50%, showing robustness to communication failures.

Conclusion: While fusion design dominates performance, deliberative communication provides additional gains in challenging scenarios. R2T introduces a reasoning-based approach to communication, enabling more efficient and context-aware information sharing in cooperative perception.
...
### [294] [When Agents Disagree: The Selection Bottleneck in Multi-Agent LLM Pipelines](https://arxiv.org/abs/2603.20324)
*Artem Maryanskyy*

Main category: cs.MA

TL;DR: 本文解决了多智能体LLM管道中团队多样性是否改善输出的矛盾证据问题，通过识别"选择瓶颈"——聚合质量中的交叉阈值来统一这些发现。实验表明，多样化团队在判断式选择下胜率为0.810，而同质团队仅为0.512（接近随机水平）。选择器质量可能比生成器多样性更具影响力。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM管道产生矛盾证据：异构Mixture-of-Agents团队优于单一模型，但同构Self-MoA团队在基于合成的聚合下持续获胜。需要解决这一矛盾并确定何时多样性有助于或损害输出质量。

Method: 跨42个任务（7个类别，N=210）的定向实验，比较判断式选择与基于合成的MoA方法，使用独立法官进行解耦评估，包含探索性证据测试更弱模型的影响。

Result: 多样化团队胜率0.810 vs 单一模型基线；同质团队0.512（Glass's Δ=2.07，接近随机）。判断式选择优于MoA式合成Δ_WR=+0.631；合成方法在42个任务中零个被法官偏好。独立法官确认所有方向性发现（Spearman ρ=0.90）。包含更弱模型改善性能并降低成本（p<10^-4）。

Conclusion: 选择器质量可能是单轮生成-选择管道中比生成器多样性更具影响力的设计杠杆。异构团队在判断式选择下表现最佳，但同质团队在合成聚合下表现接近随机，验证了交叉阈值模型s*的有效性。
...
### [295] [Measuring Reasoning Trace Legibility: Can Those Who Understand Teach?](https://arxiv.org/abs/2603.20508)
*Dani Roytburg,Shreya Sridhar,Daphne Ippolito*

Main category: cs.MA

TL;DR: 该论文评估了12个推理语言模型的90k个推理轨迹，提出"迁移效用"来衡量推理轨迹对弱模型的指导价值。研究发现表现最佳的模型可读性反而最低，轨迹长度与迁移效用存在矛盾，奖励模型不奖励可读性，这些发现为多智能体系统中推理轨迹的优化提供了方向。


<details>
  <summary>Details</summary>
Motivation: 语言模型被训练在回答用户问题前进行"推理"，输出大量思考内容。然而，现有评估仅关注最终答案的正确性，而忽略了推理过程的可读性（legibility）。作者认为推理轨迹的质量同样重要，需要被评估。

Method: 评估了12个推理语言模型（RLM）的90k个推理轨迹，引入了"迁移效用"（transfer utility）概念，衡量RLM的推理轨迹对引导较弱模型得出正确答案的有用程度。

Result: 1）表现最好的模型的推理轨迹可读性排名最低；2）基于效率的可读性指标（如轨迹长度）与迁移效用之间存在紧张关系，形成可读性帕累托前沿；3）RLM输出高度可读轨迹的能力可能取决于任务和受众；4）训练RLM的奖励模型本身不会内在奖励可读性。

Conclusion: 这些指标和发现为多智能体未来中推理轨迹的支架搭建提供了路径。
...
### [296] [Personality-Driven Student Agent-Based Modeling in Mathematics Education: How Well Do Student Agents Align with Human Learners?](https://arxiv.org/abs/2603.21358)
*Bushi Xiao,Qian Shen*

Main category: cs.MA

TL;DR: 本研究构建了基于大五人格的学生智能体模型，用于模拟真实学生的学习行为，以克服教育研究中真人实验的伦理限制。研究通过13项实证研究提炼出14项评估标准，验证学生智能体的行为真实性，结果显示71.4%的智能体行为与人类学习者一致。


<details>
  <summary>Details</summary>
Motivation: 教育研究中不同教学方法对学生学习的影响至关重要，但真人实验面临显著的伦理约束，且无法对同一学生进行重复教学实验。LLM生成式智能体为模拟学生行为提供了新途径，但在大规模应用前，必须回答一个根本问题：学生智能体是否真正可信，能否忠实地模拟人类学习？

Method: 构建了基于大五人格（Big Five Personality）的学生智能体模型，涵盖师生互动、自学和考试的全流程。为评估行为真实性，研究收集了13项关于大五人格特质与学习的实证研究，并将其提炼为14项评估标准。

Result: 研究发现71.4%的学生智能体行为与人类学习者保持一致。

Conclusion: 基于大五人格的学生智能体模型能够较好地模拟人类学习行为，为教育研究提供了新的方法论工具，但仍存在约28.6%的不一致之处，需要进一步优化。
...
### [297] [Human-Inspired Pavlovian and Instrumental Learning for Autonomous Agent Navigation](https://arxiv.org/abs/2603.22170)
...
TL;DR: 本文提出了一种受人类神经科学启发的混合强化学习架构，结合了巴甫洛夫式、工具性模型-free（MF）和工具性模型-based（MB）组件，通过情境无线线索（作为条件刺激）塑造内在价值信号，并通过贝叶斯仲裁机制自适应融合MF和MB估计，以在不确定环境中实现更安全、更快速的学习和决策。


<details>
  <summary>Details</summary>
Motivation: 解决不确定环境下自主智能体的速度与安全权衡问题：传统MF RL收敛慢且可能引发不安全探索，MB方法计算量大且对模型误差敏感。

Method: 提出一种混合RL架构，整合巴甫洛夫式学习、工具性MF和工具性MB组件；利用情境无线线索（地理参考环境特征作为条件刺激）塑造内在价值信号并偏置决策；引入内部动机驱动信号调节学习；采用贝叶斯仲裁机制基于预测可靠性自适应融合MF和MB估计。

Result: 仿真结果表明：混合方法加速学习、提高操作安全性、减少在高不确定性区域的导航；巴甫洛夫条件作用促进更安全探索和更快收敛；仲裁机制实现从探索到高效计划驱动利用的平滑过渡。

Conclusion: 生物启发的模块化架构对不确定环境下的鲁棒自适应自主系统具有显著优势。

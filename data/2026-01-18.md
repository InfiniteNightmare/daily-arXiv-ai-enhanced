...
TL;DR: 本文介绍了Babel和ORION两个工具，用于解决FAIR数据原则在实践中互操作性不足的问题，其中Babel通过标识符映射解决多标识符模式问题，ORION通过数据模型转换解决多数据模型问题。


<details>
  <summary>Details</summary>
Motivation: 虽然许多资源都使用符合FAIR原则的词汇表进行标注，但由于不同资源使用的标识符模式和数据模型存在差异，这些资源在实践中的互操作性仍然不足。

Method: 开发了两个工具：1) Babel：通过生成经过整理的标识符映射集，创建等价标识符的clique并通过高性能API暴露；2) ORION：通过摄取知识库并将其转换为社区管理的通用数据模型。

Result: 创建了一个完全互操作的知识库库，可通过https://robokop.renci.org下载使用，成功支持了数据互操作。

Conclusion: Babel和ORION工具能够有效弥补FAIR数据原则理论互操作性与实际互操作性之间的差距，为解决科学数据生态系统的互操作性问题提供实用解决方案。
...
TL;DR: 提出Self-Evolving Concierge System，通过动态专家混合(DMoE)解决LLM智能体的泛化-专业化困境，在保持稳定性的同时降低资源消耗。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型作为自主智能体部署时面临的泛化-专业化困境：单一智能体工具过多导致上下文污染和注意力衰减，而静态多智能体集群则带来显著的延迟和资源开销。

Method: 采用动态专家混合(DMoE)架构，通过基于实时对话分析的元认知引擎检测能力缺口，动态雇佣专业化子智能体；使用LRU驱逐策略处理资源约束，并引入外科手术式历史修剪机制缓解拒绝偏见。

Result: 实验结果显示该架构在保持高任务成功率的同时，相比静态智能体集群显著减少了令牌消耗。

Conclusion: 提出的动态自适应架构在平衡性能与资源效率方面具有显著优势，为智能体系统设计提供了新思路。
...
TL;DR: 该论文提出了一种新颖的多智能体协同学习框架，以解决视觉语言模型在处理分布外概念时的跨模态对齐崩溃问题，显著提升了少样本和零样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理分布外概念时面临跨模态对齐崩溃的问题，这源于模态不平衡导致的特征空间塌缩，限制了模型在新概念上的泛化能力。

Method: 该框架包含四个核心智能体：图像、文本、名称和协调智能体，通过结构化消息传递协同缓解模态不平衡。采用了多智能体特征空间名称学习、上下文交换增强的少样本学习算法，以及自适应动态平衡机制来调节智能体间贡献。

Result: 在VISTA-Beyond数据集上的实验表明，MACL在少样本和零样本设置下均显著改善性能，在不同视觉领域实现了1-5%的精度提升。

Conclusion: 多智能体协同学习框架能有效缓解视觉语言模型在处理分布外概念时的跨模态对齐崩溃问题，通过结构化协作机制实现了更鲁棒的跨模态对齐。
...
TL;DR: 本文研究了LLM在多智能体系统中角色身份偏见对策略推理的影响，发现移除角色设定和提供明确收益是策略推理的必要条件，而角色设定会系统性偏袒社会偏好结果。不同LLM架构对此敏感度差异显著。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多智能体系统中部署日益增多，但角色设定和收益可见性等设计选择如何影响策略推理尚不明确。研究者旨在探究这些系统是作为追求收益最大化的策略推理者，还是作为优先考虑角色对齐的身份驱动参与者。

Method: 使用纳什均衡达成率作为策略推理的诊断指标，在包含四个智能体的复杂环境决策博弈中，系统性地实验了四种LLM架构。通过控制角色设定和收益可见性两个变量，分析它们对均衡选择和策略推理的影响。

Result: 移除角色设定并提供明确收益使Qwen模型能实现高纳什均衡率，表明这两个条件对策略推理都是必要的。角色设定系统性使均衡选择偏向绿色转型等社会偏好结果，且完全无法达到公地悲剧的收益最优均衡。收益明确性的效果完全取决于角色设定是否存在，表明设计选择间存在强交互。Qwen架构对两种因素高度敏感，而Llama和Mistral在各类条件下均表现出僵化的推理行为。

Conclusion: 表征选择（角色设定、收益可见性）是实质性的治理决策，决定了多智能体系统是作为策略推理者还是身份驱动参与者，这对现实世界部署具有重要影响。这些设计选择会系统性改变LLM在多智能体环境中的推理行为，需要审慎考虑。
...
TL;DR: TopoDIM是一个用于LLM多智能体系统的单次通信拓扑生成框架，能够减少46.41%的token消耗，同时提升1.50%的平均性能


<details>
  <summary>Details</summary>
Motivation: 现有基于时空交互范式的方法存在高延迟和高计算成本的问题，而评估和辩论机制已被证明能提升多智能体系统的问题解决能力

Method: 提出TopoDIM框架，采用去中心化执行方式，实现单次异构通信拓扑生成，使智能体能够自主构建异质通信而无需迭代协调

Result: 实验显示TopoDIM相比最先进方法减少了46.41%的总token消耗，同时将平均性能提升了1.50%，并表现出在异构智能体间组织通信的强大适应性

Conclusion: TopoDIM通过单次拓扑生成框架，在保证性能提升的同时显著提高了通信效率，为LLM多智能体系统的通信优化提供了有效解决方案
...
TL;DR: LAMaS: 一种显式优化并行执行中关键路径长度的延迟感知多智能体编排框架，通过显式延迟监督和并行执行优化，在保持任务性能的同时显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统（MAS）由于多步执行和重复模型调用通常导致高推理延迟，这严重限制了其在时间敏感场景中的可扩展性和可用性。现有方法主要优化任务性能和推理成本，并且通常假设顺序执行，因此在并行执行下控制延迟效果不佳。

Method: 提出LAMaS（延迟感知多智能体系系统），这是一种显式优化并行执行中关键路径的延迟感知多智能体编排框架。该方法学习基于并行执行的多智能体系系统编排，通过显式延迟监督使控制器能够构建具有更低延迟的执行拓扑图。

Result: 实验表明，与最先进的多智能体架构搜索基线相比，该方法在多个基准测试中将关键路径长度减少了38-46%，同时保持甚至提高了任务性能。

Conclusion: 研究结果强调了在设计高效多智能体系系统时，在并行执行下显式优化延迟的重要性。该方法为时间敏感场景中的多智能体系系统提供了可扩展的解决方案。代码已开源。
...
TL;DR: 论文探索了多智能体多臂老虎机（MA-MAB）中过程公平性的重要性，提出了一种既保证平等决策权又能优化结果公平的框架，揭示了不同公平目标之间存在价值冲突。


<details>
  <summary>Details</summary>
Motivation: 现有MA-MAB研究主要关注结果公平性（如福利最大化、减少不平等），但心理学、经济学和罗尔斯理论表明过程公平（决策参与权）同样重要。论文旨在弥补这一理论空白。

Method: 引入了过程公平性作为新的公平目标，要求在核心中为所有智能体赋予平等的决策权，并实现结果的比例公平。通过理论分析和实证验证，对比过程公平与结果公平的权衡关系。

Result: 实证结果显示，基于结果优化的公平概念牺牲了平等话语权和代表性；而过程公平政策下结果公平目标（如平等主义和功利主义）的牺牲最小。证明不同公平概念优先考虑根本不同且互不相容的价值。

Conclusion: 过程合法性应作为重要的公平目标得到更多关注，论文提供的框架可将过程公平付诸实践，强调实现公平需要明确的规范性选择。
...
### [13] [AI Survival Stories: a Taxonomic Analysis of AI Existential Risk](https://arxiv.org/abs/2601.09765)
...
### [14] [GUI-Eyes: Tool-Augmented Perception for Visual Grounding in GUI Agents](https://arxiv.org/abs/2601.09770)
...
### [15] [Antisocial behavior towards large language model users: experimental evidence](https://arxiv.org/abs/2601.09772)
...
### [16] [Improving Chain-of-Thought for Logical Reasoning via Attention-Aware Intervention](https://arxiv.org/abs/2601.09805)
...
### [17] [Thinking Long, but Short: Stable Sequential Test-Time Scaling for Large Reasoning Models](https://arxiv.org/abs/2601.09855)
...
### [18] [A Scoping Review of the Ethical Perspectives on Anthropomorphising Large Language Model-Based Conversational Agents](https://arxiv.org/abs/2601.09869)
...
### [19] [Epistemology gives a Future to Complementarity in Human-AI Interactions](https://arxiv.org/abs/2601.09871)
...
### [20] [Beyond Rule-Based Workflows: An Information-Flow-Orchestrated Multi-Agents Paradigm via Agent-to-Agent Communication from CORAL](https://arxiv.org/abs/2601.09883)
...
### [22] [Continuum Memory Architectures for Long-Horizon LLM Agents](https://arxiv.org/abs/2601.09913)
*Joe Logan*

Main category: cs.AI

TL;DR: 本文提出了 Continuum Memory Architecture (CMA)，一种用于LLM智能体的持续性记忆架构，旨在解决传统RAG方法处理记忆时的静态性、无状态和时间连续性缺失等问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于检索增强生成(RAG)的LLM智能体将记忆视为静态查找表，存在信息永久存储、只读检索、缺乏时间连续性等局限，无法有效积累、更新和消歧记忆，限制了长期交互智能体的能力。

Method: 定义CMA架构类别，要求系统通过持久存储、选择性保留、关联路由、时间链式连接以及向高阶抽象整合等机制，在交互中维护和更新内部状态。不披露具体实现细节，而是指定架构要求。

Result: 通过在知识更新、时间关联、关联回忆和上下文消歧等任务上的实证验证，CMA在需要积累、变更或消歧记忆的任务上表现出优于RAG的行为优势，证明了其作为长期智能体必要架构基元的价值。

Conclusion: CMA是长时段智能体不可或缺的架构基元，但仍在延迟、漂移和可解释性等方面面临挑战，为未来研究指出了开放性问题。
...
### [23] [Generative AI collective behavior needs an interactionist paradigm](https://arxiv.org/abs/2601.10567)
...
### [24] [CaMeLs Can Use Computers Too: System-level Security for Computer Use Agents](https://arxiv.org/abs/2601.09923)
...
TL;DR: 本文提出了一个针对LLM/LRM幻觉管理的操作框架，基于根源意识的持续改进循环，通过模型、数据和上下文三因素分类实现针对性干预。


<details>
  <summary>Details</summary>
Motivation: LLM和LRM在金融、法律等高风险领域具有变革潜力，但其产生事实上不正确或无依据内容的幻觉倾向带来了关键可靠性风险。传统方法往往缺乏系统性管理策略。

Method: 该框架将幻觉来源分类为模型、数据和上下文相关因素，集成多方面检测方法（不确定估计、推理一致性等）与分层缓解策略（知识基础、置信度校准等）。通过分层架构（模型、上下文、数据层次形成闭环反馈）和金融数据提取案例展示应用。

Result: 该框架为在受监管环境中构建可信的生成式AI系统提供了系统性、可扩展的方法论，通过闭环反馈机制实现渐进可靠性增强。

Conclusion: 基于根源意识的持续改进框架能够有效管理LLM/LRM幻觉，为高风险领域的生成式AI系统建立可靠性提供了结构化、针对性的解决方案。该框架强调通过分类干预而非通用修正来实现精确的幻觉管理与缓解。
...
TL;DR: 提出了针对中国劳动法的专业大语言模型LabourLawLLM和评估基准LabourLawBench，在多个劳动法任务中表现优于通用和现有法律专用模型


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在处理需要精确法律知识、复杂推理和上下文敏感性的专业法律子领域时存在局限，特别是在中国劳动法领域

Method: 构建专门针对中国劳动法的LabourLawLLM模型，并创建包含多种劳动法任务的综合基准LabourLawBench，采用客观指标和基于GPT-4的主观评估结合的评估框架

Result: 实验表明LabourLawLLM在各种任务类别上始终优于通用和现有法律专用大语言模型

Conclusion: 该研究为在其他法律子领域构建专业化大语言模型提供了可扩展的方法，提高了法律AI应用的准确性、可靠性和社会价值
...
TL;DR: Memo-SQL 是一个无需训练的 NL2SQL 框架，通过结构化分解和经验感知的自我纠正解决现有系统依赖正确示例和任意分解导致准确性-效率权衡的问题，在 BIRD 基准上实现了零微调方法的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的 NL2SQL 系统存在两个关键限制：1) 仅依赖正确示例进行上下文学习，忽略了历史错误修复对中的丰富信号；2) 测试时缩放方法通常任意分解问题，产生几乎相同的 SQL 候选方案，降低了集成增益。这些方法还面临明显的准确性-效率权衡。

Method: Memo-SQL 采用结构化分解（实体、分层和原子顺序三种策略）来促进多样化推理，同时构建包含成功查询和历史错误修复对的动态内存，通过检索增强提示在推理时引入相关示例，无需微调或外部 API。

Result: 在 BIRD 基准上，Memo-SQL 实现了 68.5% 的执行准确率，在零微调方法中达到了最先进的性能，同时比先前的 TTS 方法节省了 10 倍以上的资源。

Conclusion: Memo-SQL 通过结构化分解和经验感知的自我纠正成功解决了 NL2SQL 系统的效率和准确性问题，为训练免费方法建立了新的基准。
...
TL;DR: 提出了一个基于荣格心理学类型的LLM人格建模框架，包含主导-辅助协调、强化-补偿和反思三种机制，实现既有细微特质又能动态适应交互需求的人格表达。


<details>
  <summary>Details</summary>
Motivation: 现有方法在实现既细致入微又适应性强的人格表达方面存在困难，而人格对LLM与人交互时的参与度、决策和真实感都至关重要。

Method: 采用荣格心理学类型模型，设计三种机制：主导-辅助协调保证核心表达的连贯性，强化-补偿实现情景适应性调整，反思机制驱动长期人格演化。

Result: 通过迈尔斯-布里格斯类型指标问卷和多样化挑战场景测试，初步评估表明该方法能实现连贯、情景敏感的自然交互。

Conclusion: 具有演化能力的人格感知LLM能够支持连贯且情景敏感的交互，为人机交互中自然主义智能体设计提供可能。
...
TL;DR: 本文提出 FilDeep，一种基于保真度的深度学习框架，用于解决弹性塑性固体大变形问题。该框架通过同时利用低保真度（大量但精度较低）和低保真度（少量但精度高）数据来克服数据量与精度之间的两难困境。


<details>
  <summary>Details</summary>
Motivation: 大变形计算在制造业中至关重要，传统数值方法存在局限，而深度学习通常依赖大量高精度数据，这在真实场景中难以获取。现有数据构建过程面临数据量与精度之间的两难困境，导致深度学习模型性能欠佳。

Method: 针对典型大变形问题（拉伸弯曲），提出 FilDeep 框架。框架采用多保真度数据（低保真度与高保真度数据）同时训练，并设计了注意力驱动的跨保真度模块，以有效捕捉多保真度数据间的长程物理相互作用。

Result: 大量实验表明，FilDeep 能够持续取得最先进的性能，并可在制造中高效部署。

Conclusion: FilDeep 是首个为多保真度数据设计的大变形深度学习框架，成功解决了数据量-精度两难困境，为实际制造场景中的大规模变形计算提供有效解决方案。
...
TL;DR: 基于OpenRouter平台对超过100万亿token的实际LLM交互进行分析，研究发现开放权重模型被广泛采用，创意角色扮演和编码辅助类应用异常流行，智能体推理兴起，并识别出早期用户的长期参与度更高的'玻璃鞋'效应。


<details>
  <summary>Details</summary>
Motivation: 随着首个广泛采用推理模型o1的发布，LLM领域从单次模式生成转向多步审慎推理，但对其实际应用方式的实证理解仍滞后于快速发展。本研究旨在通过分析真实世界的LLM交互数据，填补这一知识空白。

Method: 利用OpenRouter平台作为AI推理提供商，分析超过100万亿token的LLM实际交互数据，涵盖不同任务、地理位置和时间段。

Result: 观察到开放权重模型被大量采用，创意角色扮演和编码辅助类型任务异常流行，智能体推理兴起。留存分析发现早期用户群体参与度远高于后期用户，称为'玻璃鞋'效应。

Conclusion: 开发者和终端用户与LLM的实际交互方式复杂多元，数据驱动的使用理解可为模型构建者、AI开发者和基础设施提供商提供有价值的设计和部署洞见。
...
### [32] [MATRIX AS PLAN: Structured Logical Reasoning with Feedback-Driven Replanning](https://arxiv.org/abs/2601.10101)
...
### [34] [Following the Teacher's Footsteps: Scheduled Checkpoint Distillation for Domain-Specific LLMs](https://arxiv.org/abs/2601.10114)
...
TL;DR: 研究发现大型语言模型在预测重复用户行为的时间间隔方面具有基本能力，但效果不如专用机器学习模型，且过多的用户上下文反而会降低其性能。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索大型语言模型是否能够从结构化的行为数据中推断时间规律，特别是在预测重复行为（如重复购买）的时间间隔方面，并考察不同层次上下文信息对其预测行为的影响。

Method: 通过对典型的重复购买场景在零样本设置下进行系统实验，将先进的大型语言模型与统计模型和机器学习模型进行基准测试。

Result: 发现虽然大型语言模型能够超越轻量级统计基准，但始终落后于专用的机器学习模型，表明它们在捕捉定量时间结构方面的能力有限；适度的上下文能提高准确性，但过多用户级细节反而会降低性能。

Conclusion: 研究揭示了当前大型语言模型在结构化时间推理方面的基本局限性，挑战了'更多上下文=更好推理'的假设，并为设计未来能够整合统计精度与语言灵活性的上下文感知混合模型提供了指导。
...
TL;DR: 本研究提出DecisionLLM，通过将轨迹数据与任务描述对齐，让大语言模型能够处理连续数值的离线决策任务，在迷宫和竞标场景中超越传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在处理长序列决策任务（如实时竞标）时面临挑战，而大语言模型在复杂推理任务中表现出色。但LLMs无法直接理解连续数值，这限制了其在决策任务中的应用。

Method: 提出DecisionLLM框架，将轨迹数据视为独立模态，学习将轨迹数据与自然语言任务描述对齐，使LLMs能够自回归预测未来决策。建立了衡量模型规模、数据量和数据质量的缩放定律。

Result: DecisionLLM-3B在Maze2D umaze-v1上比传统Decision Transformer提升69.4分，在AuctionNet上提升0.085分。建立了有效的缩放定律体系。

Conclusion: DecisionLLM扩展了AIGB范式，证明LLMs在离线决策任务中的潜力，为在线竞标等应用指出了有前景的研究方向。
...
TL;DR: 针对针极肌电信号高采样率带来的计算挑战，本文提出了一个系统评估下采样信息损失的工作流，通过形态失真度量和分类性能分析，指导在保留诊断信息的同时降低计算负载。


<details>
  <summary>Details</summary>
Motivation: 针极肌电信号的高采样率和异质性给基于特征的机器学习模型带来计算挑战，特别是在近实时分析场景中。尽管下采样是潜在解决方案，但对其造成的诊断信息损失和分类性能影响尚不明确。

Method: 提出结合形态失真度量、特征空间分析和分类结果的系统工作流，使用三类神经肌肉疾病分类任务实验评估不同下采样算法和配置的影响。

Result: 形态感知下采样算法在保留峰值结构和信号形态方面优于标准抽取；工作流能识别在显著降低计算负载的同时保持诊断信息的下采样配置。

Conclusion: 工作流为近实时针极肌电分析提供了实用的下采样配置选择指导，并展示了可推广到其他高频时间序列应用的平衡数据缩减与模型性能的方法。
...
TL;DR: 本文提出了GFM4GA——一个用于群体异常检测的新型图基础模型，通过双级对比学习预训练捕捉群体异常结构和特征不一致性，在少样本场景下通过参数约束和异常比例加权进行微调，并利用异常邻居确定的群体上下文扩展对未见异常的自适应能力，在AUROC和AUPRC上分别平均提升2.85%和2.55%。


<details>
  <summary>Details</summary>
Motivation: 群体异常检测面临异常模式多样性的挑战；当前图基础模型虽成功应用于个体异常检测，但无法泛化到群体异常，因群体异常需要整体检测且异常群体内的个体可能看似正常。

Method: 提出GFM4GA框架：1）通过基于特征估计和群体提取的双级对比学习进行预训练，捕捉潜在的群体异常结构和特征不一致性；2）在下游任务中，采用参数约束和群体异常比例加权的少样本微调；3）利用标记异常邻居确定的群体上下文扩展对未见异常的适应能力。

Result: 实验结果表明GFM4GA优于现有群体异常检测器和个体异常检测的图基础模型，在AUROC和AUPRC指标上平均分别提升了2.85%和2.55%。

Conclusion: GFM4GA通过双级对比学习和少样本自适应微调，成功解决了群体异常检测中整体模式捕捉和对未见异常的泛化问题，为图基础模型在群体异常检测领域的应用提供了有效方案。
...
TL;DR: 论文提出EAPO（证据增强策略优化），通过引入证据奖励模型和自适应奖励-策略协同进化机制，解决了长上下文推理中结果奖励稀疏和证据检索缺乏监督的问题。


<details>
  <summary>Details</summary>
Motivation: 强化学习（RL）应用于长文本推理时，结果奖励稀疏，无法惩罚无根据的猜测，导致证据检索过程缺乏有效监督，这是当前方法的局限性。

Method: 首先建立证据增强推理范式，通过树状证据采样验证精确证据提取是关键瓶颈。然后提出EAPO算法，采用奖励模型计算组相对证据奖励，提供密集过程监督。进一步引入自适应奖励-策略协同进化机制，迭代优化奖励模型以确保精确的过程指导。

Result: 在八个基准测试上的全面评估表明，EAPO显著优于当前最先进的基线模型，有效提升了长上下文推理性能。

Conclusion: EAPO通过增强证据检索的监督机制，解决了长上下文稀疏奖励挑战，为大规模语言模型的长文本推理提供了有效解决方案。
...
TL;DR: 提出了 C‑GRASP 方法，通过基于检索增强生成（RAG）的标准化思维链和 Z‑分数优先级机制，解决大语言模型在心率变异性分析中因呼吸性窦性心律失常等生理幻觉、个体基线忽略和非线性指标短数据不稳定而引起的问题，在情绪分类和临床推理一致性上均优于基线。


<details>
  <summary>Details</summary>
Motivation: 心心率变异性（HRV）是自主神经监测的重要无创指标，但大语言模型（LLM）在用于 HRV 解释时容易产生生理幻觉，如呼吸性窦性心律失常污染、非线性指标在短数据中的不稳定性，以及忽视个体基线而依赖群体常模。现有方法缺乏能保证临床可解释性和可溯源性的推理框架，限制了其在生物医学工程中的安全应用。

Method: 构建 C‑GRASP（临床接地气的情绪信号处理推理）框架，包含一个基于 RAG 的、有约束的思维链管道，将 HRV 解释分解为八个可追溯的推理步骤。核心是 Z‑分数优先级层级，强制对个体基线变化给予比群体统计更高的权重。系统通过自动化的 RSA 感知约束，防止频域指标被呼吸性窦性心律失常污染，从而缓解频谱幻觉。在 DREAMER 数据集的 414 次试验中评估了该系统。

Result: C‑GRASP 与高规模推理模型（如 MedGemma3‑thinking）结合时，在四类情绪分类任务中达到 37.3% 的准确率，临床推理一致性（CRC）得分为 69.6%。消融实验确认，个体化 Delta Z‑分数模块是关键逻辑锚点，能有效防止原生 LLM 常见的“群体偏见”。

Conclusion: C‑GRASP 实现了从黑盒分类到透明、基于证据的临床决策支持的转变，为人工智能在生物医学工程中的安全集成铺平了道路。该方法不仅提升了情绪分类性能，更重要的是通过可追溯的推理步骤和生理合理的约束，增强了 HRV 解释的临床可信度和可解释性。
...
TL;DR: ErrEval：一个面向错误的自动问题生成评估框架，通过显式错误诊断提升评估质量


<details>
  <summary>Details</summary>
Motivation: 现有问题生成评估方法多为黑盒式整体评估，缺乏对事实幻觉、答案不匹配等关键缺陷的显式建模，导致忽略缺陷并高估问题质量

Method: 将评估重构为两阶段过程：1) 轻量级即插即用错误识别器检测和分类结构、语言、内容相关常见错误；2) 将诊断信号作为显式证据指导LLM评估器做出更细粒度、有根据的判断

Result: 在三个基准测试上的实验证明ErrEval有效性，显式诊断信息能改善与人工判断的一致性；进一步分析确认ErrEval能有效缓解低质量问题的高估问题

Conclusion: ErrEval通过显式错误诊断的问题生成评估框架，提高了评估的细粒度和准确性，解决了现有黑盒评估方法对缺陷不敏感的问题
...
TL;DR: 本文提出并应用了一个多层诊断框架，通过跨架构研究分析大语言模型在微调后的泛化失效原因。研究发现：1) 泛化依赖于架构与数据多样性的协同作用；2) 泛化高度依赖于架构特性；3) 某些架构本身更具泛化性。


<details>
  <summary>Details</summary>
Motivation: 微调大语言模型在专业任务上取得了最先进性能，但诊断这些模型为何变得脆弱且无法泛化仍是一个关键开放问题。需要深入理解架构、数据和训练策略之间的相互作用，以确保AI的可靠性。

Method: 采用多层诊断框架，包括对 Llama 3.1 8B、Gemma 2 9B 和 Mistral 模型进行针对钓鱼检测任务的微调，并结合 SHAP 分析和机理可解释性技术来揭示泛化失效的根源机制。

Result: 三个关键发现：1) Gemma 2 9B 在风格多样化的通用数据集上训练时达到 >91% F1 分数；2) Llama 3.1 8B 在窄域表现良好但无法整合多样数据导致性能显著下降；3) Mistral 模型在多种训练范式下表现一致且稳健。

Conclusion: 可靠的AI需要对架构、数据和训练策略之间的相互作用进行深入验证。本研究通过定位导致泛化失效的缺陷启发式方法，为诊断和理解泛化失败提供了具体方法论。
...
TL;DR: 本论文通过对分层推理模型(HRM)进行机制性研究，揭示了其三个令人惊讶的推理模式：在简单谜题上失败、存在'顿悟'动态以及多固定点问题，表明HRM更像是'猜测'而非'推理'，并据此提出了三种增强策略（数据增强、输入扰动和模型引导）来提升其推理性能。


<details>
  <summary>Details</summary>
Motivation: 尽管分层推理模型在多项推理任务中表现卓越，远超基于大型语言模型的推理器，但对其推理机制的理解仍不充分。本研究旨在深入探索HRM的推理模式，揭示其潜在的优势与失败模式，从而更好地理解这类模型如何进行'推理'。

Method: 本研究采用机制性分析方法，首先观察HRM在推理过程中的典型模式，然后基于发现的'猜测'现象提出三种扩展策略：1）数据增强（提升猜测质量）；2）输入扰动（利用推理随机性增加猜测数量）；3）模型引导（利用训练随机性增加猜测数量）。最终将这些方法整合为增强型HRM。

Result: 通过综合应用所有方法，研究开发了增强型HRM，在数独-极限任务上的准确率从54.5%显著提升至96.9%。实验验证了所提策略的有效性，并展示了HRM改进后的强大性能。

Conclusion: HRM本质上是执行多层次猜测而非严格逻辑推理，其表现可以通过'猜测质量-数量扩展'框架显著提升。这一发现不仅为改进HRM提供了实用方案，也为深入理解推理模型的工作机制提供了新的科学视角。
...
TL;DR: 提出一种基于文档结构和多样性约束的上下文构建框架（context bubble），可生成紧凑、信息丰富且可追踪的上下文，避免了传统检索增强生成方法的碎片化、重复内容和上下文不完整等问题。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成（RAG）使用top-k段落检索，存在信息图碎片化、内容重复、过检索以及无法覆盖查询的二级和三级方面等问题，特别是在有限上下文窗口下效果不佳。

Method: 框架通过结构感知和多样性约束的上下文气泡构建，利用文档多粒度层次结构（如图节、表格行），基于任务条件化结构先验指导检索，从高相关性锚点跨度开始，在严格令牌预算下通过约束选择构建上下文气泡，平衡查询相关性、边际覆盖率和冗余惩罚。输出包含完整检索轨迹，提供可审计性和确定性调优。

Result: 在企业文档上的实验显示，上下文气泡能显著减少冗余上下文，更好地覆盖次要方面，在有限上下文窗口内提供更好的答案质量和引用忠实度。消融研究表明，结构先验和多样性约束选择两者缺一不可。

Conclusion: 该框架通过整合文档结构和多样性约束，有效解决了传统RAG的局限性，生成紧凑且信息丰富的上下文集，为基于检索的语言模型应用提供了更高效、可审计的解决方案。
...
TL;DR: GenAI对建筑概念设计任务的影响：提升新手设计表现但降低创作自我效能感


<details>
  <summary>Details</summary>
Motivation: 研究生成式AI在建筑设计任务中对表现、创造性自我效能感和认知负荷的影响，填补AI辅助设计工具实际效果的研究空白，探究不同经验水平用户的差异化反应

Method: 36名学生参与者进行两阶段建筑概念设计任务：第一阶段独立完成，第二阶段分别使用GenAI辅助（实验组）或在线建筑项目库（对照组）。专家评估设计成果，参与者自我报告自我效能感和认知负荷。采用双重差分分析，并进行子组分析和提示使用模式分析

Result: 1) GenAI总体上未显著提升设计表现；2) 子组分析显示GenAI显著提升新手设计师的表现；3) 使用GenAI的学生创造性自我效能感下降；4) 认知负荷无显著差异；5) 迭代想法生成和视觉反馈的提示模式与认知负荷降低相关

Conclusion: GenAI的有效性取决于用户先验专业知识和交互策略（特别是提示方式），新手更能从GenAI中获益，但需注意AI辅助可能对创意自我认知产生负面影响
...
### [63] [Putting green software principles into practice](https://arxiv.org/abs/2601.09741)
*James Uther*
...
### [64] [A Governance Model for IoT Data in Global Manufacturing](https://arxiv.org/abs/2601.09744)
*Vignesh Alagappan*
...
### [65] [Enhancing Formal Software Specification with Artificial Intelligence](https://arxiv.org/abs/2601.09745)
...
### [66] [R-LAM: Reproducibility-Constrained Large Action Models for Scientific Workflow Automation](https://arxiv.org/abs/2601.09749)
*Suriya Sureshkumar*

Main category: cs.SE

TL;DR: R-LAM 框架旨在解决大行动模型在科学工作流自动化中的可重复性问题，通过结构化行动模式、确定性执行策略和显式溯源跟踪来确保可审计和可重放的执行。


<details>
  <summary>Details</summary>
Motivation: 科学工作流对可重复性、可审计性和确定性执行有严格要求，而现有的大语言模型驱动的智能体无法满足这些要求，存在隐性状态变化、非确定性执行和实验结果不可重复等问题，限制了其在科学领域的应用。

Method: 提出 R-LAM 框架，引入结构化行动模式、确定性执行策略和显式溯源跟踪，支持失败感知执行循环和受控制的工作流分叉，实现了可审计和可重放的动作执行。

Result: 实验评估表明，R-LAM 相比无约束的基于 LLM 的智能体，在代表性科学工作流中提高了可重复性成功率和执行可靠性，同时保持了对工作流执行的自适应控制。

Conclusion: R-LAM 框架为在科学工作流自动化中应用大行动模型提供了解决策可重复性约束的有效解决方案，并通过开源实现促进了可重复性研究。
...
### [67] [SAGE: Tool-Augmented LLM Task Solving Strategies in Scalable Multi-Agent Environments](https://arxiv.org/abs/2601.09750)
...
### [68] [Investigating Tool-Memory Conflicts in Tool-Augmented LLMs](https://arxiv.org/abs/2601.09760)
...
### [69] [Explicating Tacit Regulatory Knowledge from LLMs to Auto-Formalize Requirements for Compliance Test Case Generation](https://arxiv.org/abs/2601.09762)
*Zhiyi Xue,Xiaohong Chen,Min Zhang*

Main category: cs.SE

TL;DR: RAFT是一个通过多LLM显化隐性监管知识来实现需求自动形式化和合规测试生成的框架


<details>
  <summary>Details</summary>
Motivation: 高度监管领域的合规测试主要依赖手动完成，专家需将复杂法规转为可执行测试用例。现有混合方法虽通过形式化模型约束LLM来缓解幻觉问题，但仍依赖成本高昂的手动建模。

Method: 提出RAFT框架，采用自适应净化-聚合策略从多个LLM中显化隐性监管知识，并集成到三个工件：领域元模型、形式化需求表示和可测试性约束。这些工件被动态注入提示中指导高精度需求形式化和自动测试生成。

Result: 在金融、汽车和电力领域的实验表明，RAFT达到专家级表现，显著优于最先进方法，同时减少总体生成和审阅时间。

Conclusion: RAFT框架通过利用多LLM的隐性知识显化和动态提示注入，实现了高质量的需求自动形式化和合规测试生成，为高度监管领域的自动化合规测试提供了有效解决方案。
...
### [70] [LLM-Based Agentic Systems for Software Engineering: Challenges and Opportunities](https://arxiv.org/abs/2601.09822)
...
### [71] [Adoption and Evolution of Code Style and Best Programming Practices in Open-Source Projects](https://arxiv.org/abs/2601.09832)
...
### [72] [On Fun for Teaching Large Programming Courses](https://arxiv.org/abs/2601.09842)
*Walid Maalej*
...
### [73] [Beyond Strict Rules: Assessing the Effectiveness of Large Language Models for Code Smell Detection](https://arxiv.org/abs/2601.09873)
...
### [74] [Self-reflection in Automated Qualitative Coding: Improving Text Annotation through Secondary LLM Critique](https://arxiv.org/abs/2601.09905)
*Zackary Okun Dunivin,Mobina Noori,Seth Frey,Curtis Atkinson*

Main category: cs.SE

TL;DR: 提出一种两阶段LLM辅助定性编码工作流，第一级LLM使用人工设计的编码本进行高召回率标注，第二级通过自我反思机制重新审查阳性标注以减少误报，在Apache邮件数据集上显著提升分类精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于零样本或少样本的LLM定性编码方法会产生大量错误标注，即使经过精心提示也难以达到可容忍的水平，需要一种通用且简单的工作流来降低误报率。

Method: 设计两阶段工作流：第一阶段使用LLM应用人工设计的改编编码本进行初步标注；第二阶段通过另一个LLM批判性地重新阅读原文及第一阶段的推理，进行自我反思并给出最终决策；针对观察到的错误模式还开发了特定编码的批评条款。

Result: 在Apache软件基金会项目评估讨论的3000多封邮件上评估六个定性编码，第一阶段LLM的假阳性率高达8%-54%，通过第二阶段的自我反思，F1分数提升0.04-0.25，其中两个表现较差的编码从0.52和0.55提升到0.69和0.79；人工评估发现主要错误类型为误解和元讨论。

Conclusion: 采用第一级高召回率标注结合第二级批判性反思，可实现精确优先、计算量轻的控制；在人工指导和验证下，自我反思机制可集成到现有LLM辅助标注流程中，有效降低噪声并挽救原本不可用的分类器。
...
### [75] [S$^2$F: Principled Hybrid Testing With Fuzzing, Symbolic Execution, and Sampling](https://arxiv.org/abs/2601.10068)
...
### [76] [Mark My Works Autograder for Programming Courses](https://arxiv.org/abs/2601.10093)
*Yiding Qiu,Seyed Mahdi Azimi,Artem Lensky*

Main category: cs.SE

TL;DR: 介绍了一个本地化自动评分系统 Mark My Works，结合传统单元测试与LLM生成解释，并对比了AI评分与人工评分的结果。


<details>
  <summary>Details</summary>
Motivation: 解决大型编程课程难以及时、详细反馈学生代码的问题。

Method: 开发了Mark My Works系统，使用基于角色的提示分析代码提交，评估代码质量并生成教学反馈。在191名学生的课程中进行试点，将AI系统与人工评分在79份提交中进行比较。

Result: AI评分与人工评分无线性相关（r=-0.177，p=0.124），但两者均表现出相似的左偏分布。AI系统评分更保守（平均分59.95 vs 人工80.53），但生成的详细技术反馈显著更多。

Conclusion: 尽管AI与人工评分体系不同，但它们识别出相似的代码质量层级，且AI能提供更详细的技术反馈。
...
### [77] [Towards Online Malware Detection using Process Resource Utilization Metrics](https://arxiv.org/abs/2601.10164)
...
### [78] [Agentic Pipelines in Embedded Software Engineering: Emerging Practices and Challenges](https://arxiv.org/abs/2601.10220)
...
### [79] [Evolving with AI: A Longitudinal Analysis of Developer Logs](https://arxiv.org/abs/2601.10258)
*Agnia Sergeyuk,Eric Huang,Dariia Karaeva,Anastasiia Serova,Yaroslav Golubev,Iftekhar Ahmed*

Main category: cs.SE

TL;DR: 本文通过混合方法研究长期AI编程助手使用对开发工作流的影响，发现AI用户产出的代码更多但删除也更多，同时调查显示开发者感知到生产力提升但其他维度变化不大。


<details>
  <summary>Details</summary>
Motivation: 尽管AI编程助手已成为IDE标配工具，但其对日常开发的长期影响缺乏实证研究，现有研究多关注短期使用或自我报告，需要了解持续AI使用如何真正重塑编码实践，填补理解AI如何改变软件工作流程的研究空白。

Method: 采用混合方法进行纵向研究：1）收集800名开发者两年时间内的细粒度遥测数据；2）对62名专业人士进行问卷调查；3）从五个维度分析工作流变化：生产力、代码质量、代码编辑、代码重用和上下文切换。

Result: 遥测数据显示：AI用户产出的代码量显著增加，但删除的代码量也显著更多。问卷调查结果：受访者报告生产力有所提升，但认为其他维度（代码质量、代码编辑、代码重用、上下文切换）变化不大。

Conclusion: AI编程助手正在无声地重构软件工作流程，表现为更高的代码产出和删除量，而开发者对此的感知与实际行为变化存在差异。研究结果为设计未来AI增强工具提供了实证依据，揭示了工作流结构的变化可能超出开发者自我认知的范围。
...
### [80] [Model See, Model Do? Exposure-Aware Evaluation of Bug-vs-Fix Preference in Code LLMs](https://arxiv.org/abs/2601.10496)
...
### [81] [Distributed Linearly Separable Computation with Arbitrary Heterogeneous Data Assignment](https://arxiv.org/abs/2601.10177)
...
### [82] [Social Determinants of Health Prediction for ICD-9 Code with Reasoning Models](https://arxiv.org/abs/2601.09709)
*Sharim Khan,Paul Landes,Adam Cross,Jimeng Sun*

Main category: cs.LG

TL;DR: 该研究探索使用推理模型和传统大语言模型在MIMIC-III数据集上进行医院入院多标签社会健康决定因素ICD-9编码分类，通过利用现有ICD-9编码预测入院事件，取得了89%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 社会健康决定因素（SDoH）与患者预后相关，但很少在结构化数据中捕捉。虽然已有研究从临床文本中自动提取这些标记以补充诊断系统，但在长距离依赖的入院或纵向笔记中进行预测仍然具有挑战性。

Method: 在MIMIC-III数据集上，利用推理模型和传统大语言模型进行医院入院多标签SDoH ICD-9编码分类，通过挖掘现有ICD-9编码信息来预测入院事件。

Result: 方法在入院预测中实现了89%的F1分数，并发现在139个入院记录中缺失SDoH编码。同时提供了可复现结果的代码。

Conclusion: 研究证明利用现有ICD-9编码信息可以在入院事件中有效预测SDoH，为补充诊断系统提供了可行的技术路径，同时也揭示了数据集中存在的SDoH编码缺失问题。
...
### [83] [The Geometry of Thought: Disclosing the Transformer as a Tropical Polynomial Circuit](https://arxiv.org/abs/2601.09775)
...
### [84] [TimeSAE: Sparse Decoding for Faithful Explanations of Black-Box Time Series Models](https://arxiv.org/abs/2601.09776)
...
### [85] [Eluder dimension: localise it!](https://arxiv.org/abs/2601.09825)
...
### [86] [A New Convergence Analysis of Plug-and-Play Proximal Gradient Descent Under Prior Mismatch](https://arxiv.org/abs/2601.09831)
...
### [87] [A pipeline for enabling path-specific causal fairness in observational health data](https://arxiv.org/abs/2601.09841)
...
### [88] [Advancing Model Refinement: Muon-Optimized Distillation and Quantization for LLM Deployment](https://arxiv.org/abs/2601.09865)
*Jacob Sander,Brian Jalaian,Venkat R. Dasari*

Main category: cs.LG

TL;DR: 提出集成框架解决LLMs在边缘设备部署时的性能与资源矛盾，通过量化、适配与数据蒸馏实现高效压缩与加速，量化效果与精度保持验证通过基准测试


<details>
  <summary>Details</summary>
Motivation: 大语言模型在资源受限的边缘设备部署面临计算、存储和能效挑战，需要在压缩模型以加速推理的同时保持任务性能。为解决任务数据获取、微调优化和模型压缩三个核心难题，需要集成化的优化方案。

Method: 框架集成了GPTQ量化、低秩自适应（LoRA）和专门的数据蒸馏流程。使用数据蒸馏和基于Kullback-Leibler散度的知识蒸馏，结合贝叶斯超参数优化和Muon优化器，实现模型复杂度显著降低。

Result: 实验显示内存压缩达2倍（如6GB模型降至3GB），针对专业化任务实现高效推理。在标准LLM基准测试中表现优于单独使用GPTQ量化，Muon优化器显著提升了微调模型在量化过程中的精度保持能力。

Conclusion: 集成化优化框架有效平衡了LLMs在边缘部署的资源约束与性能需求，量化与蒸馏结合在压缩模型的同时保持或提升任务性能，为边缘AI提供了实用的解决方案。
...
### [89] [The PROPER Approach to Proactivity: Benchmarking and Advancing Knowledge Gap Navigation](https://arxiv.org/abs/2601.09926)
...
### [90] [Interpolation-Based Optimization for Enforcing lp-Norm Metric Differential Privacy in Continuous and Fine-Grained Domains](https://arxiv.org/abs/2601.09946)
*Chenxi Qiu*
...
### [91] [Kinematic Tokenization: Optimization-Based Continuous-Time Tokens for Learnable Decision Policies in Noisy Time Series](https://arxiv.org/abs/2601.09949)
*Griffin Kearney*
...
TL;DR: 本文认为机器学习价值链存在结构性不平等问题，即数据从输入到模型权重再到合成输出的过程中，技术信号不断被提炼，但经济价值却从数据生产者手中剥离，导致多数价值流向聚合方，而创作者报酬几乎为零。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习数据生态系统存在严重的经济不平等现象，数据生成者几乎无法从数据价值中获益，这种不平等不仅影响经济福利，还可能破坏支撑当前学习算法的反馈循环，威胁整个系统的可持续性。

Method: 通过分析73个公开数据交易案例来量化价值分配不平等现象，识别出数据来源缺失、谈判权力不对称和非动态定价三个结构性缺陷，并提出公平数据价值交换（EDVEX）框架作为解决方案。

Result: 分析显示大多数价值流向数据聚合方，文档记录中的创作者版税趋近于零，交易条款普遍缺乏透明度，证实了数据经济学中的处理不平等问题。

Conclusion: 需要建立新的市场机制来解决机器学习价值链中的结构性不平等问题，EDVEX框架为实现公平的数据价值交换提供了基础，同时提出了社区可以做出具体贡献的研究方向。
...
### [93] [In-Context Operator Learning on the Space of Probability Measures](https://arxiv.org/abs/2601.09979)
*Frank Cole,Dixi Wang,Yineng Chen,Yulong Lu,Rongjie Lai*

Main category: cs.LG

TL;DR: 提出最优运输问题的上下文算子学习方法，利用少量样本作为提示学习从分布对到OT映射的算子，无需推理时的梯度更新。


<details>
  <summary>Details</summary>
Motivation: 传统最优运输方法需要为每个分布对重新计算OT映射，计算成本高。本文旨在学习一个通用的上下文算子，仅用少量样本提示就能预测OT映射，实现高效推理。

Method: 1. 参数化解法算子
2. 两种理论框架：非参数设定（任务集中在低维流形）和参数设定（如高斯分布族）
3. 建立泛化界限和样本复杂度分析
4. 提出在参数设定中能精确恢复OT映射的显式架构

Result: 1. 非参数设定：建立了上下文准确率与提示大小、内在任务维度和模型容量之间的标度律关系
2. 参数设定：给出了精确恢复OT映射的显式架构和样本超额风险界限
3. 实验验证：合成运输任务和生成建模基准测试证明了框架的有效性

Conclusion: 上下文算子学习为最优运输问题提供了数据高效的解决方案，能仅用少量样本提示预测OT映射，同时在理论和实验上都表现出良好性能。
...
### [94] [FaTRQ: Tiered Residual Quantization for LLM Vector Search in Far-Memory-Aware ANNS Systems](https://arxiv.org/abs/2601.09985)
...
### [95] [Continuous-Depth Transformers with Learned Control Dynamics](https://arxiv.org/abs/2601.10007)
*Peter Jemley*

Main category: cs.LG

TL;DR: 提出了一种混合transformer架构，用连续深度的神经ODE模块替换离散中间层，实现了生成属性的推理时控制。


<details>
  <summary>Details</summary>
Motivation: 标准transformer通过固定离散层处理表示，限制了生成属性的动态控制能力，需要一种能通过隐语义信号进行连续深度调控的方法。

Method: 构建hybrid transformer架构，用神经ODE模块替换中间层，由向量场F_θ(H, τ, u)控制深度连续性，u为低维控制信号通过拼接注入。

Result: 实验验证：(1)梯度流稳定无爆炸/消失；(2)情感控制准确率98%/88%；(3)插值轨迹差异仅0.068%；(4)延迟与离散基线相当；同时显示控制信号在向量场中划分出具有不同曲率特性的不同动力学机制。

Conclusion: 带有学习控制信号的连续深度动力学为可引导语言生成提供了一种可行高效机制，伴随方法实现了与积分深度无关的O(1)内存训练。
...
### [96] [PID-Guided Partial Alignment for Multimodal Decentralized Federated Learning](https://arxiv.org/abs/2601.10012)
...
### [97] [Time Aggregation Features for XGBoost Models](https://arxiv.org/abs/2601.10019)
*Mykola Pinchuk*

Main category: cs.LG

TL;DR: 本文研究点击率预测中XGBoost模型的时间聚合特征，在严格的时序数据划分和无前瞻特征约束下，发现尾随窗口设计相比基线有稳定提升，事件计数窗口提供了额外的小幅增益。


<details>
  <summary>Details</summary>
Motivation: 在点击率预测中，模型通常需要利用历史数据进行特征工程。但在实际部署时，必须遵循严格的时序约束：特征只能基于当前时间点之前的真实历史数据构建，不能使用未来信息。本文旨在系统比较不同时间聚合窗口设计对模型性能的影响，为实际应用提供指导。

Method: 在Avazu点击率预测数据集上进行研究，采用严格的时序数据划分（out-of-time splits）和无前瞻特征约束（no-lookahead feature constraint）。以时间感知的目标编码为强基线，在此基础上为模型添加实体历史时间聚合特征，系统比较了多种窗口设计：尾随窗口（trailing windows）、事件计数窗口（event count windows）、间隔窗口（gap windows）和分桶窗口（bucketized windows）。

Result: 在两个滚动尾部时间折叠上（使用确定性的10%样本），尾随窗口设计相比纯目标编码基线，使ROC AUC提高了约0.0066-0.0082，PR AUC提高了约0.0084-0.0094。在所有时间聚合设计网格中，只有事件计数窗口在尾随窗口基础上提供了持续的小幅改进。间隔窗口和分桶窗口在该数据集和实验协议下表现较差。

Conclusion: 研究结果表明，在实践中应默认使用尾随窗口设计，当边际ROC AUC增益很重要时，可考虑添加事件计数窗口。间隔窗口和分桶窗口在此实验设置下表现不佳，不推荐使用。
...
### [98] [BPE: Behavioral Profiling Ensemble](https://arxiv.org/abs/2601.10024)
*Yanxin Liu,Yunqi Zhang*

Main category: cs.LG

TL;DR: BPE框架通过构建模型内在的行为画像，基于模型对测试实例的响应与其行为画像的偏差来确定集成权重，超越了传统基于模型间分歧的集成方法。


<details>
  <summary>Details</summary>
Motivation: 传统静态集成方法（如Stacking）将每个基学习器视为整体分配权重，忽略了模型在不同实例空间区域的性能差异。动态集成选择（DES）虽能局部调整，但仍主要依赖模型间分歧进行集成，忽略了模型内在特性且严重依赖验证集进行能力估计。

Method: 提出行为画像集成（BPE）框架：为每个模型构建内在的“行为画像”，基于模型对特定测试实例的响应与其已建立行为画像的偏差来推导集成权重，实现从模型间分歧到模型内在特性偏差的范式转变。

Result: 在合成和真实数据集上的广泛实验表明，BPE框架衍生的算法在预测准确性、计算效率和存储资源利用方面均显著优于最先进的集成基线方法。

Conclusion: BPE框架通过关注模型内在行为特性而非模型间分歧，提供了一种更高效、更准确的集成学习新范式，在多个性能维度上优于传统方法。
...
### [99] [Unlabeled Data Can Provably Enhance In-Context Learning of Transformers](https://arxiv.org/abs/2601.10058)
*Renpu Liu,Jing Yang*
...
### [100] [Efficient Content-based Recommendation Model Training via Noise-aware Coreset Selection](https://arxiv.org/abs/2601.10067)
...
### [101] [Comparative Evaluation of Deep Learning-Based and WHO-Informed Approaches for Sperm Morphology Assessment](https://arxiv.org/abs/2601.10070)
*Mohammad Abbadi*

Main category: cs.LG

TL;DR: 研究比较了基于图像的深度学习模型(HuSHeM)与WHO标准结合系统性炎症反应指数(SIRI)的基线方法在精子形态学评估中的表现，发现HuSHeM在预测性能、校准和临床效用方面更优。


<details>
  <summary>Details</summary>
Motivation: 现有精子形态学评估依赖主观的人工判断，存在观察者间变异性和资源限制问题，需要更客观、可重复的评估方法。

Method: 开发并评估了基于高分辨率精子形态图像的深度学习模型(HuSHeM)，与临床基线方法WHO(+SIRI)进行比较，使用独立的临床队列进行验证，性能评估包括区分度、校准和临床效用分析。

Result: HuSHeM模型在ROC曲线下面积、PR曲线下面积方面表现更优，预测概率与实际结果更一致，且在临床相关阈值下显示出更高的净临床效益。

Conclusion: 基于图像的深度学习在精子形态学评估中比传统基于规则和炎症增强的标准具有更好的预测可靠性和临床效用，可作为生育筛查和转诊流程中的决策支持工具，但不能完全替代临床判断。
...
### [102] [Sparse-RL: Breaking the Memory Wall in LLM Reinforcement Learning via Stable Sparse Rollouts](https://arxiv.org/abs/2601.10079)
...
### [103] [Bayesian Meta-Analyses Could Be More: A Case Study in Trial of Labor After a Cesarean-section Outcomes and Complications](https://arxiv.org/abs/2601.10089)
...
### [104] [LeMoF: Level-guided Multimodal Fusion for Heterogeneous Clinical Data](https://arxiv.org/abs/2601.10092)
...
### [105] [Multilingual-To-Multimodal (M2M): Unlocking New Languages with Monolingual Text](https://arxiv.org/abs/2601.10096)
*Piyush Singh Pasi*

Main category: cs.LG

TL;DR: METAL是一个轻量级对齐方法，仅用英语文本学习少量线性层，将多语言文本嵌入映射到多模态空间，在多语言文本-图像检索和音频-文本检索上实现强大零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 多模态模型在英语上表现优越，但因多语言多模态资源有限，在其他语言上性能大幅下降。现有解决方案过度依赖机器翻译，且多语言文本建模进展未充分利用。

Method: 仅使用英语文本学习少量线性层（线性变换），将预训练多语言文本嵌入对齐到多模态空间。该方法轻量简单，无需多语言多模态训练数据。

Result: 在英语上达到基线性能（Recall@10为94.9%），在11种语言（10种未见）上平均Recall@10达89.5%，实现了强大的零样本迁移。此外，可泛化至音频-文本检索和跨语言文本-图像生成。

Conclusion: METAL通过仅用英语文本学习的轻量对齐，有效实现了多语言到多模态空间的映射，为解决多语言多模态资源匮乏问题提供了高效方案，并发布了多语言评估数据集以促进进一步研究。
...
### [106] [Step-by-Step Causality: Transparent Causal Discovery with Multi-Agent Tree-Query and Adversarial Confidence Estimation](https://arxiv.org/abs/2601.10137)
*Ziyi Ding,Chenfei Ye-Hao,Zheyuan Wang,Xiao-Ping Zhang*

Main category: cs.LG

TL;DR: Tree-Query是一个树状结构的LLM专家框架，通过序列查询实现无数据的成对因果发现，并提供可解释性判断和鲁棒性置信度评分


<details>
  <summary>Details</summary>
Motivation: 传统约束因果发现方法存在误差传播问题，而最近的LLM因果预测工具往往作为不透明的黑盒运行，缺乏置信度评估。需要一种能够提供可解释性判断并具有鲁棒性置信度的因果发现框架

Method: 构建树状结构的多专家LLM框架，将成对因果发现问题简化为关于后门路径、(不)依赖性、潜在混杂因素和因果方向的短序列查询。该框架提供渐进可识别的理论保证

Result: 在基于Mooij等人和UCI因果图的无需数据基准测试中，Tree-Query在结构度量上优于直接LLM基线；饮食-体重案例研究展示了混杂因素筛选和稳定的高置信度因果结论

Conclusion: Tree-Query提供了一种原则性方法，可以从LLMs获取无需数据的因果先验知识，能够补充下游数据驱动的因果发现
...
### [107] [Understanding and Preserving Safety in Fine-Tuned LLMs](https://arxiv.org/abs/2601.10141)
*Jiawen Zhang,Yangfan Hu,Kejia Chen,Lipeng He,Jiachen Ma,Jian Lou,Dan Li,Jian Liu,Xiaohu Yang,Ruoxi Jia*

Main category: cs.LG

TL;DR: 该论文提出了一种名为安全保护微调（SPF）的轻量级方法，用于解决LLM微调过程中的安全性与实用性之间的权衡困境，通过在几何层面上分离并保护低维安全子空间，实现同时保持任务性能与安全对齐。


<details>
  <summary>Details</summary>
Motivation: 现有LLM微调方法在尝试适应下游任务时，存在明显的安全-效用权衡困境：强调安全性会损害任务性能，而优先考虑效用则通常需要进行深度微调，这往往会导致严重的安全对齐退化，即使在微调数据完全无害的情况下也可能大大增加越狱攻击的脆弱性。

Method: 通过系统实证分析揭示安全梯度与效用梯度在几何空间上的特征：1）安全梯度位于低秩子空间，效用梯度则分布在更广泛的高维空间；2）这些子空间通常负相关，导致微调过程中的方向冲突；3）主导安全方向可通过单个样本高效估计。基于这些发现，提出SPF方法，显式地移除与低秩安全子空间冲突的梯度分量。

Result: 理论上，SPF保证了效用收敛的同时边界化了安全漂移。实证上，SPF在各种下游任务中持续保持了任务性能，恢复了几乎全部预训练阶段的安全对齐，即使在对抗性微调场景下也表现出色。该方法对深度微调和动态越狱攻击具有强大抵抗力。

Conclusion: 该研究为大语言模型微调过程中实现持续安全对齐提供了新的机制性理解和实用指导，揭示了梯度子空间的几何特征，并提出了能够同时维持安全对齐与任务性能的有效解决方案。
...
### [108] [Simple Network Graph Comparative Learning](https://arxiv.org/abs/2601.10150)
...
TL;DR: LOOKAT通过将键值缓存压缩技术与向量数据库相似性搜索方法结合，将注意力机制从访存密集型转变为计算密集型，实现高达64倍KV缓存压缩同时保持95.7%的输出保真度。


<details>
  <summary>Details</summary>
Motivation: 现有量化方法仅压缩KV缓存存储但未解决注意力计算过程中的带宽问题，因为计算注意力时需要将INT4/INT8格式的键反量化为FP16格式，这限制了边缘设备部署大型语言模型的效率。

Method: 将键向量分解为子空间，学习码书并通过查找表计算注意力表，应用乘积量化和非对称距离计算方法到Transformer架构中，将注意力机制数学上等价为内积相似性搜索问题。

Result: 在GPT-2上测试，实现64倍压缩时输出保真度为95.7%，32倍压缩时为95.0%，无需架构修改或训练，保持秩相关系数ρ>0.95；理论分析显示秩相关退化与子空间数和码书大小成反比，在长达1024个token的序列长度下验证了保证。

Conclusion: LOOKAT有效将KV缓存压缩从纯存储优化扩展到带宽优化，将注意力计算从访存密集型转变为计算密集型，为边缘设备部署大型语言模型提供了高效的KV缓存压缩解决方案。
...
### [111] [Bias in the Shadows: Explore Shortcuts in Encrypted Network Traffic Classification](https://arxiv.org/abs/2601.10180)
...
### [112] [Graph Regularized PCA](https://arxiv.org/abs/2601.10199)
*Antonio Briola,Marwin Schmidt,Fabio Caccioli,Carlos Ros Perez,James Singleton,Christian Michler,Tomaso Aste*

Main category: cs.LG

TL;DR: 该算法是一种结合图正则化的PCA方法，通过拉普拉斯图惩罚项抑制高频信号、保留低频分量，以处理非球形噪声的高维数据，在保持预测性能的同时提升结构保真度。


<details>
  <summary>Details</summary>
Motivation: 高维数据的特征间常存在依赖关系，违反PCA所需的各向同性噪声假设。当噪声不是独立同分布时（协方差非球形），传统PCA不是最优的，需要一种能融合数据结构依赖性的降维方法。

Method: GR-PCA通过图正则化扩展PCA，主要包含两个核心机制：1) 学习稀疏精度矩阵获取特征依赖结构；2) 将载荷偏置至对应图拉普拉斯算子的低频傅里叶模式，抑制高频信号而保留图一致的低频分量。算法实现简单、模块化，并可扩展到大规模数据。

Result: 在多种图拓扑、信噪比和稀疏度的合成数据上，GR-PCA相比主流方法：a) 将方差集中于目标支撑集；b) 产生更低图拉普拉斯能量的载荷；c) 样本外重构保持竞争力；d) 高频信号存在时，图拉普拉斯惩罚防止过拟合，提升结构保真度（虽重构精度略有下降）。其在信号图谱相关时显著优于PCA，而在信号接近旋转不变时PCA仍具竞争力。

Conclusion: GR-PCA为结构感知降维提供了一个实用且可扩展的途径，在保持预测性能的同时，改善了结构保真度。其优势在于能够有效利用特征间的条件依赖关系，提升PCA在非球形噪声场景下的表现。
...
### [113] [PRL: Process Reward Learning Improves LLMs' Reasoning Ability and Broadens the Reasoning Boundary](https://arxiv.org/abs/2601.10201)
...
### [114] [Fundamental Limitations of Favorable Privacy-Utility Guarantees for DP-SGD](https://arxiv.org/abs/2601.10237)
...
### [115] [X-SAM: Boosting Sharpness-Aware Minimization with Dominant-Eigenvector Gradient Correction](https://arxiv.org/abs/2601.10251)
*Hongru Duan,Yongle Chen,Lei Guan*

Main category: cs.LG

TL;DR: 本文分析了SAM在优化过程中可能失效的情况，提出了基于谱几何视角的新方法X-SAM，通过特征向量对齐修正梯度，更有效地正则化Hessian最大特征值，提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统SAM方法旨在通过最小化最坏情况扰动损失来改善泛化能力，但其优化行为有时与理论预期不一致——尖锐和平坦区域都可能产生较小的扰动损失，导致梯度仍可能指向尖锐区域，SAM的锐度正则化效果减弱。本文从谱和几何角度研究此问题。

Method: 利用梯度与Hessian主特征向量的夹角作为锐度度量，提出显式特征向量对齐SAM（X-SAM）。该方法通过沿顶部特征向量的正交分解修正梯度，实现对Hessian最大特征值更直接有效的正则化。

Result: 理论证明了X-SAM的收敛性和更优泛化性能。大量实验评估证实了该方法的理论和实践优势，包括改进的泛化能力和更有效的锐度正则化。

Conclusion: X-SAM通过特征向量对齐机制解决了传统SAM在优化方向上的不一致问题，提供了更可靠、更有效的锐度感知优化方法，为提升深度学习模型泛化能力提供了新的理论视角和实践方案。
...
### [116] [Early Fault Detection on CMAPSS with Unsupervised LSTM Autoencoders](https://arxiv.org/abs/2601.10269)
...
### [117] [Queueing-Aware Optimization of Reasoning Tokens for Accuracy-Latency Trade-offs in LLM Servers](https://arxiv.org/abs/2601.10274)
*Emre Ozbas,Melih Bastopcu*

Main category: cs.LG

TL;DR: 论文研究大型语言模型服务器在异构查询流下的令牌分配优化问题。通过建立服务时间与准确率的权衡模型，在队列稳定性约束下，以加权准确率和平均系统时间为目标，得到最优令牌分配方案。


<details>
  <summary>Details</summary>
Motivation: 随着LLM服务器的广泛使用，需要处理不同类型任务的异构查询流。每种任务需要不同的计算资源（思考令牌），而令牌分配直接影响响应时间和准确率。现有研究缺乏系统性优化方法，需要在保证系统稳定性的前提下，平衡准确率与延迟的权衡。

Method: 1. 将系统建模为M/G/1队列，查询按泊松过程到达；2. 建立令牌分配与准确率（收益递减函数）和服务时间（近似仿射关系）的数学模型；3. 提出带约束的优化问题，最大化加权准确率并惩罚平均系统时间；4. 证明目标函数在稳定区域内严格凹性，确保最优解存在唯一；5. 推导一阶最优条件，得到耦合投影定点表征，并提出迭代解法；6. 开发投影梯度法保证收敛性；7. 通过舍入获得整数令牌分配。

Result: 1. 理论证明目标函数在稳定区域内严格凹，最优令牌分配存在且唯一；2. 推导出耦合投影定点条件，建立了迭代算法；3. 提供了全局步长界限保证投影梯度法收敛；4. 通过模拟评估了整数舍入解的性能损失，表明性能损失可控。

Conclusion: 该研究为LLM服务器的令牌分配优化提供了系统理论框架。通过建立准确的数学模型和分析方法，在保证系统稳定性的同时，实现了准确率与延迟之间的最优权衡。理论与算法结果具有实际应用价值，可用于指导实际LLM服务部署中的资源配置。
...
### [118] [SPIKE: Sparse Koopman Regularization for Physics-Informed Neural Networks](https://arxiv.org/abs/2601.10282)
...
### [119] [We Need a More Robust Classifier: Dual Causal Learning Empowers Domain-Incremental Time Series Classification](https://arxiv.org/abs/2601.10312)
...
### [120] [Meta Dynamic Graph for Traffic Flow Prediction](https://arxiv.org/abs/2601.10328)
*Yiqing Zou,Hanning Yuan,Qianyu Yang,Ziqiang Yuan,Shuliang Wang,Sijie Ruan*

Main category: cs.LG

TL;DR: 这篇论文提出了一个名为MetaDG的新框架，用于交通流量预测。该框架通过节点表示构建动态图结构来建模时空动态特性，解决了传统方法在动态建模范围有限以及时空异质性建模分离的局限性。


<details>
  <summary>Details</summary>
Motivation: 交通流预测作为典型的时空预测问题，面临的核心挑战在于建模复杂的时空依赖。现有方法虽然利用动态建模和时空异质性来改善表现，但仍存在两个主要局限：1）动态建模通常局限于空间拓扑动态（如邻接矩阵变化）；2）时空异质性建模通常被分别处理，缺乏统一框架。这些局限需要通过更全面的动态建模方法来解决。

Method: 提出MetaDG框架，利用节点表示构建的动态图结构来显式建模时空动态特性。该方法不仅生成动态邻接矩阵，还生成元参数，从而将动态建模扩展到拓扑之外，并将时空异质性的捕捉统一到一个维度中。

Result: 在四个真实世界数据集上进行的大量实验验证了MetaDG的有效性，表明该框架在交通预测任务中取得了良好的性能。

Conclusion: MetaDG通过将动态建模扩展到超越拓扑的范畴，并统一捕捉时空异质性，为解决交通流预测中的核心挑战提供了更全面的解决方案。这种基于动态图结构的方法为时空依赖性建模开辟了新途径。
...
### [121] [SuS: Strategy-aware Surprise for Intrinsic Exploration](https://arxiv.org/abs/2601.10349)
*Mark Kashirskiy,Ilya Makarov*

Main category: cs.LG

TL;DR: SuS提出了一种新颖的内在动机框架，通过策略稳定性与策略惊喜的组合鼓励强化学习中的探索，在数学推理任务上显著提升了准确性和策略多样性。


<details>
  <summary>Details</summary>
Motivation: 传统基于好奇心的探索方法仅依赖状态预测误差，但忽略了策略层面的行为一致性和策略相关的惊喜信号，限制了在复杂推理任务中的探索效率和多样性。

Method: 引入两个互补组件：策略稳定性（SS）衡量跨时间步行为策略的一致性；策略惊喜（SuS）捕捉相对于当前策略表示的非预期结果。通过学习的权重系数将两个信号结合到奖励公式中。

Result: 在大型语言模型的数学推理任务上，SuS实现了Passt@1提升17.4%、Passt@5提升26.4%；消融实验显示移除任一组件至少导致10%性能下降；训练中保持更高的策略多样性。

Conclusion: 策略层面的惊喜信号与稳定性信号具有协同作用，整合两者能显著增强探索效率，在多步骤推理任务中提升性能和多样性，展示了基于策略的内部动机框架的有效性。
...
### [122] [EvoMorph: Counterfactual Explanations for Continuous Time-Series Extrinsic Regression Applied to Photoplethysmography](https://arxiv.org/abs/2601.10356)
*Mesut Ceylan,Alexis Tabin,Patrick Langer,Elgar Fleisch,Filipe Barata*

Main category: cs.LG

TL;DR: 提出EvoMorph框架，为时间序列外生回归（TSER）生成生理学上可信的逆向事实解释（CFE），通过进化优化和形态感知目标，提升临床时间序列模型的可信度分析。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列CFE方法主要针对分类任务，忽略波形形态，且生成的信号缺乏生理学可信度，难以应用于连续的生物医学时间序列（如PPG），无法满足临床推理所需的稳定性评估和"假设"分析需求。

Method: EvoMorph：一个多目标进化框架，基于可解释的信号描述符定义形态感知目标，通过转换操作保持波形结构，生成多样且生理学可信的CFE。

Result: 在三个PPG数据集（心率、呼吸率、血氧饱和度）上评估，优于最近非相似邻居基线；案例研究表明EvoMorph能将逆向事实敏感性与Bootstrap集成不确定性、数据密度关联，支持不确定性量化的可解释性。

Conclusion: EvoMorph能够为连续生物医学信号生成生理学感知的逆向事实，支持不确定性感知的可解释性，推动了临床时间序列应用中的可信模型分析。
...
### [123] [PLGC: Pseudo-Labeled Graph Condensation](https://arxiv.org/abs/2601.10358)
*Jay Nandy,Arnab Kumar Mondal,Anuj Rathore,Mahesh Chandran*

Main category: cs.LG

TL;DR: 论文提出了PLGC，一种无需真实标签的自监督图压缩框架，通过潜在伪标签来匹配原图的结构和特征统计特性，在标签稀缺、噪声或分布偏移情况下表现鲁棒。


<details>
  <summary>Details</summary>
Motivation: 现有图压缩方法依赖干净的监督标签，在标签稀缺、噪声或不一致时可靠性受限，需要一种无需真实标签的鲁棒压缩方法。

Method: PLGC采用自监督框架，从节点嵌入构造潜在伪标签，通过联合学习潜在原型和节点分配来优化压缩图，使其匹配原始图的结构和特征统计特性。

Result: 在节点分类和链接预测任务中，PLGC在干净数据集上与最先进的监督压缩方法性能相当，在标签噪声下表现出显著鲁棒性，通常大幅优于所有基线方法。

Conclusion: 自监督图压缩方法在噪声或弱标签环境中具有理论和实际优势，为图数据高效学习提供了新范式。
...
### [124] [Reinforcement Learning with Multi-Step Lookahead Information Via Adaptive Batching](https://arxiv.org/abs/2601.10418)
*Nadav Merlis*
...
### [125] [DeFlow: Decoupling Manifold Modeling and Value Maximization for Offline Policy Extraction](https://arxiv.org/abs/2601.10471)
*Zhancun Mu*
...
### [126] [Projected Microbatch Accumulation yields reference-free proximal policy updates for reinforcement learning](https://arxiv.org/abs/2601.10498)
*Nilin Abrahamsen*
...
### [127] [Transformer-Based Cognitive Radio: Adaptive Modulation Strategies Using Transformer Models](https://arxiv.org/abs/2601.10519)
...
### [128] [Mixtures of Transparent Local Models](https://arxiv.org/abs/2601.10541)
...
### [129] [Process-Guided Concept Bottleneck Model](https://arxiv.org/abs/2601.10562)
*Reza M. Asiyabi,SEOSAW Partnership,Steven Hancock,Casey Ryan*

Main category: cs.LG

TL;DR: 该论文提出了过程引导概念瓶颈模型（PG-CBM），通过引入领域特定因果机制来改进标准概念瓶颈模型，提高其在科学应用中的准确性、可解释性和可信度。


<details>
  <summary>Details</summary>
Motivation: 标准的概念瓶颈模型（CBMs）存在几方面局限性：1）忽略领域特定的关系和因果机制；2）依赖完整的概念标签监督，在科学领域中监督稀疏时适用性受限；3）缺乏对科学应用中可靠性的保证。作者旨在扩展CBMs，使其能够利用领域知识并适应有限的监督情况。

Method: 提出了过程引导概念瓶颈模型（PG-CBM），该模型通过引入生物物理上有意义的中间概念来约束学习过程，使其遵循领域定义的因果机制。使用地球观测数据中的地上生物量密度估计作为案例研究，展示了如何利用多源异构训练数据并产生可解释的中间输出。

Result: 与多个基准模型相比，PG-CBM减少了误差和偏差，同时利用了多源异构训练数据并产生可解释的中间输出。除了提高准确性外，PG-CBM还增强了透明度，能够检测虚假学习，并提供科学见解。

Conclusion: PG-CBM代表了在科学应用中向更可信赖的人工智能系统迈出的一步，通过结合领域知识和因果机制，平衡了模型性能和可解释性，为稀疏监督下的科学分析提供了有前景的解决方案。
...
### [130] [Combinatorial Optimization Augmented Machine Learning](https://arxiv.org/abs/2601.10583)
*Maximilian Schiffer,Heiko Hoppe,Yue Su,Louis Bouvier,Axel Parmentier*

Main category: cs.LG

TL;DR: 这篇综述文章系统性地探讨了组合优化增强机器学习（COAML）这一新兴领域，它通过将组合优化模块嵌入学习流程，构建数据驱动且保持可行性的决策策略。


<details>
  <summary>Details</summary>
Motivation: 组合优化与机器学习结合已成为一个重要研究方向，通过嵌入组合优化模块到学习流程中，能够构建既数据驱动又保持可行性的决策策略，弥补机器学习、运筹学和随机优化之间的传统鸿沟。

Method: 建立了COAML的统一框架，描述了其方法论构建模块，并将它们与经验成本最小化建立形式化联系；基于不确定性和决策结构形式建立了问题设置分类法；回顾了静态和动态问题的算法方法；对调度、车辆路径、随机规划和强化学习等领域的应用进行了调查。

Result: 系统地综述了COAML领域的最新研究进展，包括方法论的构建、问题分类、算法回顾和应用调研，为组合优化和机器学习交叉领域的研究提供了全面概览。

Conclusion: 该综述既是该领域的入门教程，也为组合优化和机器学习交叉领域的未来研究提供了路线图，并识别了关键的研究前沿。
...
### [131] [ProbFM: Probabilistic Time Series Foundation Model with Uncertainty Decomposition](https://arxiv.org/abs/2601.10591)
...
### [132] [Single-Stage Huffman Encoder for ML Compression](https://arxiv.org/abs/2601.10673)
...
### [133] [On the origin of neural scaling laws: from random graphs to natural language](https://arxiv.org/abs/2601.10684)
...
### [134] [Data-driven stochastic reduced-order modeling of parametrized dynamical systems](https://arxiv.org/abs/2601.10690)
...
### [135] [Communication-Efficient and Privacy-Adaptable Mechanism -- a Federated Learning Scheme with Convergence Analysis](https://arxiv.org/abs/2601.10701)
...
### [136] [Distributed Perceptron under Bounded Staleness, Partial Participation, and Noisy Communication](https://arxiv.org/abs/2601.10705)
...
### [137] [High-accuracy and dimension-free sampling with diffusions](https://arxiv.org/abs/2601.10708)
*Khashayar Gatmiry,Sitan Chen,Adil Salim*

Main category: cs.LG

TL;DR: 提出了一种基于低阶逼近和配点法的新扩散模型求解器，实现了迭代复杂度的对数级改进。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型求解器需要通过离散化求解微分方程，迭代复杂度随维度和精度呈多项式增长，计算成本高。本文旨在开发一种迭代复杂度与精度呈多对数关系的新求解器，减少计算负担。

Method: 设计了一种新求解器，结合低阶逼近和配点法（Lee, Song, Vempala 2018），仅需近似访问数据分布的得分函数，实现高效采样。

Result: 证明了新求解器的迭代复杂度与1/ε呈多对数关系，且不显式依赖于环境维度，仅通过目标分布支撑集的有效半径影响复杂度，首次为基于扩散的采样器提供了“高精度”保证。

Conclusion: 该方法显著降低了扩散模型采样的计算复杂度，为实现高效采样的扩散模型提供了理论框架，特别是对高维数据具有潜在优势。
...
### [138] [DInf-Grid: A Neural Differential Equation Solver with Differentiable Feature Grids](https://arxiv.org/abs/2601.10715)

...
TL;DR: ProMediConv是首个针对法律纠纷调解场景的前瞻性对话代理基准框架，基于972个真实案例构建高保真数据集，并提出细粒度评估指标MAD（Mean Attribute Difference）以捕捉调解过程中当事人行为模式（BP）的动态变化。实验表明现有LLM在多方法略、动态多方调解中仍面临显著挑战，而基于该数据集训练的ProMediAgent基线显著优于各类通用与法律专用大模型。


<details>
  <summary>Details</summary>
Motivation: 现有LLM调解研究存在三大局限：任务建模脱离真实调解动态（忽视调解员的主动引导角色与多方交互）、数据保真度低（依赖合成对话或静态法律文档）、评估指标粗粒度（仅关注效率与最终结果，忽略对话过程中当事人心理状态的渐进变化）。这些问题阻碍了AI辅助冲突解决领域的发展。

Method: 1) 将调解流程建模为三阶段（理解Understanding、介入Intercession、和解Reconciliation）的主动多方对话框架；2) 归纳11种调解策略与4类当事人行为模式（BP）状态；3) 从15本已出版调解书籍中采集981个真实案例（过滤后972个），通过五智能体流水线（文本净化、Legal Counsel、Mediation Secretary、对话重建、BP分类器）将非结构化文本重建为结构化多轮对话；4) 提出MAD指标量化当事人BP在对话前后的平均变化；5) 训练解耦的ProMediAgent基线（RoBERTa策略规划器 + Qwen2.5-7B响应生成器，先SFT后REINFORCE强化学习）；6) 在通用LLM（Qwen2.5、Llama3.1、GLM-4、ChatGPT）+ Proactive/ProCoT/ICL_AIF策略规划方法、法律专用LLM（Fuzi-Mingcha、Wisdom-Interrogatory）上进行综合基准测试。

Result: ProMediAgent在所有指标上显著优于基线：SR@20达0.9071（提升17.93%）、SSR达0.9428、AT降至10.34、MAD达0.9893；策略规划方法对通用LLM效果不稳定且token消耗翻倍至三倍；法律专用LLM表现较差（Fuzi-Mingcha SR@20仅0.6295）；消融实验验证11类受限策略集优于开放策略空间；随机策略选择导致性能大幅下降（SR@20仅0.6769）；MAD与人类判断的成功率相关性（r=0.7386）显著高于SR@20；案例研究揭示"捷径式调解"现象（ChatGPT/ProCoT仅12轮达成表面协议但MAD仅1.5，而ProMediAgent用14轮实现MAD=3.0的更彻底解决）。

Conclusion: ProMediConv为AI辅助冲突解决提供了严谨基础与量化标准，证实结构化策略先验对规范调解代理动作空间至关重要，同时揭示当前LLM在动态多方调解中存在的持续局限性。该框架数据集与代码已开源，但目前仅限于中国法律语境下的文本对话，未来应拓展至多模态与跨文化跨语言场景。
...
TL;DR: 本文研究美国国会议员的认识论取向（基于证据vs基于直觉的语言风格）是否在个体层面存在系统性差异，并考察其与意识形态极端程度、跨平台一致性以及立法有效性的关系。研究发现：意识形态越极端的议员在国会发言中使用的证据导向语言越少；EMI在国会发言和Twitter之间存在跨平台一致性，但Twitter整体EMI较低；EMI与立法有效性得分（LES）显著正相关。


<details>
  <summary>Details</summary>
Motivation: 先前研究已发现自20世纪70年代中期以来，美国国会全体发言中的证据导向语言持续下降，并与立法生产率和极化趋势相关联。但这些研究主要聚焦于国会会议的聚合层面，缺乏对个体议员层面差异、跨平台一致性以及与立法有效性关系的探讨。立法过程本质上是人际互动和制度运行的结合，如果证据导向语言有助于建立共享的评估标准，则其可能影响个体议员的立法有效性。

Method: 1. 使用Evidence-Minus-Intuition (EMI) 分数衡量文本中的证据导向与直觉导向语言的相对比例，结合三个大语言模型（Llama-3.1-8B-Instruct、Qwen2.5-7B-Instruct、Apertus-8B-Instruct-2509）的评分与基于mGTE嵌入的语义相似度得分。2. 数据来源：美国国会发言记录（1873-2024，第43-118届国会）和国会议员Twitter/X帖子（2013-2022）。3. 意识形态极端程度用DW-NOMINATE第一维度的绝对值衡量。4. 立法有效性用立法有效性得分（LES）衡量。5. 使用线性混合效应模型（lme4包），在议员层面聚类的CR2稳健标准误，控制党派、议院、任期、委员会主席身份、领导职位、投票份额、议题构成和沟通量等变量。

Result: 1. 意识形态极端程度与EMI显著负相关（b = -0.116, p < 0.05），表明意识形态越极端的议员在国会发言中使用的证据导向语言越少，两党均呈现此模式。2. EMI在国会发言和Twitter之间存在跨平台一致性（Pearson r = 0.442, p < 0.05），但Twitter整体EMI显著较低（b = -0.150, p < 0.05），表明议员存在稳定的个人认识论取向同时会根据沟通环境调整语言风格。3. EMI与立法有效性得分（LES）显著正相关（b = 0.163, p < 0.05），即使在控制意识形态、党派、任期、委员会主席身份、领导职位、议题构成和沟通量等广泛变量后仍稳健。

Conclusion: 证据导向语言不仅是国会话语的聚合特征，也是个体议员层面立法沟通和有效性的有意义属性。意识形态极端程度、跨平台一致性和立法有效性三者均与个体的认识论取向相关。这表明证据导向沟通可能作为一种"认识论礼仪"（epistemic civility），有助于维护立法机构的议事规范、展现政策能力并促进同僚间的协调，对民主制度的有效运作具有制度性意义。
...
### [60] [What a Random Draw from the MCP Registry Contains, and What Tool-Use Benchmarks Contain Instead](https://arxiv.org/abs/2609.10962)
...
### [61] [Agent-Integrated Software: Interaction Contracts and Continuous Assurance](https://arxiv.org/abs/2609.11381)
...
### [62] [Ecdysis: Efficient and Effective Training of Runtime Harnesses for LLM Agents](https://arxiv.org/abs/2609.11677)
...
### [63] [A Model-Centric DevOps Architecture for DEVS-Based Digital Twin Simulation Services](https://arxiv.org/abs/2609.11122)
...
### [64] [When Passing Tests Hides Vulnerabilities: An Empirical Study of Silent Failures in Agentic Systems](https://arxiv.org/abs/2609.10548)
...
### [65] [Optimizing AI Inference Across the Deployment Stack](https://arxiv.org/abs/2609.10550)
...
### [66] [ReqEvolve: User-Oriented Software Self-Evolution through Automatic Requirement Interpretation](https://arxiv.org/abs/2609.10590)
...
### [67] [Generative AI for trustworthy systems - Towards a health check model](https://arxiv.org/abs/2609.10595)
...
### [68] [On the Relation between Code Quality and Machine Learning Performance: A Large-scale Empirical Study](https://arxiv.org/abs/2609.10610)
...
### [69] [AI Safety: Not Optional, Not Later](https://arxiv.org/abs/2609.10630)
...
### [70] [Numbat: Building and Verifying a Self-Contained Machine-Learning Stack](https://arxiv.org/abs/2609.10632)
*Thang Tran,Lan Dang*
...
### [71] [Governed Human-AI Prioritization Under Uncertainty: Adaptive Estimation and Dependency-Constrained Portfolio Selection](https://arxiv.org/abs/2609.10648)
...
### [72] [LLMVul: A Vulnerability-Labeled Dataset of LLM-Generated C/C++ Functions from Real Production Repositories](https://arxiv.org/abs/2609.10945)
...
### [73] [Engineering Reliable Commit Gates for Agentic AI: Cost-Aware Verification Portfolios under Common-Mode Data Failures](https://arxiv.org/abs/2609.10969)
...
### [74] [RCL: A Retrieval-Confidence Layer for Detecting Insufficient Context in Enterprise Retrieval-Augmented Code Generation](https://arxiv.org/abs/2609.11023)
...
### [75] [SaltBench: A Referee-Gated Protocol for Measuring Method Effects in Machine-Checked Software Work](https://arxiv.org/abs/2609.11076)
*Jason Hickey*
...
### [76] [FST Pay: Deterministic Safety-Gated Architecture for Youth Digital Payments](https://arxiv.org/abs/2609.11195)
...
### [77] [TripleBound: Triplet-Guided Heterogeneous Graph Learning for Microservice Decomposition](https://arxiv.org/abs/2609.11212)
...
### [78] [Exploring the Role of Security Experience and ChatGPT Usage Strategies on Secure Software Engineering Education](https://arxiv.org/abs/2609.11303)
...
### [79] [CoSTAR: Data Synthesis-Driven Constraint-Aware COBOL Section Summarization for Legacy System Modernization](https://arxiv.org/abs/2609.11332)
...
### [80] [Deep Learning-based Bug Triage System](https://arxiv.org/abs/2609.11420)
*Sourabh Pal*
...
### [81] [ChurnBench: A Drift-Aware Benchmark Demonstrating That Refresh Scheduling, Not Cache Age, Governs Staleness in Agentic AI](https://arxiv.org/abs/2609.11515)
...
### [85] [(Whose defaults?) Is artificial intelligence reorienting archaeological methods?](https://arxiv.org/abs/2609.11198)
...
### [86] [Characterizing Bluesky Content Moderation Service: From Automation of Service to Landscape of Harms](https://arxiv.org/abs/2609.11373)
...
### [87] [Buyer Artificial Intelligence-Enabled Environmental Governance and Supplier Environmental Controversies: An Organizational Information Processing and Signaling](https://arxiv.org/abs/2609.11391)
...
### [88] [Governing AI Research Through Peer Review: A Mixed-Methods Study of the Longitudinal Effects of Ethics Flags Across Resubmissions](https://arxiv.org/abs/2609.10740)
...
### [89] [Designing Technology for Social Wellbeing in Built Environments: A Conceptual Framework](https://arxiv.org/abs/2609.10779)
...
### [90] [Alternative AI Philosophy: Daoism as Method for AI in Education](https://arxiv.org/abs/2609.10842)
*Qin Xie*
...
### [91] [Following the Preference, Missing the Optimum: Compliance Without Optimization in AI Housing Recommendation](https://arxiv.org/abs/2609.10856)
*Hsuan Lo*
...
### [92] [Work, Wellbeing, and Choice: Empirical Lessons for AI Futures](https://arxiv.org/abs/2609.11019)
...
### [93] [From Digital Accountability to Accountable Digitality Through Needs-Aware Information Systems: The Case of Auditable Child-Welfare Judgments](https://arxiv.org/abs/2609.11125)
*Soheil Human*
...
### [94] [SoulAuth: An Actor-native Identity Architecture and Rust Reference Implementation for Humans and Long-lived AI Actors](https://arxiv.org/abs/2609.11258)
...
### [95] [A global mobile network coverage raster product at 1km resolution, 1999--2030](https://arxiv.org/abs/2609.11320)
...
### [96] [Who Bears the Risk When Generative AI Enters Transport? A Distributional Sociotechnical Audit of Algorithmic Equity, Synthetic-Data Validity, and Public Trust](https://arxiv.org/abs/2609.11611)
...
### [97] [A Deadline-Driven Algorithm for Polyamorous Scheduling](https://arxiv.org/abs/2609.10641)
...
### [98] [Lower Bounds for Private Graph Optimization Problems using Reconstruction Attacks](https://arxiv.org/abs/2609.10877)
...
### [99] [Quasi-Monte Carlo Beyond Hardy-Krause II: $(1 + \varepsilon)n$ Samples Suffice](https://arxiv.org/abs/2609.10921)
...
### [100] [A deterministic $(1+\varepsilon)^n$ approximation for the permanent of a nonnegative matrix](https://arxiv.org/abs/2609.11049)
...
### [101] [Single-Exponential Algorithms and a Polynomial Kernel for Strong Connectivity Augmentation](https://arxiv.org/abs/2609.11160)
...
### [102] [A Reusable Framework for Robust Approximation Algorithms in the Interval Uncertainty Model](https://arxiv.org/abs/2609.11621)
...
### [105] [ORCH: Organizational Principles Enable Collective Intelligence in Embodied AI](https://arxiv.org/abs/2609.11737)
...
### [106] [But How Would AI Agents Run a Town's Economy?](https://arxiv.org/abs/2609.11108)
...
### [107] [The Truth Was Never Gone: Perfect Aliasing in Compliant-Context Truth Probes](https://arxiv.org/abs/2609.10739)
*Dylan Jayabahu*
...
### [108] [Adaptive Margin Ordinal Loss: Penalizing Center-Class Hedging in Ordinal Classification](https://arxiv.org/abs/2609.10752)
*Manisha Kandel*
...
### [109] [Counterfactual Marginalisation: Framework for Evaluating Robustness to Nuisance Variables](https://arxiv.org/abs/2609.10778)
...
### [111] [AUC Maximization from Biased Positive-unlabeled Data with Confidence](https://arxiv.org/abs/2609.10928)
...
### [113] [Importance Weighting for Unlabeled-unlabeled Learning under Distribution Shift](https://arxiv.org/abs/2609.10994)
*Atsutoshi Kumagai,Tomoharu Iwata,Hiroshi Takahashi,Taishi Nishiyama,Kazuki Adachi,Yasuhiro Fujiwara*
...
### [114] [Topological Necessities: Mechanism-Invariant Strategic Subgoals for Cross-Embodiment Goal-Conditioned Control](https://arxiv.org/abs/2609.11014)
*Hao Shi,Xi Li*
...
### [115] [T1: Terminal Agent Reinforcement Learning for Long-Horizon Tasks](https://arxiv.org/abs/2609.11042)
...
### [116] [Solving Few-Shot Multiobjective Multitask Optimization via Iterative Sequential Transfer](https://arxiv.org/abs/2609.11228)
...
### [117] [Reification as a Transferable Vocabulary: Zero-Shot Link Prediction with Vanilla GNNs](https://arxiv.org/abs/2609.11347)
*Camille Pradel*
...
### [118] [Prevalence Determines Precision:Silent Contamination in Detector-Defined Datasets](https://arxiv.org/abs/2609.11449)
...
### [119] [LoaDiff: Conditional Generation of Electricity Consumption Time Series for Energy Analytics](https://arxiv.org/abs/2609.11639)
...
### [120] [Thinking with Looped Flows](https://arxiv.org/abs/2609.11801)
...
### [121] [Model-Aware Schedules Improve Generation via Fiberwise Optimal Transport](https://arxiv.org/abs/2609.11842)
...
### [122] [The Last AI Built by Humans: Toward Genuine Recursive Self-Improvement](https://arxiv.org/abs/2609.11873)
...
### [123] [General Quantification of Covariate and Concept Shifts](https://arxiv.org/abs/2609.11918)
...
### [124] [EMMI: Edge Multi-Modal Intelligence for Communication-Efficient MLLM Inference via Fused Representation Compression](https://arxiv.org/abs/2609.11058)
...
### [125] [Phase-Decoupled, Model-Calibrated Power Control for Disaggregated LLM Serving](https://arxiv.org/abs/2609.11133)
...
### [129] [M3-Former: Multimodal Transformer with Mixture-of-Experts for Long-Term Vessel Trajectory Prediction](https://arxiv.org/abs/2609.10559)
...
### [130] [Halo: Improving forecast accuracy through heteroscedastic estimation](https://arxiv.org/abs/2609.10589)
*Adam Cataldo*
...
### [131] [Zero-shot rib design: merging training-free generative prior with topology optimization](https://arxiv.org/abs/2609.10643)
...
### [132] [Byzantine-Robust Federated Fire Detection with a Rotating Coordinator](https://arxiv.org/abs/2609.10647)
...
### [133] [Artificial Intelligence Algorithms for the Detection of Pathologies Related to Lung Cancer through Image Analysis using Convolutional Neural Networks and Data Augmentation: a systematic mapping of the literature](https://arxiv.org/abs/2609.10652)
...
### [134] [GEOSTEER: Geodesic Optimization for Activation Steering in Large Language Models](https://arxiv.org/abs/2609.10658)
...
### [135] [Conformal Calibration Transfer](https://arxiv.org/abs/2609.10737)
*Achref Doula*
...
### [136] [A Bellman Optimality Equation for Plasticity](https://arxiv.org/abs/2609.10776)
...
### [137] [From Connectivity to Rewards: Dense Reward Learning with Directed State Graphs](https://arxiv.org/abs/2609.10781)
...
### [138] [RiVaT-Fuse: Reliability-Calibrated Variational Tensor Fusion for Multimodal Prediction under Modality Uncertainty](https://arxiv.org/abs/2609.10798)
...
### [139] [Processing and classifying bird songs using wavelet techniques and supervised learning](https://arxiv.org/abs/2609.10826)
...
### [140] [Flow Duality and Source Geometry for Categorical Generation](https://arxiv.org/abs/2609.10863)
*Etrit Haxholli*
...
### [141] [Certifying Lower Bounds for Risk-Sensitive Reinforcement Learning under Adversarial State Perturbations](https://arxiv.org/abs/2609.10866)
...
### [142] [Learning Orthogonal Multi-Index Models Beyond Small Initialization: Incremental Learning, Competitive Dynamics and Symmetry](https://arxiv.org/abs/2609.10879)
...
### [143] [Relatively Smart II: Tractable or Semi-Supervised Instance-Optimal Learning](https://arxiv.org/abs/2609.10886)
...
### [144] [Measuring the Value of World-Model Updates: A Counterfactual Utility Protocol for Continual Adaptation](https://arxiv.org/abs/2609.10954)
...
### [145] [When More Is Not Better: Component Anti-Synergy in a P300 Speller](https://arxiv.org/abs/2609.10961)
...
### [146] [Phases in a class of associative memories via hidden neurons](https://arxiv.org/abs/2609.10976)
...
### [147] [Thompson Sampling for Non-Monotone Convex Ridge Bandits: Monotonicity Is Not Needed for Polynomial Regret](https://arxiv.org/abs/2609.10981)
*Xuan Li*
...
### [152] [Bidirectional Multimodal Fusion of Sky Images and Time-Series for Solar Forecasting with Large Language Models](https://arxiv.org/abs/2609.11135)
...
### [153] [LILA: Calibration-Free Structured Pruning of Large Language Models via Latent Spectral Geometry](https://arxiv.org/abs/2609.11163)
...
### [154] [When does a spectral prior help graph learning? Connectivity-loss estimation under road-network disruptions](https://arxiv.org/abs/2609.11166)
*Van-Truong Le*
...
### [155] [Semi-Tensor Product-Based Multi-Term Randomized T-SVD and Its Visual Applications](https://arxiv.org/abs/2609.11168)
...
### [157] [Convex Optimization with Nested Evolving Feasible Sets (CONES) under Time-Varying Loss Functions](https://arxiv.org/abs/2609.11207)
*Rahul Vaze*
...
### [158] [REVA: Reusable Evidence View Aggregation for Context-Efficient RAG Serving](https://arxiv.org/abs/2609.11209)
...
### [159] [Legible Failures: Detecting and Repairing In-Context Binding Errors](https://arxiv.org/abs/2609.11216)
...
### [160] [Polyhedral Geometry of Time-to-First-Spike Neural Networks](https://arxiv.org/abs/2609.11227)
...
### [162] [A Dynamic Fusion Large Language Model for Traffic Flow Prediction](https://arxiv.org/abs/2609.11314)
*Xue Qiu,Jianli Xiao*
...
### [164] [Combining Synthetic and Real Data for Low-Resource Historical OCR: A Manchu Case Study](https://arxiv.org/abs/2609.11495)
...
### [165] [Generalized Score Matching for Parameter Estimation on Convex Domains](https://arxiv.org/abs/2609.11521)
...
### [166] [Particle GFlowNets: Rethinking Generative Marginalization Models](https://arxiv.org/abs/2609.11538)
...
### [167] [A Dataset and Model for Imputing Water Surface Elevation on a Large and Extremely Sparse Spatiotemporal Graph](https://arxiv.org/abs/2609.11580)
...
### [168] [RDDMPI: Residual Denoising Diffusion Model for Probabilistic Multivariate Time Series Imputation](https://arxiv.org/abs/2609.11648)
...
### [169] [Musec: MomentUm SpEctral Clipping for Stable Muon-type Training](https://arxiv.org/abs/2609.11655)
...
### [170] [Learnware and AI Model Management System](https://arxiv.org/abs/2609.11656)
*Zhi-Hua Zhou*
...
### [171] [Why Does Post-Training Quantization Work?](https://arxiv.org/abs/2609.11716)
...
### [173] [Dynamic language model representations for multi-objective reaction optimisation](https://arxiv.org/abs/2609.11790)
...
### [174] [AdamX: Cosine similarity meets gradient descent](https://arxiv.org/abs/2609.11867)
...
### [175] [CoRA-NAS: Coarse Ranking and Anchor-Residual Refinement for Neural Architecture Search](https://arxiv.org/abs/2609.11884)
...
### [176] [CausalArena: Benchmarking Causal Discovery in the Foundation Model Era](https://arxiv.org/abs/2609.11897)
...
### [177] [TART: A Modular Tool for Technique-Aware Audio-to-Tablature Guitar Transcription](https://arxiv.org/abs/2609.11904)
...
### [178] [From Protocols to Evidence: Bounded Claims for AI in Service of the Common Good](https://arxiv.org/abs/2609.11910)
...
### [179] [Data Scarcity and Model Sparsity: Mixtures-of-Experts Overfit More to Repeated Data](https://arxiv.org/abs/2609.11917)
...
### [180] [OmniTable: A Unified Wide-Table System for Petabyte-Scale LLM Data Curation and Exploration](https://arxiv.org/abs/2609.11148)
...
TL;DR: 本文针对扩散模型LoRA微调中秩(rank)选择缺乏系统性指导的问题，在CIFAR-10上对DDPM U-Net和Tiny DiT两种骨干网络进行了受控实证研究。结果表明，在固定优化预算下，中等秩(尤其是rank 4)提供了最佳的质量-效率权衡，更高秩的边际收益递减。


<details>
  <summary>Details</summary>
Motivation: LoRA已成为扩散模型参数高效微调的主流方法，但其秩选择在实际中常被非正式处理（直接沿用语言模型文献的默认值或粗略扫描），缺乏在受控条件下对秩与微调质量关系的系统性表征。现有自适应秩分配方法虽有效但引入了额外复杂性，难以隔离固定秩的基线效应。因此需要在统一优化设置下进行控制变量的秩选择实证研究，为实践者提供可复现的参考基线。

Method: 设计三条受控实验轨道：(1) 主DDPM秩扫描——在CIFAR-10(32×32)上对DDPM U-Net遍历rank∈{2,4,8,16,32}，训练10个epoch；(2) 扩展预算DDPM验证——对rank∈{4,8,16}训练20个epoch；(3) Tiny DiT跨骨干验证——对轻量Transformer2DModel在rank∈{4,8,16}上训练10个epoch。所有实验统一batch size=64、学习率=1e-4、仅更新LoRA适配器参数。LoRA注入注意力投影模块(to_q/k/v/out)。评估使用本地pytorch-fid协议（固定CIFAR-10测试参考集，2000张生成图像），同时记录可训练参数数、运行时、峰值GPU内存和最终训练损失。

Result: 主DDPM扫描中，rank 4获最佳FID(124.1380)，rank 8接近(124.2136)，rank 16和32 FID反而更差(132.25/131.65)，呈非单调关系。扩展预算(20 epoch)下rank 16仅小幅改善(131.3555)，未逆转中等秩优势；rank 4仍最佳(130.8580)。Tiny DiT验证显示一致的秩排序趋势：rank 4最佳(383.24)，rank 8次之(387.81)，rank 16最差(402.03)。运行时和内存开销随秩线性增长但幅度有限(内存约2048→2074 MB)，而质量在rank 4后即饱和。可训练参数从rank 2的49152线性增至rank 32的786432(占比0.04%→0.69%)。

Conclusion: 在固定优化预算下，LoRA秩应被视为预算受限的系统决策而非"越大越好"的缩放旋钮。中等秩(尤其rank 4)一致提供最佳质量-效率权衡，且该趋势在两种不同架构(卷积U-Net与Transformer)和不同训练预算下均成立。FID值在低分辨率、受限预算下应作为相对优化指标而非绝对质量度量。该研究建议实践者优先尝试中等秩，并同时追踪质量与系统指标(运行时、内存)，为采用更复杂自适应秩分配策略提供受控基线参考。
...
### [253] [Explainability Assistant: A Conversational XAI Interface for Interpreting Energy Consumption Models](https://arxiv.org/abs/2609.11860)
...
### [254] [On the Regularization Landscape for the Linear Recommendation Models](https://arxiv.org/abs/2609.11876)
...
### [255] [MindTopo: Can Foundation Models Reason in Topological Space?](https://arxiv.org/abs/2609.11900)
...
### [258] [GPU-CFR: 80x Faster Counterfactual Regret Minimization by Compiling the Game to Static Dataflow and CUDA Graph Replay](https://arxiv.org/abs/2609.11923)
...
### [259] [The Internet of Collaborating Things: Agentic Edge AI for Autonomous Cross-Domain Collaboration](https://arxiv.org/abs/2609.10542)
...
### [260] [MUC-FL: Block-Wise Marginal Utility Contribution for Communication-Efficient Federated Learning](https://arxiv.org/abs/2609.10545)
...
### [261] [From Token Interfaces to Token Semantics: A Formal Composition and Conformance Model for Implementation-Neutral Token Specifications](https://arxiv.org/abs/2609.10547)
*John deVadoss*
...
### [262] [Design and Operation of a Federated GPU Cluster for Digital Humanities within DHinfra.at](https://arxiv.org/abs/2609.10552)
...
### [263] [Memory Profiling and Migration for Heterogeneous Memory Architectures](https://arxiv.org/abs/2609.10554)
...
### [264] [Composable CXL Memory as a Kubernetes-Native Shared Memory for LLM Serving](https://arxiv.org/abs/2609.10790)
...
### [265] [ExaServe: Large-Scale LLM Serving on Exascale HPC Systems](https://arxiv.org/abs/2609.10812)
...
### [266] [Navigating Small-World Networks with Distance Predictions](https://arxiv.org/abs/2609.10885)
...
### [267] [Online Treasure Hunt in Vertex-Permuted Dynamic Rings](https://arxiv.org/abs/2609.11013)
...
### [268] [The Computing Channel: How Modulation Programs the Airwaves](https://arxiv.org/abs/2609.11145)
...
### [269] [Can AI Remediate Backend Failures Safely? GuardedAct with Blast-Radius-Aware Sandboxing](https://arxiv.org/abs/2609.11264)
...
### [270] [Rethinking Sparse Formats for RISC-V: A Hierarchical Approach to High-Performance SpMV](https://arxiv.org/abs/2609.11352)
...
### [271] [Taming Bitwise Behavior in GPU Kernels with Tensor Core: Black-Box Reconstruction, Compiler Enforcement, and Static Verification](https://arxiv.org/abs/2609.11356)
...
### [272] [Entwine: Coordinating Tiled Computation and Fine-Grained Communication across GPUs](https://arxiv.org/abs/2609.11562)
...
### [273] [Building py-kvcache: A Performance Characterization of External KV Caching for vLLM with NVMe SSDs](https://arxiv.org/abs/2609.11744)
...
### [274] [VikingRAG: Accurate and Token-efficient Retrieval-augmented Generation over Structured Documents](https://arxiv.org/abs/2609.11390)
...
### [275] [When Synthetic Data Hurts: On Catastrophic Forgetting in Skill Retrieval for LLM Agents](https://arxiv.org/abs/2609.10750)
...
### [276] [UniRec: Cross-stage Multi-Task Fusion with Preference Alignment for Cascaded Recommender Systems](https://arxiv.org/abs/2609.11052)
...
### [277] [TimelyRAG: Semantic-Temporal Hybrid Retrieval for Time-Critical Question Answering in Overlapping-Evolving Documents](https://arxiv.org/abs/2609.11572)
...
### [278] [FedHUR: Learning Hierarchical Utility-Guided Client Relations for Personalized Federated Recommendation](https://arxiv.org/abs/2609.11632)
...
### [279] [Your Retriever Already Knows: Distribution-Shape QPP for RAG Retrieval Sufficiency](https://arxiv.org/abs/2609.11646)
...
### [280] [Generative Late-Interaction Embeddings For Visual Document Retrieval](https://arxiv.org/abs/2609.11808)
